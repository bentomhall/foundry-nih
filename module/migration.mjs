...
export const migrateWorld = async function() {
  ...
  for ( const [actor, valid] of actors ) {
    try {
      ...
      err.message = `Failed nih system migration for Actor ${actor.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const [item, valid] of items ) {
    try {
      ...
      err.message = `Failed nih system migration for Item ${item.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed nih system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const table of game.tables ) {
    try {
      ...
      err.message = `Failed nih system migration for RollTable ${table.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes ) {
    try {
      ...
      err.message = `Failed nih system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
...
export async function refreshCompendium(pack) {
  ...
  nih.moduleArt.suppressArt = true;
  ...
  await pack.configure({locked: wasLocked});
  nih.moduleArt.suppressArt = false;
