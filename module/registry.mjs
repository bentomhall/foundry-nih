...
class MessageRegistry {
  ...
  static track(message) {
    const origin = message.getFlag("nih", "originatingMessage");
    const type = message.getFlag("nih", "roll.type");
...
class SpellListRegistry {
   ...
   */
  static get options() {
    return Object.entries(CONFIG.NIH.spellListTypes).map(([type, group]) => {
   ...
   * @param {string} type          Type of list as defined in `CONFIG.NIH.spellListTypes`. Can also be a combination
...
export class SpellList {
   ...
   */
  get name() {
    return nih.registry[SpellList.#REGISTRIES[this.metadata.type]]?.get(this.metadata.identifier)?.name
      ?? this.metadata.name;
  }
