...
export default class AbilityTemplate extends foundry.canvas.placeables.MeasuredTemplate {
  ...
  static fromActivity(activity, options={}) {
    ...
    const templateShape = nih.config.areaTargetTypes[target.type]?.template;
    ...
    switch ( templateShape ) {
      ...
      case "rect": // 5e rectangular AoEs are always cubes
        ...
        if ( game.settings.get("nih", "gridAlignedSquareTemplates") ) {
     ...
     * @function nih.preCreateActivityTemplate
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("nih.preCreateActivityTemplate", activity, templateData) === false ) return null;
     ...
     * @function nih.createActivityTemplate
     * @memberof hookEvents
     * @param {Activity} activity            Activity for which the template is being placed.
     * @param {AbilityTemplate[]} templates  The templates being placed.
     */
    Hooks.callAll("nih.createActivityTemplate", activity, created);
  ...
  _onMovePlacement(event) {
    ...
    const baseDistance = this.document.flags.nih?.dimensions?.size;
    if ( this.document.flags.nih?.dimensions?.adjustedSize && baseDistance ) {
