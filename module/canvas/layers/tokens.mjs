export default class TokenLayer5e extends foundry.canvas.layers.TokenLayer {
  ...
  isOccupiedGridSpaceBlocking(gridSpace, token, { preview=false }={}) {
    const tokenSize = CONFIG.NIH.actorSizes[token.actor?.system.traits.size]?.numerical ?? 2;
    const modernRules = game.settings.get("nih", "rulesVersion") === "modern";
    const halflingNimbleness = token.actor?.getFlag("nih", "halflingNimbleness");
    const found = this.#getRelevantOccupyingTokens(gridSpace, token, { preview }).filter(t => {
      ...
      if ( t.actor.statuses.intersects(CONFIG.NIH.neverBlockStatuses) ) return false;

      const size = t.actor.system.details?.type?.swarm || t.actor.system.traits?.size;
      const occupiedSize = CONFIG.NIH.actorSizes[size]?.numerical ?? 2;
     ...
     * @function nih.determineOccupiedGridSpaceBlocking
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("nih.determineOccupiedGridSpaceBlocking", gridSpace, token, { preview }, found);
    return found.size > 0;
  }
  ...
  isOccupiedGridSpaceDifficult(gridSpace, token, { preview=false }={}) {
    ...
    const found = this.#getRelevantOccupyingTokens(gridSpace, token, { preview }).filter(t => {
      ...
      const occupiedSize = CONFIG.NIH.actorSizes[t.actor?.system.traits.size]?.numerical ?? 2;
     ...
     * @function nih.determineOccupiedGridSpaceDifficult
     * @memberof hookEvents
     * @param {GridOffset3D} gridSpace  The grid space being checked.
     * @param {Token5e} token           The token being moved.
     * @param {object} options          Additional options.
     ...
     */
    Hooks.callAll("nih.determineOccupiedGridSpaceDifficult", gridSpace, token, { preview }, found);
    return found.size > 0;
  }
