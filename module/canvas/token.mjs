...
export default class Token5e extends foundry.canvas.placeables.Token {
  ...
  findMovementPath(waypoints, options) {
    ...
    if ( (game.settings.get("nih", "movementAutomation") !== "full") || !this.document.actor?.system.isCreature
      || this.document.actor.statuses.intersects(CONFIG.NIH.neverBlockStatuses) ) {
  ...
  _getMovementCostFunction(options) {
    ...
    if ( game.settings.get("nih", "movementAutomation") === "none" ) return costFunction;
  ...
  constrainMovementPath(waypoints, options) {
    ...
    ignoreTokens ||= game.settings.get("nih", "movementAutomation") !== "full";
    ignoreTokens ||= !this.actor?.system.isCreature;
    ignoreTokens ||= this.actor?.statuses?.intersects(CONFIG.NIH.neverBlockStatuses);
  ...
  _drawHPBar(number, bar, data) {
    ...
    const hpColor = nih.documents.Actor5e.getHPColor(value, effectiveMax);

    // Determine colors to use
    const blk = 0x000000;
    const c = CONFIG.NIH.tokenHPColors;
