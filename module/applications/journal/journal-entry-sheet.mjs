...
export default class JournalEntrySheet5e extends foundry.applications.sheets.journal.JournalEntrySheet {
  ...
  static DEFAULT_OPTIONS = {
    classes: ["nih2", "nih2-journal", "titlebar"]
  };
  ...
  /**
   * Append the nih system styling classes to journal pages inside system journal sheets.
  ...
  static onRenderJournalPageSheet(page, element) {
    if ( page.document.parent?.sheet instanceof JournalEntrySheet5e ) {
      element.classList.add("nih2", "nih2-journal", "titlebar", "dialog-lg");
    }
  }
  ...
  static async _injectNavigation(entry, html) {
    const nav = entry.getFlag("nih", "navigation");
    if ( !nav ) return;
  ...
  static #makeNavigation(doc, dir) {
    ...
    Object.assign(anchor.dataset, { link: "", tooltip: `NIH.JOURNALENTRY.Navigation.${i18n[dir]}`, uuid: doc.uuid });
