...
export default class SpellsUnlinkedConfig extends DocumentSheet5e {
  ...
  static PARTS = {
    spell: {
      template: "systems/nih/templates/journal/spell/unlinked-spell.hbs"
    },
    source: {
      template: "systems/nih/templates/journal/spell/unlinked-source.hbs"
    }
  };
  ...
  /** @inheritDoc */
  get title() {
    return game.i18n.localize("JOURNALENTRYPAGE.NIH.SpellList.UnlinkedSpells.Configuration");
  }
  ...
  async _prepareContext(options) {
    const context = {
      ...
      spellLevelOptions: Object.entries(CONFIG.NIH.spellLevels).map(([value, label]) => ({ value, label })),
      spellSchoolOptions: Object.entries(CONFIG.NIH.spellSchools).map(([value, { label }]) => ({ value, label }))
    };
    return context;
  }
