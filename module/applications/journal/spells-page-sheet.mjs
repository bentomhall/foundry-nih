...
export default class JournalSpellListPageSheet extends JournalEntryPageHandlebarsSheet {
  ...
  static EDIT_PARTS = {
    ...
    config: {
      ...
      template: "systems/nih/templates/journal/spell/config.hbs"
    },
    list: {
      classes: ["right", "spell-list"],
      template: "systems/nih/templates/journal/spell/list.hbs"
    }
  };
  ...
  static VIEW_PARTS = {
    content: {
      root: true,
      template: "systems/nih/templates/journal/spell/view.hbs"
    }
  };
  ...
  static get GROUPING_MODES() {
    return nih.dataModels.journal.SpellListJournalPageData.GROUPING_MODES;
  }
  ...
  _configureRenderParts(options) {
    ...
    if ( ("content" in parts) && this.options.displayAsTable ) {
      parts.content.template = "systems/nih/templates/journal/spell/table.hbs";
    }
    return parts;
  }
    ...
    for ( const data of context.spells ) {
      ...
      switch ( context.grouping ) {
        case "level":
          ...
          section = context.sections[level] ??= { header: CONFIG.NIH.spellLevels[level], spells: [] };
          break;
        case "school":
          const school = spell.system.school;
          section = context.sections[school] ??= { header: CONFIG.NIH.spellSchools[school]?.label, spells: [] };
          break;
        case "alphabetical":
