...
export default class JournalClassPageSheet extends JournalEntryPageHandlebarsSheet {
  ...
  static EDIT_PARTS = {
    ...
    config: {
      ...
      template: "systems/nih/templates/journal/page-{type}-edit.hbs"
    }
  };
  ...
  static VIEW_PARTS = {
    content: {
      root: true,
      template: "systems/nih/templates/journal/page-{type}-view.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.styleOptions = [
      { value: "", label: game.i18n.localize("JOURNALENTRYPAGE.NIH.Class.Style.Inferred") },
      { rule: true },
      { value: "2024", label: game.i18n.localize("JOURNALENTRYPAGE.NIH.Class.Style.Modern") },
      { value: "2014", label: game.i18n.localize("JOURNALENTRYPAGE.NIH.Class.Style.Legacy") }
    ];
    ...
    if ( linked.system.primaryAbility ) {
      ...
      ).format(Array.from(linked.system.primaryAbility.value).map(v => CONFIG.NIH.abilities[v]?.label));
    }
  ...
  async _getSpellProgression(item) {
    ...
    const spellcastingModel = CONFIG.NIH.spellcasting[spellcasting.type];
    const table = {};
    ...
    if ( spellcastingModel?.isSingleLevel ) {
      ...
      const maxSpellLevel = Object.keys(CONFIG.NIH.spellLevels).length - 1;
     ...
     * @memberof hookEvents
     */
    Hooks.callAll(`nih.build${spellcasting.type.capitalize()}SpellcastingTable`, table, item, spellcasting);
  ...
  async _getFeatures(item, { modernStyle, optional=false }) {
    const prepareFeature = async (f, level) => {
      ...
      return {
        document, level,
        name: modernStyle ? game.i18n.format("JOURNALENTRYPAGE.NIH.Class.Features.Name", {
    ...
    if ( asi.levels.length ) {
      ...
      const name = game.i18n.localize("NIH.ADVANCEMENT.AbilityScoreImprovement.Journal.Name");
      features.push({
        description: game.i18n.format(
          `NIH.ADVANCEMENT.AbilityScoreImprovement.Journal.Description${modernStyle ? "Modern" : "Legacy"}`,
          {
            class: item.name,
            firstLevel: formatNumber(firstLevel),
            firstLevelOrdinal: formatNumber(firstLevel, { ordinal: true }),
            maxAbilityScore: formatNumber(CONFIG.NIH.maxAbilityScore),
    ...
    for ( const advancement of asi.boons ) {
      ...
      features.push({
        description: game.i18n.format(
          `NIH.ADVANCEMENT.AbilityScoreImprovement.Journal.DescriptionEpic${recommendation ? "Recommendation" : ""}`,
          { recommendation: recommendation?.toAnchor().outerHTML }
        ),
        level: advancement.level,
        name: game.i18n.format("JOURNALENTRYPAGE.NIH.Class.Features.Name", {
