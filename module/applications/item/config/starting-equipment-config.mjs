...
export default class StartingEquipmentConfig extends DocumentSheet5e {
  ...
  static PARTS = {
    config: {
      template: "systems/nih/templates/apps/starting-equipment-config.hbs"
    }
  };
  ...
  /** @override */
  get title() {
    return game.i18n.localize("NIH.StartingEquipment.Action.Configure");
  }
  ...
  async _onDrop(event) {
    ...
    if ( !item.system.constructor._schemaTemplates?.includes(PhysicalItemTemplate) ) {
      ui.notifications.error(game.i18n.format("NIH.StartingEquipment.Warning.ItemTypeInvalid", {
  ...
  _onSortEntry(event, data) {
    ...
    if ( dropEntry?.type in EquipmentEntryData.GROUPING_TYPES ) {
      ...
      if ( depth > 3 ) {
        ui.notifications.warn("NIH.StartingEquipment.Warning.Depth", { localize: true });
        return;
      }
