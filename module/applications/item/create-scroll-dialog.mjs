...
export default class CreateScrollDialog extends Dialog5e {
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/nih/templates/apps/spell-scroll-dialog.hbs"
    }
  };
  ...
  async _prepareContentContext(context, options) {
    ...
    context.fields = [{
      field: new StringField({
        ...
        label: game.i18n.localize("NIH.Scroll.Explanation.Label"),
        hint: game.i18n.localize("NIH.Scroll.Explanation.Hint")
      }),
      name: "explanation",
      options: [
        { value: "full", label: game.i18n.localize("NIH.Scroll.Explanation.Complete") },
        { value: "reference", label: game.i18n.localize("NIH.Scroll.Explanation.Reference") },
        { value: "none", label: game.i18n.localize("NIH.None") }
      ],
      value: this.config.explanation ?? "reference"
    }, {
      field: new NumberField({ label: game.i18n.localize("NIH.SpellLevel") }),
      name: "level",
      options: Object.entries(CONFIG.NIH.spellLevels)
        .map(([value, label]) => ({ value, label }))
        .filter(l => Number(l.value) >= this.spell.system.level),
      value: this.config.level ?? this.spell.system.level
    }];
    context.values = {
      bonus: new NumberField({ label: game.i18n.localize("NIH.BonusAttack") }),
      dc: new NumberField({ label: game.i18n.localize("NIH.Scroll.SaveDC") })
    };
    context.valuePlaceholders = {};
    for ( const level of Array.fromRange(this.config.level + 1).reverse() ) {
      context.valuePlaceholders = CONFIG.NIH.spellScrollValues[level];
  ...
  /** @override */
  _onClose(options={}) {
    if ( !options.nih?.submitted ) this.#config = null;
  }
