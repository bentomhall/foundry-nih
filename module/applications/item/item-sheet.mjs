...
export default class ItemSheet5e extends ItemSheet {
  ...
  /** @inheritdoc */
  get template() {
    return `systems/nih/templates/items/${this.item.type}.hbs`;
  }
  ...
  async getData(options) {
    ...
    switch ( item.type ) {
      case "feat":
        const featureType = CONFIG.NIH.featureTypes[item.system.type?.value];
        if ( featureType ) {
        ...
        break;
      case "spell":
        context.spellComponents = {...CONFIG.NIH.spellComponents, ...CONFIG.NIH.spellTags};
        break;
    }
  ...
  async _getItemBaseTypes() {
    ...
    const baseIds = CONFIG.NIH[`${type}Ids`];
  ...
  _getItemConsumptionTargets() {
    ...
    // Attributes
      ...
      const attrData = game.nih.isV10 ? actor.system : actor.type;
    ...
    // Hit Dice
      ...
      return {
        smallest: game.i18n.localize("NIH.ConsumeHitDiceSmallest"),
        ...CONFIG.NIH.hitDieTypes.reduce((obj, hd) => { obj[hd] = hd; return obj; }, {}),
        largest: game.i18n.localize("NIH.ConsumeHitDiceLargest")
      };
    }
      ...
      return actor.items.reduce((obj, i) => {
        ...
        if ( uses.per && uses.max ) {
          const label = uses.per === "charges"
            ? ` (${game.i18n.format("NIH.AbilityUseChargesLabel", {value: uses.value})})`
            : ` (${game.i18n.format("NIH.AbilityUseConsumableLabel", {max: uses.max, per: uses.per})})`;
          obj[i.id] = i.name + label;
        }

        // Recharging items
        const recharge = i.system.recharge || {};
        if ( recharge.value ) obj[i.id] = `${i.name} (${game.i18n.format("NIH.Recharge")})`;
        return obj;
      }, {});
    }
    else return {};
  ...
  _getItemStatus() {
    switch ( this.item.type ) {
      case "class":
        return game.i18n.format("NIH.LevelCount", {ordinal: this.item.system.levels.ordinalString()});
      case "equipment":
      case "weapon":
        return game.i18n.localize(this.item.system.equipped ? "NIH.Equipped" : "NIH.Unequipped");
      case "feat":
        const typeConfig = CONFIG.NIH.featureTypes[this.item.system.type.value];
        if ( typeConfig?.subtypes ) return typeConfig.subtypes[this.item.system.type.subtype] ?? null;
        break;
      case "spell":
        return CONFIG.NIH.spellPreparationModes[this.item.system.preparation];
      case "tool":
        return CONFIG.NIH.proficiencyLevels[this.item.system.prof?.multiplier || 0];
    }
    return null;
  }
  ...
  _getItemProperties() {
    ...
    switch ( this.item.type ) {
      case "consumable":
        for ( const [k, v] of Object.entries(this.item.system.properties ?? {}) ) {
          if ( v === true ) props.push(CONFIG.NIH.physicalWeaponProperties[k]);
        }
        break;
      case "equipment":
        props.push(CONFIG.NIH.equipmentTypes[this.item.system.armor.type]);
      ...
      case "weapon":
        for ( const [k, v] of Object.entries(this.item.system.properties) ) {
          if ( v === true ) props.push(CONFIG.NIH.weaponProperties[k]);
        }
        break;
    }
    ...
    if ( this.item.system.actionType ) {
      props.push(CONFIG.NIH.itemActionTypes[this.item.system.actionType]);
    }
  ...
  _getSubmitData(updateData={}) {
    ...
    if ( uses?.max ) {
      ...
      if ( !maxRoll.isDeterministic ) {
        ...
        return ui.notifications.error(game.i18n.format("NIH.FormulaCannotContainDiceError", {
          name: game.i18n.localize("NIH.LimitedUses")
        }));
      }
    }
    ...
    if ( duration?.value ) {
      ...
      if ( !durationRoll.isDeterministic ) {
        ...
        return ui.notifications.error(game.i18n.format("NIH.FormulaCannotContainDiceError", {
          name: game.i18n.localize("NIH.Duration")
        }));
      }
    }

    // Check class identifier
    if ( formData.system?.identifier && !nih.utils.validators.isValidIdentifier(formData.system.identifier) ) {
      ...
      return ui.notifications.error(game.i18n.localize("NIH.IdentifierError"));
    }
  ...
  activateListeners(html) {
    ...
    if ( this.isEditable ) {
      ...
      html.find(".effect-control").click(ev => {
        const unsupported = game.nih.isV10 && this.item.isOwned;
     ...
     */
    Hooks.call("nih.getItemAdvancementContext", html, contextOptions);
  ...
  _getAdvancementContextMenuOptions() {
    ...
    return [
      {
        name: "NIH.AdvancementControlEdit",
        icon: "<i class='fas fa-edit fa-fw'></i>",
        condition,
        callback: li => this._onAdvancementAction(li[0], "edit")
      },
      {
        name: "NIH.AdvancementControlDuplicate",
      ...
      {
        name: "NIH.AdvancementControlDelete",
  ...
  async _onDropAdvancement(event, data) {
    ...
    if ( this.item.isEmbedded && !game.settings.get("nih", "disableAdvancements") ) {
  ...
  _onConfigureTraits(event) {
    ...
    switch (a.dataset.options) {
      case "saves":
        options.choices = CONFIG.NIH.abilities;
        options.valueKey = null;
        options.labelKey = "label";
        break;
      case "skills.choices":
        options.choices = CONFIG.NIH.skills;
        options.valueKey = null;
        options.labelKey = "label";
        break;
      case "skills":
        const skills = this.item.system.skills;
        const choices = skills.choices?.length ? skills.choices : Object.keys(CONFIG.NIH.skills);
        options.choices = Object.fromEntries(Object.entries(CONFIG.NIH.skills).filter(([s]) => choices.includes(s)));
  ...
  _onAdvancementAction(target, action) {
    ...
    switch (action) {
      case "add": return game.nih.applications.advancement.AdvancementSelection.createDialog(this.item);
