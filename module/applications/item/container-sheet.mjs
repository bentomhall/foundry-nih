...
export default class ContainerSheet extends ItemSheet5e {
  ...
  static PARTS = {
    ...super.PARTS,
    contents: {
      template: "systems/nih/templates/items/contents.hbs",
      templates: [
        "systems/nih/templates/inventory/inventory.hbs", "systems/nih/templates/inventory/encumbrance.hbs"
      ],
      scrollable: [""]
    }
  };
  ...
  /** @override */
  static TABS = [
    { tab: "contents", label: "NIH.ITEM.SECTIONS.Contents" },
    ...super.TABS
  ];
  ...
  _onDrop(event) {
    ...
    if ( Hooks.call("nih.dropItemSheetData", this.item, this, data) === false ) return;
  ...
  async _onDropFolder(event, data) {
    ...
    if ( recursiveWarning ) ui.notifications.warn("NIH.ContainerRecursiveError", { localize: true });
  ...
  async _onDropItem(event, data) {
    ...
    const parentContainers = await this.item.system.allContainers();
    if ( (this.item.uuid === item.uuid) || parentContainers.includes(item) ) {
      ui.notifications.error("NIH.ContainerRecursiveError", { localize: true });
      return;
    }
