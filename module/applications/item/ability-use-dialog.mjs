...
export default class AbilityUseDialog extends Dialog {
  constructor(item, dialogData={}, options={}) {
    ...
    this.options.classes = ["nih", "dialog"];
  ...
  static async create(item) {
    ...
    const data = {
      item: item,
      title: game.i18n.format("NIH.AbilityUseHint", {type: game.i18n.localize(CONFIG.Item.typeLabels[item.type]), name: item.name}),
    ...
    const html = await renderTemplate("systems/nih/templates/apps/ability-use.hbs", data);

    // Create the Dialog and return data as a Promise
    const icon = data.isSpell ? "fa-magic" : "fa-fist-raised";
    const label = game.i18n.localize(`NIH.AbilityUse${data.isSpell ? "Cast" : "Use"}`);
    return new Promise(resolve => {
      const dlg = new this(item, {
        title: `${item.name}: ${game.i18n.localize("NIH.AbilityUseConfig")}`,
        content: html,
        buttons: {
          use: {
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const consumeSpellSlot = (lvl > 0) && CONFIG.NIH.spellUpcastModes.includes(itemData.preparation.mode);
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( quantity <= 0 ) return game.i18n.localize("NIH.AbilityUseUnavailableHint");

    // Abilities which use Recharge
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "NIH.AbilityUseChargedHint" : "NIH.AbilityUseRechargeHint", {
    ...
    if ( item.type === "consumable" ) {
      let str = "NIH.AbilityUseNormalHint";
      if ( uses.value > 1 ) str = "NIH.AbilityUseConsumableChargeHint";
      else if ( item.system.quantity === 1 && uses.autoDestroy ) str = "NIH.AbilityUseConsumableDestroyHint";
      else if ( item.system.quantity > 1 ) str = "NIH.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(`NIH.Consumable${item.system.consumableType.capitalize()}`),
        value: uses.value,
        quantity: item.system.quantity,
        max: uses.max,
        per: CONFIG.NIH.limitedUsePeriods[uses.per]
      });
    }

    // Other Items
    else {
      return game.i18n.format("NIH.AbilityUseNormalHint", {
        type: game.i18n.localize(CONFIG.Item.typeLabels[item.type]),
        value: uses.value,
        max: uses.max,
        per: CONFIG.NIH.limitedUsePeriods[uses.per]
