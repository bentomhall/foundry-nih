...
export default class CalendarHUD extends BaseCalendarHUD {
  ...
  static PARTS = {
    startButtons: {
      ...
      template: "systems/nih/templates/apps/calendar-buttons.hbs"
    },
    core: {
      classes: ["calendar-core"],
      template: "systems/nih/templates/apps/calendar-core.hbs"
    },
    endButtons: {
      classes: ["calendar-buttons"],
      template: "systems/nih/templates/apps/calendar-buttons.hbs"
    ...
    return [
      {
        ...
        position: "start",
        tooltip: game.i18n.format("NIH.CALENDAR.Action.ReverseTime", { amount: defaultAmount }),
        visible: game.user.isGM,
        additional: CalendarHUD.TIME_CONTROL_VALUES.map(({ value, unit }) => ({
          action: "reverse",
          dataset: { value, unit },
          label: `-${formatTime(value, unit, { unitDisplay: "narrow" })}`,
          tooltip: game.i18n.format("NIH.CALENDAR.Action.ReverseTime", {
      ...
      },
      {
        ...
        position: "start",
        tooltip: game.i18n.localize("NIH.CALENDAR.Action.SetDate"),
        visible: game.user.isGM
      },
      {
        ...
        position: "start",
        tooltip: game.i18n.localize("NIH.CALENDAR.Action.OpenCharacterSheet"),
      ...
      },
      {
        ...
        position: "end",
        tooltip: game.i18n.format("NIH.CALENDAR.Action.AdvanceTime", { amount: defaultAmount }),
        visible: game.user.isGM,
        additional: CalendarHUD.TIME_CONTROL_VALUES.map(({ value, unit }) => ({
          action: "advance",
          dataset: { value, unit },
          label: `+${formatTime(value, unit, { unitDisplay: "narrow" })}`,
          tooltip: game.i18n.format("NIH.CALENDAR.Action.AdvanceTime", {
            amount: formatTime(value, unit).titleCase()
          })
        }))
      },
      {
        ...
        position: "end",
        tooltip: game.i18n.localize("NIH.CALENDAR.Action.OpenPartySheet"),
  ...
  async _prepareButtonsContext(context, options) {
     ...
     * @function nih.prepareCalendarButtons
     * @memberof hookEvents
    ...
    const controls = this._doEvent(this._getCalendarButtons, {
      ...
      hookName: "nih.prepareCalendarButtons",
      hookResponse: true,
  ...
  async renderCore(deltas={}) {
    const prefs = game.settings.get("nih", "calendarPreferences");
    const dateFormatter = CONFIG.NIH.calendar.formatters.find(f => f.value === prefs.formatters.date);
    ...
    const timeFormatter = CONFIG.NIH.calendar.formatters.find(f => f.value === prefs.formatters.time);
  ...
  static onUpdateWorldTime(worldTime, deltaTime, options, userId) {
    if ( this.shouldDisplay ) nih.ui.calendar?.renderCore(options.nih?.deltas);
  }
}
