...
export default class CompendiumBrowserSettingsConfig extends Application5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    tag: "form",
    window: {
      title: "NIH.CompendiumBrowser.Sources.Label",
  ...
  static PARTS = {
    sidebar: {
      id: "sidebar",
      template: "systems/nih/templates/compendium/sources-sidebar.hbs"
    },
    packs: {
      id: "packs",
      template: "systems/nih/templates/compendium/sources-packs.hbs"
    }
  };
  ...
  _preparePackGroupContext(documentType, packs, sources) {
    ...
    return {
      ...
      entries: Array.from(packs.map(id => {
        ...
        if ( packageName === "nih" ) {
          tag = flags?.nih?.sourceBook?.replace("SRD ", "");
        }
        return {
          tag, title,
          id: collection,
  ...
  async _onToggleSource(target) {
    ...
    const setting = { ...game.settings.get("nih", "packSourceConfiguration"), ...packs };
    await game.settings.set("nih", "packSourceConfiguration", setting);
    this.render();
  }
  ...
  static collateSources() {
    ...
    const setting = game.settings.get("nih", "packSourceConfiguration");
