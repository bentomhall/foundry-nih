...
export default class CalendarSettingsConfig extends BaseSettingsConfig {
  ...
  static DEFAULT_OPTIONS = {
    window: {
      title: "NIH.CALENDAR.Configuration.Label"
    }
  };
  ...
  async _prepareConfigContext(context, options) {
    const data = game.settings.get("nih", "calendarConfig");
    ...
    if ( !CONFIG.NIH.calendar.application ) {
      ...
      context.message = {
        level: "warn",
        text: game.i18n.localize("NIH.CALENDAR.Configuration.UnavailableMessage")
      };
    }
  ...
  async _preparePreferencesContext(context, options) {
    const data = game.settings.get("nih", "calendarPreferences");
    ...
    context.fields = [
      ...
      },
      {
        ...
        options: CONFIG.NIH.calendar.formatters,
    ...
    context.legend = game.i18n.localize("NIH.CALENDAR.Configuration.Preferences");
    if ( !CONFIG.NIH.calendar.application ) {
    ...
    } else if ( !game.settings.get("nih", "calendarConfig")?.enabled ) {
      context.disabled = !game.user.isGM;
      context.message = {
        level: "warn",
        text: game.i18n.localize("NIH.CALENDAR.Configuration.DisabledMessage")
      };
    }
    return context;
  }
