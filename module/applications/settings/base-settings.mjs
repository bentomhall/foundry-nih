...
export default class BaseSettingsConfig extends Application5e {
   ...
   * @param {string} name  Setting key within the nih namespace.
   * @returns {object}
   */
  createSettingField(name) {
    const setting = game.settings.settings.get(`nih.${name}`);
    if ( !setting ) throw new Error(`Setting \`nih.${name}\` not registered.`);
    ...
    const data = {
      ...
      value: game.settings.get("nih", name)
    };
  ...
  static async #onCommitChanges(event, form, formData) {
    ...
    for ( const [key, value] of Object.entries(foundry.utils.expandObject(formData.object)) ) {
      const setting = game.settings.settings.get(`nih.${key}`);
      const current = game.settings.get("nih", key, { document: true });
      const prior = current?._source?.value ?? current;
      const updated = await game.settings.set("nih", key, value, { document: true });
