...
export default class ActivityUsageDialog extends Dialog5e {
  ...
  static PARTS = {
    scaling: {
      template: "systems/nih/templates/activity/activity-usage-scaling.hbs"
    },
    concentration: {
      template: "systems/nih/templates/activity/activity-usage-concentration.hbs"
    },
    consumption: {
      template: "systems/nih/templates/activity/activity-usage-consumption.hbs"
    },
    creation: {
      template: "systems/nih/templates/activity/activity-usage-creation.hbs"
    },
    footer: {
  ...
  async _prepareContext(options) {
    if ( "scaling" in this.config ) {
      this.#item = this.#item.clone({ "flags.nih.scaling": this.config.scaling }, { keepId: true });
    }
    return {
  ...
  async _prepareConcentrationContext(context, options) {
    if ( !this.activity.requiresConcentration || game.settings.get("nih", "disableConcentration")
      || !this._shouldDisplay("concentration") ) return context;
    context.hasConcentration = true;
    context.notes = [];

    context.fields = [{
      field: new BooleanField({ label: game.i18n.localize("NIH.Concentration") }),
      ...
      const existingConcentration = Array.from(this.actor.concentration.effects).map(effect => {
        ...
        return {
          ...
          label: data?.data?.name ?? this.actor.items.get(data?.id)?.name
            ?? game.i18n.localize("NIH.ConcentratingItemless")
        };
      });
      if ( existingConcentration.length ) {
        ...
        context.fields.push({
          field: new StringField({
            required: true, label: game.i18n.localize("NIH.ConcentratingEnd"), blank: optional
          }),
        ...
        context.notes.push({
          type: "info", message: game.i18n.localize(`NIH.ConcentratingWarnLimit${optional ? "Optional" : ""}`)
        });
      } else if ( !this.actor.system.attributes?.concentration?.limit ) {
        context.notes.push({
          type: "warn", message: game.i18n.localize("NIH.ConcentratingWarnLimitZero")
        });
      }
    }
  ...
  async _prepareConsumptionContext(context, options) {
    ...
    const activationConfig = CONFIG.NIH.activityActivationTypes[this.activity.activation.type];
      ...
      if ( current && !containsConsumption ) {
        ...
        context.fields.push({
          ...
          field: new BooleanField({
            label: game.i18n.format("NIH.CONSUMPTION.Type.Action.Prompt", {
              type: activationConfig.label
            }),
            hint: game.i18n.format("NIH.CONSUMPTION.Type.Action.PromptHint", {
    ...
    if ( this.activity.requiresSpellSlot && this.activity.consumption.spellSlot
      ...
      field: new BooleanField({ label: game.i18n.localize("NIH.SpellCastConsume") }),
      input: context.inputs.createCheckboxInput,
  ...
  async _prepareCreationContext(context, options) {
    ...
    if ( this.activity.target?.template?.type && this._shouldDisplay("create.measuredTemplate") ) {
      ...
      context.template = {
        field: new BooleanField({ label: game.i18n.localize("NIH.TARGET.Action.PlaceTemplate") }),
  ...
  async _prepareFooterContext(context, options) {
    context.buttons = [{
      ...
      label: this.options.button.label ?? `NIH.AbilityUse${this.activity.isSpell ? "Cast" : "Use"}`,
      type: "button"
    }];
    return context;
  }
  ...
  async _prepareScalingContext(context, options) {
    ...
    if ( this.activity.requiresSpellSlot && context.linkedActivity && (this.config.scaling !== false) ) {
      ...
      const spellSlotOptions = Object.entries(CONFIG.NIH.spellLevels).map(([level, label]) => {
        if ( (Number(level) < minimumLevel) || (Number(level) > maximumLevel) ) return null;
        return { value: `spell${level}`, label };
      }).filter(_ => _);
      context.spellSlots = {
        field: new StringField({ required: true, blank: false, label: game.i18n.localize("NIH.SpellCastUpcast") }),
        name: "spell.slot",
      ...
      const spellMethod = CONFIG.NIH.spellcasting[this.item.system.method];
      ...
      const spellSlotOptions = Object.entries(this.actor.system.spells).map(([value, slot]) => {
        ...
        const model = CONFIG.NIH.spellcasting[slot.type];
        if ( model?.exclusive.slots && (this.item.system.method !== slot.type) ) return null;
        const label = game.i18n.format(`NIH.SpellLevel${slot.type.capitalize()}`, {
      ...
      if ( !spellSlotOptions.some(o => !o.disabled) ) context.notes.push({
        type: "warn", message: game.i18n.format("NIH.SpellCastNoSlotsLeft", {
          name: this.item.name
        })
      });
    }
      ...
      if ( max > 1 ) context.scaling = {
        field: new NumberField({ min: 1, max, label: game.i18n.localize("NIH.ScalingValue") }),
        name: "scalingValue",
