...
export default class EnchantUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    ...super.PARTS,
    creation: {
      template: "systems/nih/templates/activity/enchant-usage-creation.hbs"
    }
  };
  ...
  async _prepareCreationContext(context, options) {
    ...
    if ( (enchantments.length > 1) && this._shouldDisplay("create.enchantment") ) {
      const existingProfile = this.activity.existingEnchantment?.flags.nih?.enchantmentProfile;
      context.hasCreation = true;
      context.enchantment = {
        field: new StringField({ required: true, blank: false, label: game.i18n.localize("NIH.ENCHANTMENT.Label") }),
        name: "enchantmentProfile",
        value: this.config.enchantmentProfile,
        options: enchantments.map(e => ({
          value: e._id,
          label: e._id === existingProfile
            ? game.i18n.format("NIH.ENCHANT.Enchantment.Active", { name: e.effect.name })
            : e.effect.name
        }))
      };
