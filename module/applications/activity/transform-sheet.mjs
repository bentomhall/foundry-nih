...
export default class TransformSheet extends ActivitySheet {
  ...
  static PARTS = {
    ...super.PARTS,
    effect: {
      template: "systems/nih/templates/activity/transform-effect.hbs",
      templates: [
        ...super.PARTS.effect.templates,
        "systems/nih/templates/activity/parts/transform-profiles.hbs",
        "systems/nih/templates/activity/parts/transform-settings.hbs"
      ]
    }
  };
  ...
  async _prepareEffectContext(context, options) {
    ...
    const settings = new TransformationSetting({
      ...(context.source.transform.customize ? context.source.settings
        : CONFIG.NIH.transformation.presets[context.source.transform.preset]?.settings ?? {}),
    ...
    context.presetOptions = [
      { value: "", label: game.i18n.localize("NIH.TRANSFORM.Preset.Default") },
      { rule: true },
      ...Object.entries(CONFIG.NIH.transformation.presets)
        .map(([value, { label }]) => ({ value, label }))
    ];

    context.creatureSizeOptions = Object.entries(CONFIG.NIH.actorSizes)
      .map(([value, { label }]) => ({ value, label }));
    context.creatureTypeOptions = Object.entries(CONFIG.NIH.creatureTypes)
      .map(([value, { label }]) => ({ value, label }));
    context.movementTypeOptions = Object.entries(CONFIG.NIH.movementTypes)
      .map(([value, { label }]) => ({ value, label }));

    context.profileModes = [
      { value: "", label: game.i18n.localize("NIH.TRANSFORM.FIELDS.transform.mode.Direct") },
      { value: "cr", label: game.i18n.localize("NIH.TRANSFORM.FIELDS.transform.mode.CR") }
    ];
  ...
  async _processSubmitData(event, submitData) {
    ...
    if ( submitData.transform?.customize && !this.activity._source.settings ) {
      ...
      submitData.settings = new TransformationSetting(foundry.utils.mergeObject({
        ...(CONFIG.NIH.transformation.presets[preset]?.settings ?? {}),
