...
export default class OrderUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    order: {
      template: "systems/nih/templates/activity/order-usage.hbs"
    },
    footer: {
  ...
  _prepareBuildContext(context, options) {
    context.build = {
      choices: CONFIG.NIH.facilities.sizes,
      field: new StringField({ nullable: false, blank: false, label: "NIH.FACILITY.FIELDS.size.label" }),
      name: "building.size",
  ...
  _prepareCostsContext(context, { days, gold }) {
    ...
    context.costs = {
      days: {
        field: new NumberField({ nullable: true, integer: true, min: 0, label: "NIH.TimeDay" }),
        name: "costs.days",
        value: this.config.costs?.days ?? days ?? duration
      },
      gold: {
        field: new NumberField({ nullable: true, integer: true, min: 0, label: "NIH.CurrencyGP" }),
        name: "costs.gold",
  ...
  async _prepareCraftContext(context, options) {
    ...
    context.craft = {
      legend: game.i18n.localize(`NIH.FACILITY.Orders.${this.activity.order}.present`),
      item: {
    ...
    if ( this.activity.order === "harvest" ) {
      ...
      context.craft.baseItem = {
        field: new BooleanField({
          label: "NIH.FACILITY.Craft.BaseItem.Label",
          hint: "NIH.FACILITY.Craft.BaseItem.Hint"
        }),
  ...
  async _prepareFooterContext(context, options) {
    context.buttons = [{
      ...
      label: "NIH.FACILITY.Order.Execute"
    }];
    return context;
  }
  ...
  async _prepareOrderContext(context, options) {
    ...
    if ( this.activity.order === "build" ) {
      const { days, value: gold } = CONFIG.NIH.facilities.sizes.cramped;
      this._prepareBuildContext(context, options);
      this._prepareCostsContext(context, { ...options, days, gold });
      return;
    }

    let { duration } = game.settings.get("nih", "bastionConfiguration");
    if ( (this.activity.order === "craft") || (this.activity.order === "harvest") ) {
      ...
      const config = CONFIG.NIH.facilities.orders[this.activity.order];
  ...
  async _prepareTradeContext(context, options) {
    ...
    if ( !trade.creatures.max && !trade.stock.max ) {
      context.trade = {
        stocked: {
          field: new BooleanField({
            label: "NIH.FACILITY.Trade.Stocked.Label",
            hint: "NIH.FACILITY.Trade.Stocked.Hint"
          }),
      ...
      context.trade = {
        sell: {
          field: new BooleanField({ label: "NIH.FACILITY.Trade.Sell.Label" }),
          name: "trade.sell",
          value: isSelling
        }
      };
      ...
      if ( trade.stock.max ) {
        ...
        const label = `NIH.FACILITY.Trade.Stock.${isSelling ? "Sell" : "Buy"}`;
        context.trade.stock = {
        ...
        context.trade.creatures = {
          buy, sell,
          hint: "NIH.FACILITY.Trade.Creatures.Buy",
          price: {
            field: new NumberField({
              nullable: false, min: 0, integer: true,
              label: "NIH.FACILITY.Trade.Price.Label",
              hint: "NIH.FACILITY.Trade.Price.Hint"
            }),
  ...
  _prepareBuildData(submitData) {
    if ( (this.config.building?.size ?? "cramped") !== submitData.building?.size ) {
      const { days, value: gold } = CONFIG.NIH.facilities.sizes[submitData.building.size];
