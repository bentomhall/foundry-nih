...
export default class ActivitySheet extends PseudoDocumentSheet {
  ...
  static PARTS = {
    ...
    identity: {
      template: "systems/nih/templates/activity/identity.hbs",
      templates: [
        "systems/nih/templates/activity/parts/activity-identity.hbs",
        "systems/nih/templates/activity/parts/activity-visibility.hbs"
      ]
    },
    activation: {
      template: "systems/nih/templates/activity/activation.hbs",
      templates: [
        "systems/nih/templates/activity/parts/activity-time.hbs",
        "systems/nih/templates/activity/parts/activity-targeting.hbs",
        "systems/nih/templates/activity/parts/activity-consumption.hbs"
      ]
    },
    effect: {
      template: "systems/nih/templates/activity/effect.hbs",
      templates: [
        "systems/nih/templates/activity/parts/activity-effects.hbs",
        "systems/nih/templates/activity/parts/activity-effect-level-limit.hbs",
        "systems/nih/templates/activity/parts/activity-effect-settings.hbs"
      ]
    }
  };
  ...
  async _prepareActivationContext(context, options) {
    ...
    context.activationTypes = [
      ...Object.entries(CONFIG.NIH.activityActivationTypes).map(([value, config]) => ({
        value,
        label: game.i18n.localize(config.label),
        group: game.i18n.localize(config.group)
      })),
      { value: "", label: game.i18n.localize("NIH.NoneActionLabel") }
    ];
    context.affectsPlaceholder = game.i18n.localize(
      `NIH.TARGET.Count.${context.data.target?.template?.type ? "Every" : "Any"}`
    );
    context.durationUnits = [
      { value: "inst", label: game.i18n.localize("NIH.TimeInst") },
      ...Object.entries(CONFIG.NIH.scalarTimePeriods).map(([value, label]) => ({
        value, label, group: game.i18n.localize("NIH.DurationTime")
      })),
      ...Object.entries(CONFIG.NIH.permanentTimePeriods).map(([value, label]) => ({
        value, label, group: game.i18n.localize("NIH.DurationPermanent")
      })),
      { value: "spec", label: game.i18n.localize("NIH.Special") }
    ];
    context.rangeUnits = [
      ...Object.entries(CONFIG.NIH.rangeTypes).map(([value, label]) => ({ value, label })),
      ...Object.entries(CONFIG.NIH.movementUnits).map(([value, { label }]) => ({
        value, label, group: game.i18n.localize("NIH.RangeDistance")
      }))
    ];
    ...
    const consumptionTypeOptions = Array.from(this.activity.validConsumptionTypes).map(value => ({
      value,
      label: CONFIG.NIH.activityConsumptionTypes[value].label
    }));
    context.consumptionTargets = context.source.consumption.targets.map((data, index) => {
      const typeConfig = CONFIG.NIH.activityConsumptionTypes[data.type] ?? {};
      ...
      return {
        ...
        scalingModes: canScale ? [
          { value: "", label: game.i18n.localize("NIH.CONSUMPTION.Scaling.None") },
          { value: "amount", label: game.i18n.localize("NIH.CONSUMPTION.Scaling.Amount") },
        ...
        selectedTarget: ("validTargets" in typeConfig) && ["itemUses", "material"].includes(data.type)
          ...
          : data.target,
        targetPlaceholder: data.type === "itemUses" ? game.i18n.localize("NIH.CONSUMPTION.Target.ThisItem") : "",
    ...
    context.recoveryTypes = [
      { value: "recoverAll", label: game.i18n.localize("NIH.USES.Recovery.Type.RecoverAll") },
      { value: "loseAll", label: game.i18n.localize("NIH.USES.Recovery.Type.LoseAll") },
      { value: "formula", label: game.i18n.localize("NIH.USES.Recovery.Type.Formula") }
    ];
  ...
  async _prepareEffectContext(context, options) {
    ...
    if ( context.activity.effects ) {
      ...
      context.appliedEffects = context.activity.effects.reduce((arr, data) => {
        ...
        const effect = {
          ...
          additionalSettings: "systems/nih/templates/activity/parts/activity-effect-settings.hbs"
        };
    ...
    if ( context.activity.damage?.parts ) {
      ...
      const typeOptions = Object.entries(CONFIG.NIH.damageTypes).map(([value, { label }]) => ({ value, label }));
  ...
  _getTabs() {
    return this._markTabs({
      ...
      activation: {
        ...
        label: "NIH.ACTIVITY.SECTIONS.Activation",
        tabs: {
          time: {
            id: "time", group: "activation", icon: "fa-solid fa-clock",
            label: "NIH.ACTIVITY.SECTIONS.Time"
          },
          consumption: {
          ...
          targeting: {
            ...
            label: "NIH.TARGET.FIELDS.target.label"
          }
        }
      },
      effect: {
  ...
  static #addRecovery(event, target) {
    const periods = new Set(
      Object.entries(CONFIG.NIH.limitedUsePeriods).filter(([, config]) => !config.deprecated).map(([k]) => k)
    );
