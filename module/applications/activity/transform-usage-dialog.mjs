...
export default class TransformUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    ...super.PARTS,
    creation: {
      template: "systems/nih/templates/activity/transform-usage-creation.hbs"
    }
  };
    ...
    if ( this._shouldDisplay("create.transform") && (profiles.length > 1) ) {
      ...
      context.transformFields = [{
        field: new StringField({
          required: true, blank: false, label: game.i18n.localize("NIH.TRANSFORM.Profile.Label")
        }),
  ...
  getProfileLabel(profile, rollData) {
    ...
    switch ( this.activity.transform.mode ) {
      case "cr":
        ...
        return game.i18n.format("NIH.TRANSFORM.Profile.ChallengeRatingLabel", { cr: formatCR(cr) });
      default:
