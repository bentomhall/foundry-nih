...
export default class SummonSheet extends ActivitySheet {
  ...
  static PARTS = {
    ...super.PARTS,
    effect: {
      template: "systems/nih/templates/activity/summon-effect.hbs",
      templates: [
        ...super.PARTS.effect.templates,
        "systems/nih/templates/activity/parts/summon-changes.hbs",
        "systems/nih/templates/activity/parts/summon-profiles.hbs"
      ]
    }
  };
    ...
    context.abilityOptions = [
      {
        ...
        label: game.i18n.format("NIH.DefaultSpecific", {
          default: this.activity.isSpell ? game.i18n.localize("NIH.Spellcasting").toLowerCase()
            : CONFIG.NIH.abilities[this.activity.ability]?.label.toLowerCase()
              ?? game.i18n.localize("NIH.None").toLowerCase()
        })
      },
      ...Object.entries(CONFIG.NIH.abilities).map(([value, { label }]) => ({ value, label }))
    ];
    context.creatureSizeOptions = Object.entries(CONFIG.NIH.actorSizes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureSizes.has(value)
    }));
    context.creatureTypeOptions = Object.entries(CONFIG.NIH.creatureTypes).map(([value, config]) => ({
      value, label: config.label, selected: this.activity.creatureTypes.has(value)
    }));

    context.profileModes = [
      { value: "", label: game.i18n.localize("NIH.SUMMON.FIELDS.summon.mode.Direct") },
      { value: "cr", label: game.i18n.localize("NIH.SUMMON.FIELDS.summon.mode.CR") }
    ];
  ...
  _getTabs() {
    ...
    tabs.effect.label = "NIH.SUMMON.SECTIONS.Summoning";
    ...
    tabs.effect.tabs = this._markTabs({
      profiles: {
        ...
        label: "NIH.SUMMON.SECTIONS.Profiles"
      },
      changes: {
        id: "changes", group: "effect", icon: "fa-solid fa-sliders",
        label: "NIH.SUMMON.SECTIONS.Changes"
      }
    });
    return tabs;
  }
