...
export default class CombatTracker5e extends foundry.applications.sidebar.tabs.CombatTracker {
  ...
  renderGroups(html) {
    ...
    for ( const [key, { combatants, expanded }] of groups.entries() ) {
      ...
      groupContainer.classList.add("combatant", "combatant-group", "collapsible", "nih2-collapsible");
      ...
      let count = game.i18n.format(`NIH.COMBATANT.Counted.${getPluralRules().select(children.length)}`, {
        number: formatNumber(children.length)
      });
      if ( activeEntry !== undefined ) {
        groupContainer.classList.add("active");
        count = game.i18n.format("NIH.COMBAT.Group.ActiveCount", {
      ...
      groupContainer.querySelector(".name").innerText = game.i18n.format("NIH.COMBAT.Group.Title", { name });
