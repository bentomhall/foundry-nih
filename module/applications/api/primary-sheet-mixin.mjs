  ...
  return class PrimarySheet5e extends DragDropApplicationMixin(Base) {
    ...
    _renderModeToggle() {
      ...
      if ( this.isEditable && !toggle ) {
        ...
        toggle.dataset.tooltip = "NIH.SheetModeEdit";
        toggle.setAttribute("aria-label", game.i18n.localize("NIH.SheetModeEdit"));
      ...
      elements.innerHTML = `
        <div class="source-book">
          <button type="button" class="unbutton control-button header-control" data-action="showConfiguration"
                  data-config="source" data-tooltip aria-label="${game.i18n.localize("NIH.SOURCE.Action.Configure")}">
    ...
    _renderSource() {
      ...
      elements.querySelector("span").innerText = editable
        ? (source.label || game.i18n.localize("NIH.SOURCE.FIELDS.source.label"))
        : source.label;
    }
    ...
    async _preparePartContext(partId, context, options) {
       ...
       * @function nih.prepareSheetContext
       * @memberof hookEvents
       ...
       */
      Hooks.callAll("nih.prepareSheetContext", this, partId, context, options);
    ...
    _applyItemTooltips(element) {
      ...
      `;
      element.dataset.tooltipClass = "nih2 nih-tooltip item-tooltip themed theme-light";
    ...
    async _onChangeSheetMode(event) {
      ...
      const label = game.i18n.localize(`NIH.SheetMode${toggle.checked ? "Play" : "Edit"}`);
