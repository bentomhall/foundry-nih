  ...
  class BaseApplication5e extends HandlebarsApplicationMixin(Base) {
    ...
    *_getHeaderControlContextEntries() {
    ...
    async _prepareContext(options) {
      ...
      context.inputs = { ...foundry.applications.fields, ...nih.applications.fields };
      return context;
    }
    ...
    async _renderFrame(options) {
      ...
      if ( (options.window?.icon ?? "").includes(".") ) {
        ...
        const newIcon = document.createElement(options.window.icon?.endsWith(".svg") ? "nih-icon" : "img");
    ...
    _onRender(context, options) {
      ...
      this.element.querySelectorAll("[data-context-menu]").forEach(control =>
        control.addEventListener("click", nih.applications.ContextMenu5e.triggerEvent)
      );
    ...
    _disableFields() {
      const selector = `.window-content :is(${[
        "INPUT", "SELECT", "TEXTAREA", "BUTTON", "NIH-CHECKBOX", "COLOR-PICKER", "DOCUMENT-TAGS",
