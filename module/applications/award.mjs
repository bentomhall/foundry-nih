...
export default class Award extends Application5e {
  ...
  static PARTS = {
    award: {
      template: "systems/nih/templates/apps/award.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.currency = Object.entries(CONFIG.NIH.currencies).reduce((obj, [k, { label, icon }]) => {
      obj[k] = {
        label, icon,
    ...
    context.hideXP = game.settings.get("nih", "levelingMode") === "noxp";
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<nih-icon class="fa-fw" src="systems/nih/icons/svg/backpack.svg"></nih-icon>',
  ...
  _saveDestinations(destinations) {
    ...
    target.setFlag("nih", "awardDestinations", destinations);
  }
  ...
  static async displayAwardMessages(results) {
    ...
    for ( const [destination, result] of results ) {
      ...
      if ( result.xp ) entries.push(`
        <span class="award-entry">
          ${formatNumber(result.xp)} ${game.i18n.localize("NIH.ExperiencePoints.Abbreviation")}
        </span>
      `);
      if ( !entries.length ) continue;

      const content = game.i18n.format("NIH.Award.Message", {
        name: destination.name, award: `<span class="nih2">${game.i18n.getListFormatter().format(entries)}</span>`
      });
  ...
  static async handleAward(message) {
    if ( !game.user.isGM ) {
      ui.notifications.error("NIH.Award.NotGMError", { localize: true });
      return;
    }
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("nih", "awardDestinations");
  ...
  static parseAwardCommand(message) {
    ...
    for ( const part of command.split(" ") ) {
      ...
      try {
        new Roll(amount);
        if ( label in CONFIG.NIH.currencies ) currency[label] = amount;
    ...
    if ( unrecognized.length ) throw new Error(game.i18n.format("NIH.Award.UnrecognizedWarning", {
