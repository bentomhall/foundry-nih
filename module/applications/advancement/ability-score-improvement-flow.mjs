...
export default class AbilityScoreImprovementFlow extends AdvancementFlow {
  ...
  /** @inheritDoc */
  static _customElements = super._customElements.concat(["nih-checkbox"]);
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/nih/templates/advancement/ability-score-improvement-flow.hbs"
    });
  }
  ...
  async getData() {
    const points = {
      assigned: Object.keys(CONFIG.NIH.abilities).reduce((assigned, key) => {
    ...
    const abilities = Object.entries(CONFIG.NIH.abilities).reduce((obj, [key, data]) => {
    ...
    const modernRules = game.settings.get("nih", "rulesVersion") === "modern";
    const pluralRules = new Intl.PluralRules(game.i18n.lang);
    return foundry.utils.mergeObject(super.getData(), {
      abilities, lockImprovement, points, recommendation,
      feat: this.feat,
      pointCap: game.i18n.format(
        `NIH.ADVANCEMENT.AbilityScoreImprovement.CapDisplay.${pluralRules.select(points.cap)}`, { points: points.cap }
      ),
      pointsRemaining: game.i18n.format(
        `NIH.ADVANCEMENT.AbilityScoreImprovement.PointsRemaining.${pluralRules.select(points.available)}`,
  ...
  async _onBrowseCompendium(event) {
    ...
    // TODO: Remove this unnecessary check when https://github.com/foundryvtt/nih/issues/5139 is implemented
  ...
  async _onDrop(event) {
    ...
    if ( (item.type !== "feat") || (item.system.type.value !== "feat") ) {
      ui.notifications.error("NIH.ADVANCEMENT.AbilityScoreImprovement.Warning.Type", {localize: true});
      return null;
    }
