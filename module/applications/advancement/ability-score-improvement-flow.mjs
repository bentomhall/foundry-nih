...
export default class AbilityScoreImprovementFlow extends AdvancementFlow {
  ...
  async getData() {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      pointsRemaining: game.i18n.format(
        `NIH.AdvancementAbilityScoreImprovementPointsRemaining.${pluralRule}`, {points: points.available}
      )
    });
  }
  ...
  async _onDrop(event) {
    ...
    if ( (item.type !== "feat") || (item.system.type.value !== "feat") ) return ui.notifications.error(
      game.i18n.localize("NIH.AdvancementAbilityScoreImprovementFeatWarning")
    );
