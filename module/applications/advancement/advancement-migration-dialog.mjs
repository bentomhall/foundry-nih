...
export default class AdvancementMigrationDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["nih", "advancement-migration", "dialog"],
      jQuery: false,
      width: 500
    });
  }
  ...
  static createDialog(item, advancements) {
    ...
    return new Promise(async (resolve, reject) => {
      const dialog = new this({
        title: `${game.i18n.localize("NIH.AdvancementMigrationTitle")}: ${item.name}`,
        content: await renderTemplate(
          "systems/nih/templates/advancement/advancement-migration-dialog.hbs",
          { item, advancements: advancementContext }
        ),
        buttons: {
          continue: {
            icon: '<i class="fas fa-check"></i>',
            label: game.i18n.localize("NIH.AdvancementMigrationConfirm"),
