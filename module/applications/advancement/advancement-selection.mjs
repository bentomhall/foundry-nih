...
export default class AdvancementSelection extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["nih", "sheet", "advancement"],
      template: "systems/nih/templates/advancement/advancement-selection.hbs",
      title: "NIH.AdvancementSelectionTitle",
      width: 500,
      height: "auto"
    });
  }
  ...
  getData() {
    const context = { types: {} };
    for ( const [name, advancement] of Object.entries(CONFIG.NIH.advancementTypes) ) {
    ...
    context.types = nih.utils.sortObjectEntries(context.types, "label");
    return context;
  }
  ...
  static async createDialog(item, { rejectClose=false, options={} }={}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(item, {
        title: `${game.i18n.localize("NIH.AdvancementSelectionTitle")}: ${item.name}`,
        buttons: {
          submit: {
            callback: html => {
