...
export default class TraitConfig extends AdvancementConfig {
  ...
  static PARTS = {
    config: {
      ...
      template: "systems/nih/templates/advancement/advancement-controls-section.hbs"
    },
    details: {
      container: { classes: ["column-container"], id: "column-left" },
      template: "systems/nih/templates/advancement/trait-config-details.hbs"
    },
    guaranteed: {
      container: { classes: ["column-container"], id: "column-left" },
      template: "systems/nih/templates/advancement/trait-config-guaranteed.hbs"
    },
    choices: {
      container: { classes: ["column-container"], id: "column-left" },
      template: "systems/nih/templates/advancement/trait-config-choices.hbs"
    },
    traits: {
      container: { classes: ["column-container"], id: "column-right" },
      template: "systems/nih/templates/advancement/trait-config-traits.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    const traitConfig = rep.size === 1 ? CONFIG.NIH.traits[rep.first()] : null;
    if ( traitConfig ) {
      context.default.title = traitConfig.labels.title;
      context.default.icon = traitConfig.icon;
    } else {
      context.default.title = game.i18n.localize("NIH.TraitGenericPlural.other");
    ...
    context.trait = {
      ...
      selected: this.trait,
      selectedHeader: `${CONFIG.NIH.traits[this.trait].labels.localization}.other`,
      typeField: new StringField({
        required: true, blank: false, label: game.i18n.localize("NIH.ADVANCEMENT.Trait.TraitType")
      }),
      typeOptions: Object.entries(CONFIG.NIH.traits)
    ...
    context.mode = {
      hint: CONFIG.NIH.traitModes[this.advancement.configuration.mode].hint,
      options: Object.entries(CONFIG.NIH.traitModes).map(([value, { label }]) => ({ value, label }))
    };
  ...
  _onRender(context, options) {
    ...
    for ( const checkbox of this.element.querySelectorAll(".trait-list nih-checkbox[checked]") ) {
      const toCheck = (checkbox.name.endsWith("*") || checkbox.name.endsWith("ALL"))
        ? checkbox.closest("ol").querySelectorAll(`nih-checkbox:not([name="${checkbox.name}"])`)
        : checkbox.closest("li").querySelector("ol")?.querySelectorAll("nih-checkbox");
  ...
  _onChangeForm(formConfig, event) {
    ...
    if ( (event.target.name === "configuration.mode")
      ...
      const validTraitTypes = filteredKeys(CONFIG.NIH.traits, c => c[checkKey]);
