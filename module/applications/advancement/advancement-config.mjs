...
export default class AdvancementConfig extends FormApplication {
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      return ui.notifications.error(game.i18n.localize("NIH.AdvancementItemGrantRecursiveWarning"));
    }

    // Abort if this uuid exists already
    if ( existingItems.includes(item.uuid) ) {
      return ui.notifications.warn(game.i18n.localize("NIH.AdvancementItemGrantDuplicateWarning"));
    }
