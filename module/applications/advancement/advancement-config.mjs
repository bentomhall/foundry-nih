...
export default class AdvancementConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["nih", "advancement", "dialog"],
      template: "systems/nih/templates/advancement/advancement-config.hbs",
      width: 400,
      height: "auto",
      submitOnChange: true,
      closeOnSubmit: false,
  ...
  get title() {
    ...
    return `${game.i18n.format("NIH.AdvancementConfigureTitle", { item: this.item.name })}: ${type}`;
  }
  ...
  /** @inheritDoc */
  getData() {
    const levels = Object.fromEntries(Array.fromRange(CONFIG.NIH.maxLevel + 1).map(l => [l, l]));
    if ( ["class", "subclass"].includes(this.item.type) ) delete levels[0];
    else levels[0] = game.i18n.localize("NIH.AdvancementLevelAnyHeader");
    const context = {
      appId: this.id,
      CONFIG: CONFIG.NIH,
      ...
      classRestrictionOptions: [
        { value: "", label: game.i18n.localize("NIH.AdvancementClassRestrictionNone") },
        { value: "primary", label: game.i18n.localize("NIH.AdvancementClassRestrictionPrimary") },
        { value: "secondary", label: game.i18n.localize("NIH.AdvancementClassRestrictionSecondary") }
      ],
  ...
  async _onDrop(event) {
    ...
    if ( item.uuid === this.item.uuid ) {
      ui.notifications.error("NIH.ADVANCEMENT.ItemGrant.Warning.Recursive", {localize: true});
      return null;
    }

    // Abort if this uuid exists already
    if ( existingItems.find(i => i.uuid === item.uuid) ) {
      ui.notifications.warn("NIH.ADVANCEMENT.ItemGrant.Warning.Duplicate", {localize: true});
      return null;
    }
