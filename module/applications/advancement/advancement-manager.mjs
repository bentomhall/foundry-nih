...
export default class AdvancementManager extends Application5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      ...
      title: "NIH.ADVANCEMENT.Manager.Title.Default"
    },
    actions: {
  ...
  get subtitle() {
    ...
    if ( level ) parts.push(game.i18n.format("NIH.AdvancementLevelHeader", { level }));

    // Step Count
    const visibleSteps = this.steps.filter(s => !s.automatic);
    const visibleIndex = visibleSteps.indexOf(this.step);
    if ( visibleIndex >= 0 ) parts.push(game.i18n.format("NIH.ADVANCEMENT.Manager.Steps", {
  ...
  async _prepareContext(options) {
    ...
    return {
      ...
      flowClasses: this.step.flow instanceof Application ? "nih advancement flow" : "",
  ...
  async close(options={}) {
    if ( !options.skipConfirmation ) {
      return new foundry.applications.api.Dialog({
        window: {
          title: `${game.i18n.localize("NIH.ADVANCEMENT.Manager.ClosePrompt.Title")}: ${this.actor.name}`
        },
        position: { width: 400 },
        content: game.i18n.localize("NIH.ADVANCEMENT.Manager.ClosePrompt.Message"),
        buttons: [
          {
            action: "stop",
            icon: "fas fa-times",
            label: game.i18n.localize("NIH.ADVANCEMENT.Manager.ClosePrompt.Action.Stop"),
            default: true,
            callback: () => {
          ...
          },
          {
            ...
            label: game.i18n.localize("NIH.ADVANCEMENT.Manager.ClosePrompt.Action.Continue")
          }
        ]
      }).render({ force: true });
    }
      ...
      do {
        ...
        if ( (type === "delete") && this.step.item ) {
          if ( this.step.flow?.retainedData?.retainedItems ) {
            this.step.flow.retainedData.retainedItems[this.step.item.flags.nih?.sourceId] = this.step.item.toObject();
          }
  ...
  async #restart(event) {
    const restart = await foundry.applications.api.Dialog.confirm({
      window: { title: game.i18n.localize("NIH.ADVANCEMENT.Manager.RestartPrompt.Title") },
      content: `<p>${game.i18n.localize("NIH.ADVANCEMENT.Manager.RestartPrompt.Message")}</p>`
    });
    if ( !restart ) return;
...
class AdvancementVisualizer extends Application5e {
  ...
  static PARTS = {
    steps: {
      template: "systems/nih/templates/advancement/advancement-visualizer.hbs"
    }
  };
