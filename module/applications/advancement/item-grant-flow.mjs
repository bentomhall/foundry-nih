...
export default class ItemGrantFlow extends AdvancementFlow {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/nih/templates/advancement/item-grant-flow.hbs"
    });
  }
  ...
  async getContext() {
    ...
    const added = this.retainedData?.items.map(i => foundry.utils.getProperty(i, "flags.nih.sourceId"))
  ...
  getSelectAbilities() {
    ...
    return {
      options: config.spell?.ability.size > 1 ? config.spell.ability.reduce((obj, k) => {
        obj[k] = CONFIG.NIH.abilities[k]?.label;
        return obj;
      }, {}) : null,
  ...
  async _updateObject(event, formData) {
    const retainedData = this.retainedData?.items.reduce((obj, i) => {
      obj[foundry.utils.getProperty(i, "flags.nih.sourceId")] = i;
      return obj;
    }, {});
