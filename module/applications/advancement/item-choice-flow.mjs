...
export default class ItemChoiceFlow extends ItemGrantFlow {
  ...
  async retainData(data) {
    ...
    this.selected = new Set(data.items.map(i => foundry.utils.getProperty(i, "flags.nih.sourceId")));
  }
  ...
  async getContext() {
    ...
    if ( this.advancement.configuration.type ) {
      ...
      if ( (this.advancement.configuration.type === "feat") && this.advancement.configuration.restriction.type ) {
        const typeConfig = CONFIG.NIH.featureTypes[this.advancement.configuration.restriction.type];
      ...
      context.selectLabel = game.i18n.format("NIH.ADVANCEMENT.ItemChoice.Action.SelectSpecific", { type });
    } else {
      context.selectLabel = game.i18n.localize("NIH.ADVANCEMENT.ItemChoice.Action.SelectGeneric");
    }
  ...
  async _onBrowseCompendium(event) {
    ...
    if ( current >= max ) {
      ui.notifications.warn("NIH.ADVANCEMENT.ItemChoice.Warning.MaxSelected", { localize: true });
      return;
    }
    ...
    if ( config.type ) {
      ...
      if ( (config.type === "feat") && config.restriction.type ) {
        const typeConfig = CONFIG.NIH.featureTypes[config.restriction.type];
  ...
  async _onChangeInput(event) {
    if ( event.target.tagName === "NIH-CHECKBOX" ) {
  ...
  async _onDrop(event) {
    ...
    if ( (this.advancement.configuration.type === "spell") && spellLevel === "available" ) {
      ...
      if ( item.system.level > maxSlot ) {
        ui.notifications.error(game.i18n.format("NIH.ADVANCEMENT.ItemChoice.Warning.SpellLevelAvailable", {
          level: CONFIG.NIH.spellLevels[maxSlot]
        }));
        return null;
      }
    }
  ...
  _maxSpellSlotLevel() {
    ...
    if ( spellcasting?.type ) {
      const progression = Object.fromEntries(Object.keys(CONFIG.NIH.spellcasting).map(k => [k, 0]));
