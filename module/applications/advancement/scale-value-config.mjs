...
export default class ScaleValueConfig extends AdvancementConfig {
  ...
  static PARTS = {
    ...
    details: {
      container: { classes: ["column-container"], id: "column-left" },
      template: "systems/nih/templates/advancement/scale-value-config-details.hbs"
    },
    levels: {
      container: { classes: ["column-container"], id: "column-right" },
      template: "systems/nih/templates/advancement/scale-value-config-levels.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.distanceOptions = Object.entries(CONFIG.NIH.movementUnits)
  ...
  _prepareLevelData() {
    let lastValue = null;
    let levels = Array.fromRange(CONFIG.NIH.maxLevel + 1);
  ...
  async _processSubmitData(event, submitData) {
    ...
    if ( typeChange && (submitData.configuration.type !== this.advancement.configuration.type) ) {
      ...
      await this.advancement.update(Array.fromRange(CONFIG.NIH.maxLevel, 1).reduce((obj, lvl) => {
