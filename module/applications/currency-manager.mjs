...
export default class CurrencyManager extends Application5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    tag: "form",
    window: {
      title: "NIH.CurrencyManager.Title"
    }
  };
  ...
  static PARTS = {
    ...
    convert: {
      template: "systems/nih/templates/apps/currency-manager-convert.hbs"
    },
    transfer: {
      template: "systems/nih/templates/apps/currency-manager-transfer.hbs"
    }
  };
  ...
  _getTabs() {
    return {
      convert: {
        ...
        label: "NIH.CurrencyManager.Convert.Label",
      ...
      transfer: {
        ...
        label: "NIH.CurrencyManager.Transfer.Label",
  ...
  static deductActorCurrency(actor, amount, denomination, options={}) {
    ...
    if ( remainder ) throw new Error(game.i18n.format("NIH.CurrencyManager.Error.InsufficientFunds", {
      denomination,
  ...
  static getActorCurrencyUpdates(actor, amount, denomination, { recursive=false, priority="low", exact=true }={}) {
    ...
    const currencies = Object.entries(CONFIG.NIH.currencies).map(([denom, { conversion }]) => {
      return [denom, conversion];
    }).sort(([, a], [, b]) => priority === "high" ? a - b : b - a);
    const baseConversion = CONFIG.NIH.currencies[denomination].conversion;
