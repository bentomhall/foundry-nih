...
export default class EncounterActorSheet extends MultiActorSheet {
  ...
  static PARTS = {
    header: {
      template: "systems/nih/templates/actors/encounter/header.hbs"
    },
    tabs: {
    ...
    members: {
      ...
      template: "systems/nih/templates/actors/encounter/members.hbs",
      scrollable: [""]
    },
    inventory: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/nih/templates/actors/tabs/actor-inventory.hbs",
      templates: ["systems/nih/templates/inventory/inventory.hbs"],
      scrollable: [""]
    },
    description: {
      container: { classes: ["tab-body"], id: "tabs" },
  ...
  /** @override */
  static TABS = [
    { tab: "members", label: "NIH.ENCOUNTER.Tab.Members" },
    { tab: "inventory", label: "NIH.ENCOUNTER.Tab.Loot" },
    { tab: "description", label: "NIH.ENCOUNTER.Tab.Description" }
  ];
  ...
  async _prepareHeaderContext(context, options) {
    ...
    context.subtitles = [];
    if ( difficulty ) context.subtitles.push(game.i18n.localize(`NIH.ENCOUNTER.Difficulty.${difficulty}`));
    return context;
  }
  ...
  async _prepareInventoryContext(context, options) {
    ...
    const inventory = Object.values(CONFIG.Item.dataModels)
      ...
      .map(model => {
        ...
        if ( foundry.utils.isSubclass(model, nih.dataModels.item.ContainerData) ) return section;
  ...
  async _prepareMembersContext(context, options) {
    ...
    context.members = await Promise.all(members.map(async ({ actor, quantity }, index) => {
      ...
      member.subtitle = [
        ...
        game.i18n.format("NIH.ExperiencePoints.Format", { value: formatter.format(system.details.xp.value) })
      ].filterJoin(" • ");
    ...
    const [low, med, high] = (CONFIG.NIH.ENCOUNTER_DIFFICULTY[level] ?? []).map(t => t * creatures.length);
  ...
  static async #onAward() {
    new Award({
      award: {
        ...
        savedDestinations: this.actor.getFlag("nih", "awardDestinations"),
