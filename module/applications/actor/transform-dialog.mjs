...
export default class TransformDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    buttons: [{
      ...
      label: "NIH.TRANSFORM.Action.Transform",
      type: "submit"
    }],
    ...
    window: {
      title: "NIH.TRANSFORM.Dialog.Title",
  ...
  static PARTS = {
    details: {
      template: "systems/nih/templates/apps/transform-details.hbs"
    },
    presets: {
      container: { id: "settings-area" },
      template: "systems/nih/templates/apps/transform-presets.hbs"
    },
    settings: {
      container: { id: "settings-area" },
      template: "systems/nih/templates/apps/transform-settings.hbs"
    },
    footer: {
  ...
  async _preparePresetsContext(context, options) {
    context.presets = Object.entries(CONFIG.NIH.transformation.presets).reduce((obj, [key, config]) => {
      obj[key] = {
        ...config,
  ...
  #disableFields(changed) {
    const handleDisable = field => {
      ...
      const config = foundry.utils.getProperty(CONFIG.NIH.transformation, field.name);
      ...
      this.element.querySelectorAll(selector).forEach(element => {
        ...
        if ( element.disabled && element.tagName === "NIH-CHECKBOX" ) element.checked = false;
        else if ( element.disabled ) element.value = "";
      });
    };
    if ( changed ) handleDisable(changed);
    else this.element.querySelectorAll("nih-checkbox").forEach(e => handleDisable(e));
  }
  ...
  _onChangeForm(formConfig, event) {
    if ( event.target?.tagName === "NIH-CHECKBOX" ) this.#disableFields(event.target);
  ...
  static async #setPreset(event, target) {
    const preset = CONFIG.NIH.transformation.presets[target.dataset.preset];
