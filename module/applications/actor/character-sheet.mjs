...
export default class ActorSheet5eCharacter extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["nih", "sheet", "actor", "character"]
    });
  }
  ...
  async getData(options={}) {
    ...
    context.resources = ["primary", "secondary", "tertiary"].reduce((arr, r) => {
      ...
      res.name = r;
      res.placeholder = game.i18n.localize(`NIH.Resource${r.titleCase()}`);
    ...
    return foundry.utils.mergeObject(context, {
      disableExperience: game.settings.get("nih", "disableExperienceTracking"),
      ...
      weightUnit: game.i18n.localize(`NIH.Abbreviation${
        game.settings.get("nih", "metricWeightUnits") ? "Kg" : "Lbs"}`),
  ...
  _prepareItems(context) {
    ...
    let {items, spells, feats, backgrounds, classes, subclasses} = context.items.reduce((obj, item) => {
      ...
      ctx.attunement = {
        [CONFIG.NIH.attunementTypes.REQUIRED]: {
          icon: "fa-sun",
          cls: "not-attuned",
          title: "NIH.AttunementRequired"
        },
        [CONFIG.NIH.attunementTypes.ATTUNED]: {
          icon: "fa-sun",
          cls: "attuned",
    ...
    const maxLevelDelta = CONFIG.NIH.maxLevel - this.actor.system.details.level;
    classes = classes.reduce((arr, cls) => {
      const ctx = context.itemContext[cls.id] ??= {};
      ctx.availableLevels = Array.fromRange(CONFIG.NIH.maxLevel + 1).slice(1).map(level => {
    ...
    const features = {
      ...
      active: {
        label: "NIH.FeatureActive", items: [],
        hasActions: true, dataset: {type: "feat", "activation.type": "action"} },
      passive: {
        label: "NIH.FeaturePassive", items: [],
  ...
  _prepareItemToggleState(item, context) {
    if ( item.type === "spell" ) {
      ...
      if ( isAlways ) context.toggleTitle = CONFIG.NIH.spellPreparationModes.always;
      else if ( isPrepared ) context.toggleTitle = CONFIG.NIH.spellPreparationModes.prepared;
      else context.toggleTitle = game.i18n.localize("NIH.SpellUnprepared");
    }
    else {
      const isActive = !!item.system.equipped;
      context.toggleClass = isActive ? "active" : "";
      context.toggleTitle = game.i18n.localize(isActive ? "NIH.Equipped" : "NIH.Unequipped");
    ...
    switch ( button.dataset.action ) {
      case "convertCurrency":
        return Dialog.confirm({
          title: `${game.i18n.localize("NIH.CurrencyConvert")}`,
          content: `<p>${game.i18n.localize("NIH.CurrencyConvertHint")}</p>`,
