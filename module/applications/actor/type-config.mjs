...
export default class ActorTypeConfig extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["nih", "actor-type", "trait-selector"],
      template: "systems/nih/templates/apps/actor-type.hbs",
      width: 280,
      height: "auto",
      choices: {},
      allowCustom: true,
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("NIH.CreatureTypeTitle")}: ${this.object.name}`;
  }
  ...
  getData(options={}) {
    ...
    if ( foundry.utils.getType(attr) !== "Object" ) attr = {
      value: (attr in CONFIG.NIH.creatureTypes) ? attr : "humanoid",
      subtype: "",
      swarm: "",
      custom: ""
    };
    ...
    // Populate choices
    const types = {};
    for ( let [k, v] of Object.entries(CONFIG.NIH.creatureTypes) ) {
      types[k] = {
    ...
    return {
      ...
      custom: {
        value: attr.custom,
        label: game.i18n.localize("NIH.CreatureTypeSelectorCustom"),
        chosen: attr.value === "custom"
      },
      subtype: attr.subtype,
      swarm: attr.swarm,
      sizes: Array.from(Object.entries(CONFIG.NIH.actorSizes)).reverse().reduce((obj, e) => {
        obj[e[0]] = e[1];
        return obj;
      }, {}),
      preview: ActorNIH.formatCreatureType(attr) || "–"
    };
  }
  ...
  _onChangeInput(event) {
    ...
    this.form.preview.value = ActorNIH.formatCreatureType(typeObject) || "—";
  }
