...
export default class GroupActorSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["nih", "sheet", "actor", "group"],
      template: "systems/nih/templates/actors/group-sheet.hbs",
  ...
  async getData(options={}) {
    ...
    // Text labels
    context.labels = {
      currencies: Object.entries(CONFIG.NIH.currencies).reduce((obj, [k, c]) => {
        obj[k] = c.label;
        return obj;
      }, {})
    };
  ...
  #getSummary(stats) {
    ...
    const members = [];
    if ( stats.nMembers ) members.push(`${stats.nMembers} ${game.i18n.localize("NIH.GroupMembers")}`);
    if ( stats.nVehicles ) members.push(`${stats.nVehicles} ${game.i18n.localize("NIH.GroupVehicles")}`);
    if ( !members.length ) return game.i18n.localize("NIH.GroupSummaryEmpty");
    return game.i18n.format("NIH.GroupSummary", {members: formatter.format(members)});
  }
  ...
  #prepareMembers() {
    ...
    for ( const member of this.object.system.members ) {
      ...
      m.hp.color = nih.documents.ActorNIH.getHPColor(m.hp.current, m.hp.max).css;
  ...
  #prepareMovementSpeed() {
    ...
    let speeds = [
      [movement.land, `${game.i18n.localize("NIH.MovementLand")} ${movement.land}`],
      [movement.water, `${game.i18n.localize("NIH.MovementWater")} ${movement.water}`],
      [movement.air, `${game.i18n.localize("NIH.MovementAir")} ${movement.air}`]
    ];
  ...
  _createItem(button) {
    ...
    delete system.type;
    const name = game.i18n.format("NIH.ItemNew", {type: game.i18n.localize(CONFIG.Item.typeLabels[type])});
