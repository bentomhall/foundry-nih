...
export default class GroupActorSheet extends MultiActorSheet {
  ...
  static PARTS = {
    header: {
      template: "systems/nih/templates/actors/group/header.hbs"
    },
    members: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/nih/templates/actors/group/members.hbs",
      templates: ["systems/nih/templates/actors/group/member.hbs"],
      scrollable: [""]
    },
    inventory: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/nih/templates/actors/group/inventory.hbs",
      templates: [
    ...
    biography: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/nih/templates/actors/group/biography.hbs",
      scrollable: [""]
    },
    tabs: {
      id: "tabs",
  ...
  /** @override */
  static TABS = [
    { tab: "members", label: "NIH.Group.Member.other", icon: "fa-solid fa-users"},
    { tab: "inventory", label: "NIH.Inventory", svg: "systems/nih/icons/svg/backpack.svg" },
    { tab: "biography", label: "NIH.Biography", icon: "fa-solid fa-feather" }
  ];
  ...
  /** @inheritDoc */
  get inventorySource() {
    const inventorySource = this.actor.getFlag("nih", "inventorySource") ?? "group";
  ...
  async _prepareHeaderContext(context, options) {
    context.showXP = game.settings.get("nih", "levelingMode") !== "noxp";
  ...
  _renderInventoryToggle() {
    ...
    button.innerHTML = `
      <button type="button" class="always-interactive split-control" data-action="toggleInventory"
              ...
              aria-label="${game.i18n.localize("NIH.Group.Action.Inventory.Group")}">
        <i class="fa-solid fa-list-ul fa-fw" inert></i>
      </button>
      <button type="button" class="always-interactive split-control" data-action="toggleInventory"
              data-inventory="vehicle" aria-pressed="${!groupInventory}" data-tooltip
              aria-label="${game.i18n.localize("NIH.Group.Action.Inventory.Vehicle")}">
  ...
  async _prepareMemberEncumbrance(actor, context) {
    ...
    const defaultUnits = CONFIG.NIH.encumbrance.baseUnits.default;
    const baseUnits = CONFIG.NIH.encumbrance.baseUnits[actor.type] ?? defaultUnits;
    const systemUnits = game.settings.get("nih", "metricWeightUnits") ? "metric" : "imperial";
    context.encumbrance = {
      pct,
  ...
  _prepareMemberSkills(actor, context) {
    context.skills = Object.fromEntries(Object.entries(actor.system.skills ?? {}).map(([key, skill]) => {
      ...
      const label = game.i18n.format(actor.isOwner ? "NIH.SkillRoll" : "NIH.SkillTitle", {
        ability: CONFIG.NIH.abilities[ability]?.label,
        skill: CONFIG.NIH.skills[key]?.label
      });
  ...
  async _prepareVehicleContext(actor, context, options) {
    ...
    context.properties = [];
    if ( attributes.ac.value ) context.properties.push({ label: "NIH.AC", value: attributes.ac.value });
    if ( attributes.hp.dt ) context.properties.push({ label: "NIH.HITPOINTS.DT.abbr", value: attributes.hp.dt });
    const speed = attributes.travel.paces.max;
    if ( Number.isFinite(speed) ) context.properties.push({ label: "NIH.Speed", value: speed });
  }
  ...
  static #onAward() {
    new Award({
      award: { savedDestinations: this.actor.getFlag("nih", "awardDestinations") },
      origin: this.actor
    }).render({ force: true });
  }
  ...
  static #onChangePace(event, target) {
    ...
    const paces = Object.keys(CONFIG.NIH.travelPace);
  ...
  static #onToggleInventory(event, target) {
    ...
    this.actor.setFlag("nih", "inventorySource", inventory);
  }
  ...
  _getEntryContextOptions() {
    return super._getEntryContextOptions().concat([{
      name: "NIH.Group.Action.SetPrimaryVehicle",
      ...
      name: "NIH.Group.Action.UnsetPrimaryVehicle",
