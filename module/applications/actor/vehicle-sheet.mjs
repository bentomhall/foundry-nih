...
export default class ActorSheet5eVehicle extends ActorSheet5e {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["nih", "sheet", "actor", "vehicle"]
    });
  }
  ...
  _computeEncumbrance(totalWeight, actorData) {
    ...
    totalWeight /= game.settings.get("nih", "metricWeightUnits")
      ? CONFIG.NIH.encumbrance.vehicleWeightMultiplier.metric
      : CONFIG.NIH.encumbrance.vehicleWeightMultiplier.imperial;
  ...
  _prepareCrewedItem(item, context) {
    ...
    context.toggleTitle = game.i18n.localize(`NIH.${isCrewed ? "Crewed" : "Uncrewed"}`);

    // Handle crew actions
    if ( item.type === "feat" && item.system.activation.type === "crew" ) {
      context.cover = game.i18n.localize(`NIH.${item.system.cover ? "CoverTotal" : "None"}`);
  ...
  _prepareItems(context) {
    const cargoColumns = [{
      label: game.i18n.localize("NIH.Quantity"),
      css: "item-qty",
      property: "quantity",
      editable: "Number"
    }];
    ...
    const equipmentColumns = [{
      ...
      label: game.i18n.localize("NIH.AC"),
      css: "item-ac",
      property: "system.armor.value"
    }, {
      label: game.i18n.localize("NIH.HP"),
      css: "item-hp",
      property: "system.hp.value",
      editable: "Number"
    }, {
      label: game.i18n.localize("NIH.Threshold"),
      css: "item-threshold",
      property: "threshold"
    }];
    ...
    const features = {
      actions: {
        label: game.i18n.localize("NIH.ActionPl"),
        items: [],
        hasActions: true,
        crewable: true,
      ...
      passive: {
        label: game.i18n.localize("NIH.Features"),
        items: [],
        dataset: {type: "feat"}
      },
      reactions: {
        label: game.i18n.localize("NIH.ReactionPl"),
        items: [],
    ...
    const cargo = {
      crew: {
        label: game.i18n.localize("NIH.VehicleCrew"),
      ...
      passengers: {
        label: game.i18n.localize("NIH.VehiclePassengers"),
      ...
      cargo: {
        label: game.i18n.localize("NIH.VehicleCargo"),
        items: [],
        dataset: {type: "loot"},
        columns: [{
          label: game.i18n.localize("NIH.Quantity"),
          ...
          label: game.i18n.localize("NIH.Price"),
          css: "item-price",
          property: "system.price.value",
          editable: "Number"
        }, {
          label: game.i18n.localize("NIH.Weight"),
          css: "item-weight",
    ...
    for ( const item of context.items ) {
      ...
      const isCargo = item.flags.nih?.vehicleCargo === true;
      if ( isCargo ) {
   ...
   * @returns {Promise<ActorNIH>|null}  Actor after update if any changes were made.
   * @private
   */
  ...
  async _onDropSingleItem(itemData) {
    ...
    foundry.utils.setProperty(itemData, "flags.nih.vehicleCargo", isCargo);
