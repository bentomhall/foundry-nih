 ...
 *
 * @param {ActorNIH} actor           Actor that is taking the short rest.
...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/nih/templates/apps/short-rest.hbs",
      classes: ["nih", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("nih", "restVariant");
   ...
   * @param {ActorNIH} [options.actor]  Actor that is taking the short rest.
   * @returns {Promise}                Promise that resolves when the rest is completed or rejects when canceled.
   */
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("NIH.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("NIH.Rest"),
            callback: html => {
              let newDay = false;
              if ( game.settings.get("nih", "restVariant") !== "epic" ) {
