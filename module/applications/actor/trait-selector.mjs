 ...
 *
 * @param {ActorNIH} actor                       Actor for whose traits are being edited.
...
export default class TraitSelector extends BaseConfigSheet {
  constructor(actor, trait, options={}) {
    if ( !CONFIG.NIH.traits[trait] ) throw new Error(
      `Cannot instantiate TraitSelector with a trait not defined in CONFIG.NIH.traits: ${trait}.`
    );
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      id: "trait-selector",
      classes: ["nih", "trait-selector", "subconfig"],
      template: "systems/nih/templates/apps/trait-selector.hbs",
      width: 320,
      height: "auto",
      sheetConfig: false,
      allowCustom: true
  ...
  async getData() {
    ...
    return {
      ...
      bypasses: "bypasses" in data ? Object.entries(CONFIG.NIH.physicalWeaponProperties).reduce((obj, [k, v]) => {
