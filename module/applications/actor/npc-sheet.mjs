...
export default class NPCActorSheet extends BaseActorSheet {
  ...
  static PARTS = {
    header: {
      template: "systems/nih/templates/actors/npc-header.hbs"
    },
    sidebarCollapser: {
    ...
    sidebar: {
      container: { classes: ["main-content"], id: "main" },
      template: "systems/nih/templates/actors/npc-sidebar.hbs"
    },
    features: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/nih/templates/actors/tabs/actor-features.hbs",
    ...
    inventory: {
      ...
      templates: [
        ...
        "systems/nih/templates/inventory/encumbrance.hbs"
      ],
      scrollable: [""]
    },
    spells: {
    ...
    biography: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/nih/templates/actors/tabs/npc-biography.hbs",
      scrollable: [""]
    },
    specialTraits: {
      classes: ["flexcol"],
  ...
  async _prepareFeaturesContext(context, options) {
    const sections = Object.entries(CONFIG.NIH.activityActivationTypes).reduce((obj, [id, config], i) => {
    ...
    sections.passive = {
      id: "passive", label: "NIH.Features", order: 0, items: [], minWidth: 210,
      columns: ["recovery", "uses", "roll", "formula", "controls"]
    };
    ...
    context.listControls = {
      ...
      list: "features",
      filters: [
        { key: "action", label: "NIH.ACTIVATION.Type.Action.Label" },
        { key: "bonus", label: "NIH.ACTIVATION.Type.BonusAction.Label" },
        { key: "reaction", label: "NIH.ACTIVATION.Type.Reaction.Label" },
        { key: "legendary", label: "NIH.ACTIVATION.Type.Legendary.Label" },
        { key: "lair", label: "NIH.ACTIVATION.Type.Lair.Label" }
      ],
      sorting: [
  ...
  async _prepareHeaderContext(context, options) {
    ...
    for ( const res of ["legact", "legres"] ) {
      ...
      context[res] = Array.fromRange(max, 1).map(n => {
        ...
        return {
          n: max - n, filled,
          tooltip: `NIH.${i18n}.Label`,
          label: game.i18n.format(`NIH.${i18n}.Ordinal.${plurals.select(n)}`, { n }),
    ...
    if ( this._mode === this.constructor.MODES.PLAY ) {
      ...
      context.showInitiativeScore = game.settings.get("nih", "rulesVersion") === "modern";
    }
    context.showLoyalty = context.important && game.settings.get("nih", "loyaltyScore") && game.user.isGM;
  ...
  async _prepareSidebarContext(context, options) {
    ...
    if ( details.habitat.value.length || details.habitat.custom ) {
      ...
      context.habitat = [
        ...habitat.value.map(({ type, subtype }) => {
          let { label } = CONFIG.NIH.habitats[type] ?? {};
          if ( label && (!any || (type === "any")) ) {
            if ( subtype ) label = game.i18n.format("NIH.Habitat.Subtype", { type: label, subtype });
            return { label };
          }
          return null;
        }, []).filter(_ => _),
    ...
    if ( this.actor.system.skills.prc ) context.senses.push({
      ...
      label: game.i18n.localize("NIH.PassivePerception"),
      value: this.actor.system.skills.prc.passive
    });

    // Skills & Tools
    const skillSetting = game.settings.get("nih", "defaultSkills");
    ...
    // Speed
    context.speed = [
      ...Object.entries(CONFIG.NIH.movementTypes).map(([k, { label }]) => {
        const value = attributes.movement[k];
        if ( !value ) return null;
        const data = { label, value };
        if ( (k === "fly") && attributes.movement.hover ) data.icons = [{
          icon: "fas fa-cloud", label: game.i18n.localize("NIH.MOVEMENT.Hover")
        }];
        return data;
      }),
    ...
    if ( details?.treasure?.value.size ) {
      ...
      context.treasure = Array.from(details.treasure.value)
        .map(id => {
          const { label } = CONFIG.NIH.treasure[id] ?? {};
  ...
  async _prepareSpecialTraitsContext(context, options) {
    ...
    context.flags.sections.unshift({
      label: game.i18n.localize("NIH.NPC.Label"),
      fields: [{
        ...
        label: "NIH.NPC.FIELDS.attributes.price.label",
        hint: "NIH.NPC.FIELDS.attributes.price.hint",
        fields: [{
  ...
  async _prepareSpellsContext(context, options) {
    ...
    context.spellcasting.push({
      label: game.i18n.format("NIH.SpellcastingClass", {
        class: spellcaster?.name ?? game.i18n.format("NIH.NPC.Label")
      }),
      level: spellcaster?.system.levels ?? attributes.spell.level,
      ability: {
        ability, mod,
        label: CONFIG.NIH.abilities[ability]?.label
      },
      ...
      concentration: {
        ...
        tooltip: game.i18n.format("NIH.AbilityConfigure", { ability: game.i18n.localize("NIH.Concentration") })
      }
    });
  ...
  async _prepareItem(item, ctx) {
    ...
    const isPassive = item.system.properties?.has("trait")
      || CONFIG.NIH.activityActivationTypes[item.system.activities?.contents[0]?.activation.type]?.passive;
  ...
  async _onRender(context, options) {
    ...
    elements.innerText = xp === null ? "" : game.i18n.format("NIH.ExperiencePoints.Format", {
