...
export default class BaseRestDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    templates: ["systems/nih/templates/actors/rest/rest-request.hbs"]
  };
   ...
   */
  get promptNewDay() {
    const duration = CONFIG.NIH.restTypes[this.config.type]
      ?.duration?.[game.settings.get("nih", "restVariant")] ?? 0;
  ...
  async _prepareContext(options) {
    const context = {
      ...
      variant: game.settings.get("nih", "restVariant")
    };
    if ( this.promptNewDay ) context.fields.push({
      disabled: !!this.config.request,
      field: new BooleanField({
        label: game.i18n.localize("NIH.REST.NewDay.Label"),
        hint: game.i18n.localize("NIH.REST.NewDay.Hint")
      }),
    ...
    const rest = CONFIG.NIH.restTypes[this.config.type];
    if ( "recoverTemp" in rest ) context.hitPoints.push({
      disabled: !!this.config.request,
      field: new BooleanField({
        label: game.i18n.localize("NIH.REST.RecoverTempHP.Label")
      }),
      input: context.inputs.createCheckboxInput,
      name: "recoverTemp",
    ...
    if ( "recoverTempMax" in rest ) context.hitPoints.push({
      disabled: !!this.config.request,
      field: new BooleanField({
        label: game.i18n.localize("NIH.REST.RecoverTempMaxHP.Label"),
        hint: game.i18n.localize("NIH.REST.RecoverTempMaxHP.Hint")
      }),
      input: context.inputs.createCheckboxInput,
      name: "recoverTempMax",
    ...
    if ( this.isPartyGroup ) {
      const restSettings = this.actor.getFlag("nih", "restSettings") ?? {};
      context.request = [
        {
          field: new BooleanField({
            label: game.i18n.localize("NIH.REST.Request.AutoRest.Label"),
            hint: game.i18n.localize("NIH.REST.Request.AutoRest.Hint")
          }),
          name: "autoRest",
          input: context.inputs.createCheckboxInput,
  ...
  static async #handleFormSubmission(event, form, formData) {
    ...
    if ( this.isPartyGroup ) {
      ...
      this.actor.setFlag("nih", "restSettings", data);
    }
        ...
        buttons: [
          {
            ...
            label: game.i18n.localize("NIH.REST.Label"),
            name: "rest",
            type: "submit"
          }
        ],
