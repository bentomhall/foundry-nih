...
export default class MultiActorSheet extends BaseActorSheet {
  ...
  async _prepareMemberPortrait(actor, context) {
    const showTokenPortrait = this.actor.getFlag("nih", "showTokenPortrait");
  ...
  async _onDropCreateItems(event, items, behavior) {
    ...
    if ( foundNonPhysical ) ui.notifications.warn("NIH.Group.Warning.PhysicalItemOnly", { localize: true });
  ...
  static addDocumentSheetConfigOptions(app, html) {
    ...
    const showTokenPortrait = doc.getFlag("nih", "showTokenPortrait");
    const artOptions = {
      false: game.i18n.localize("NIH.Group.Config.Art.portraits"),
      true: game.i18n.localize("NIH.Group.Config.Art.tokens")
    };
    const fieldset = document.createElement("fieldset");
    fieldset.innerHTML = `
      <legend>${game.i18n.localize("NIH.Group.Config.Legend")}</legend>
      <div class="form-group">
        <label>${game.i18n.localize("NIH.Group.Config.Art.Label")}</label>
        <div class="form-fields">
          <select name="flags.nih.showTokenPortrait" data-dtype="Boolean">
