...
export default class ArmorClassConfig extends BaseConfigSheet {
  ...
  static PARTS = {
    config: {
      template: "systems/nih/templates/actors/config/armor-class-config.hbs"
    }
  };
  ...
  /** @override */
  get title() {
    return game.i18n.localize("NIH.ArmorClass");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    context.calculationOptions = Object.entries(CONFIG.NIH.armorClasses).reduce((arr, [value, config]) => {
    ...
    const config = CONFIG.NIH.armorClasses[context.source.calc];
    context.formula = {
