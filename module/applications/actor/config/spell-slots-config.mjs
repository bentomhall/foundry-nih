...
export default class SpellSlotsConfig extends BaseConfigSheet {
  ...
  static PARTS = {
    config: {
      template: "systems/nih/templates/actors/config/spell-slots-config.hbs"
    }
  };
  ...
  /** @override */
  get title() {
    return game.i18n.localize("NIH.SpellSlotsConfig");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    const maxLevel = Object.keys(CONFIG.NIH.spellLevels).length - 1;
    ...
    const models = Object.entries(CONFIG.NIH.spellcasting).sort(([a]) => a === "spell" ? -1 : 0);
      ...
      for ( let i = model.isSingleLevel ? maxLevel : 1; i <= maxLevel; i++ ) {
        ...
        arr.push({
          ...
          label: model.isSingleLevel
            ? game.i18n.localize(`NIH.SPELLCASTING.METHODS.${method.capitalize()}.abbr`)
