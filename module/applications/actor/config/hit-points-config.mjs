...
export default class HitPointsConfig extends BaseConfigSheet {
  ...
  static PARTS = {
    config: {
      template: "systems/nih/templates/actors/config/hit-points-config.hbs"
    }
  };
  ...
  /** @override */
  get title() {
    return game.i18n.localize("NIH.HitPoints");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    const ability = CONFIG.NIH.abilities[CONFIG.NIH.defaultAbilities.hitPoints];
    const mod = this.document.system.abilities?.[CONFIG.NIH.defaultAbilities.hitPoints]?.mod ?? 0;
  ...
  static async #rollFormula(event, target) {
    try {
      ...
      ui.notifications.error("NIH.HPFormulaError", { localize: true });
      throw error;
    }
  }
