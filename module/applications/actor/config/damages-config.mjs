...
export default class DamagesConfig extends TraitsConfig {
  ...
  static PARTS = {
    traits: {
      template: "systems/nih/templates/actors/config/damages-config.hbs"
    }
  };
  ...
  /** @override */
  get otherLabel() {
    return game.i18n.localize("NIH.DamageTypes");
  }
  ...
  async _preparePartContext(partId, context, options) {
    ...
    context.bypasses = new SelectChoices(Object.entries(CONFIG.NIH.itemProperties).reduce((obj, [k, v]) => {
    ...
    if ( this.options.trait === "dm" ) {
      ...
      context.bypassHint = "NIH.DamageModification.BypassHint";
      context.hint = "NIH.DamageModification.Hint";
      context.value.field = new FormulaField({ determinstic: true });
      context.value.key = "amount";
    } else {
      context.bypassHint = "NIH.DamagePhysicalBypassHint";
  ...
  _processChoice(data, key, choice, categoryChosen=false) {
    ...
    const config = CONFIG.NIH.damageTypes[key];
