...
export default class SkillToolRollConfigurationDialog extends D20RollConfigurationDialog {
  ...
  async _prepareConfigurationContext(context, options) {
    ...
    if ( this.options.chooseAbility ) context.fields.unshift({
      field: new foundry.data.fields.StringField({
        required: true, blank: false, label: game.i18n.localize("NIH.Abilities")
      }),
      name: "ability",
      options: Object.entries(CONFIG.NIH.abilities).map(([value, { label }]) => ({ value, label })),
  ...
  _onChangeForm(formConfig, event) {
    ...
    if ( this.config.skill && (event.target?.name === "ability") ) {
      const skillLabel = CONFIG.NIH.skills[this.config.skill]?.label ?? "";
      const ability = event.target.value ?? this.config.ability;
      const abilityLabel = CONFIG.NIH.abilities[ability]?.label ?? "";
      const flavor = game.i18n.format("NIH.SkillPromptTitle", { skill: skillLabel, ability: abilityLabel });
