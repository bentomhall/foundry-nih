...
export default class ItemListControlsElement extends MaybeAdoptable {
  ...
  static CONFIG = {
    inventory: {
      ...
      grouping: [
        {
          key: "type",
          label: "NIH.FilterGroupCategory",
          dataset: { icon: "fa-solid fa-layer-group", classes: "active" }
        },
        { key: "contents", label: "NIH.FilterGroupCategory", dataset: { icon: "fa-solid fa-layer-group" } }
      ]
    }
  };
   ...
   */
  get prefs() {
    return game.user.getFlag("nih", `sheetPrefs.${this.app.document.type}.tabs.${this.tab}`);
  }
      ...
      <ul class="unlist controls">
        <li>
          <button type="button" class="unbutton filter-control always-interactive" data-action="clear"
                  data-tooltip aria-label="${game.i18n.localize("NIH.FilterClear")}">
    ...
    if ( !foundry.utils.isEmpty(this.#filters) ) {
      ...
      item.innerHTML = `
        <button type="button" class="unbutton filter-control filter always-interactive" data-action="filter"
                aria-label="${game.i18n.localize("NIH.Filter")}">
  ...
  async _onCycleMode(event) {
    ...
    const current = Math.max(0, modes.indexOf(game.user.getFlag("nih", flag)));
    await game.user.setFlag("nih", flag, modes[(current + 1) % modes.length]);
    if ( action === "group" ) {
      this._initGrouping();
      this._applyGrouping();
    } else {
