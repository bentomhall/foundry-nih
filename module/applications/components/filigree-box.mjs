...
export default class FiligreeBoxElement extends AdoptedStyleSheetMixin(MaybeAdoptable) {
  ...
  static CSS = `
    :host {
      ...
      isolation: isolate;
      min-height: 56px;
      filter: var(--filigree-drop-shadow, drop-shadow(0 0 12px var(--nih-shadow-15)));
    }
    .backdrop {
      --chamfer: 12px;
      position: absolute;
      inset: 0;
      background: var(--filigree-background-color, var(--nih-color-card));
      z-index: -2;
      clip-path: polygon(
        var(--chamfer) 0,
    ...
    .filigree {
      position: absolute;
      fill: var(--filigree-border-color, var(--nih-color-gold));
      z-index: -1;
