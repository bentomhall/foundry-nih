...
export default class EffectsElement extends (foundry.applications.elements.AdoptableHTMLElement ?? HTMLElement) {
  ...
  static prepareCategories(effects, { parent }={}) {
    ...
    const categories = {
      enchantment: {
        type: "enchantment",
        label: game.i18n.localize("NIH.ENCHANTMENT.Category.General"),
        effects: [],
        isEnchantment: true
      },
      temporary: {
        type: "temporary",
        label: game.i18n.localize("NIH.EffectTemporary"),
        effects: []
      },
      enchantmentActive: {
        type: "activeEnchantment",
        label: game.i18n.localize("NIH.ENCHANTMENT.Category.Active"),
        effects: [],
        isEnchantment: true
      },
      passive: {
        type: "passive",
        label: game.i18n.localize("NIH.EffectPassive"),
        effects: []
      },
      enchantmentInactive: {
        type: "inactiveEnchantment",
        label: game.i18n.localize("NIH.ENCHANTMENT.Category.Inactive"),
        effects: [],
        isEnchantment: true
      },
      inactive: {
        type: "inactive",
        label: game.i18n.localize("NIH.EffectInactive"),
        effects: []
      },
      suppressed: {
        type: "suppressed",
        label: game.i18n.localize("NIH.EffectUnavailable"),
        effects: [],
        disabled: true,
        info: [game.i18n.localize("NIH.EffectUnavailableInfo")]
      }
    };
    ...
    for ( const category of Object.values(categories) ) {
      category.localizationPrefix = category.isEnchantment ? "NIH.ENCHANTMENT.Action." : "NIH.Effect";
    }
  ...
  async _onToggleCondition(conditionId) {
    const existing = this.document.effects.get(staticID(`nih${conditionId}`));
  ...
  async _onCreate(target) {
    ...
    return this.document.createEmbeddedDocuments("ActiveEffect", [{
      ...
      name: isActor ? game.i18n.localize("NIH.EffectNew") : this.document.name,
  ...
  async _onClickEffectSource(event) {
    ...
    if ( !doc.testUserPermission(game.user, "LIMITED") ) {
      ui.notifications.warn("NIH.DocumentViewWarn", { localize: true });
      return;
    }
