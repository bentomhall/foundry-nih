  ...
  return class TargetedApplicationElement extends Base {
    ...
    buildTargetContainer() {
      ...
      this.targetSourceControl.innerHTML = `
        <button type="button" class="unbutton" data-mode="targeted" aria-pressed="false">
          <i class="fa-solid fa-bullseye" inert></i> ${game.i18n.localize("NIH.Tokens.Targeted")}
        </button>
        <button type="button" class="unbutton" data-mode="selected" aria-pressed="false">
          <i class="fa-solid fa-expand" inert></i> ${game.i18n.localize("NIH.Tokens.Selected")}
        </button>
      `;
      this.targetSourceControl.querySelectorAll("button").forEach(b =>
        b.addEventListener("click", this._onChangeTargetMode.bind(this))
      );
      if ( !this.chatMessage?.getFlag("nih", "targets")?.length ) this.targetSourceControl.hidden = true;
    ...
    buildTargetsList() {
      ...
      switch ( this.targetingMode ) {
        case "targeted":
          this.chatMessage?.getFlag("nih", "targets")?.forEach(t => targetedTokens.set(t.uuid, t.name));
          break;
        case "selected":
      ...
      if ( targets.length ) this.targetList.replaceChildren(...targets);
        ...
        li.innerText = game.i18n.localize(`NIH.Tokens.None${this.targetingMode.capitalize()}`);
