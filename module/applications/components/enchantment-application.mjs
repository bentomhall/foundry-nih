...
export default class EnchantmentApplicationElement extends MaybeAdoptable {
  ...
  connectedCallback() {
    ...
    const scaling = this.chatMessage.getFlag("nih", "scaling");
    if ( scaling ) item = item.clone({ "flags.nih.scaling": scaling });
    ...
    if ( maxTargets ) {
      ...
      this.countArea.innerHTML = game.i18n.format("NIH.ENCHANT.Enchanted", {
   ...
   */
  buildItemList() {
    const enchantedItems = nih.registry.enchantments.applied(this.enchantmentActivity.uuid).map(enchantment => {
      ...
      if ( item.isOwner ) {
        ...
        control.ariaLabel = game.i18n.localize("NIH.ENCHANTMENT.Action.Remove");
        control.dataset.action = "removeEnchantment";
        control.dataset.tooltip = "NIH.ENCHANTMENT.Action.Remove";
    ...
    else this.dropArea.innerHTML = `<p>${game.i18n.localize("NIH.ENCHANT.DropArea")}</p>`;
  ...
  async _onDrop(event) {
    ...
    const concentrationId = this.chatMessage.getFlag("nih", "use.concentrationId");
    const concentration = this.enchantmentActivity.actor.effects.get(concentrationId);
    if ( concentrationId && !concentration ) {
      ui.notifications.error("NIH.ENCHANT.Warning.ConcentrationEnded", { console: false, localize: true });
      return;
    }

    this.enchantmentActivity.applyEnchantment(
      this.chatMessage.getFlag("nih", "use.enchantmentProfile"),
      droppedItem,
