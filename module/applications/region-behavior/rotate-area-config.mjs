...
export default class RotateAreaConfig extends foundry.applications.sheets.RegionBehaviorConfig {
  ...
  static PARTS = {
    form: {
      template: "systems/nih/templates/region-behaviors/rotate-area-config.hbs",
      scrollable: [""]
    },
    footer: {
  ...
  _prepareSubmitData(event, form, formData, updateData) {
    ...
    if ( submitData.system?.regions?.ids?.includes(this.document.parent.id) ) {
      throw new Error(game.i18n.localize("NIH.REGIONBEHAVIORS.ROTATEAREA.Warning.RecursiveRegion"));
    }
    return submitData;
  }
}
