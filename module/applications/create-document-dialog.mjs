...
export default class CreateDocumentDialog extends Dialog5e {
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/nih/templates/apps/document-create.hbs"
    }
  };
  ...
  static async #handleFormSubmission(event, form, formData) {
    if ( !form.checkValidity() ) throw new Error(game.i18n.format("DOCUMENT.NIH.Warning.SelectType", {
      name: game.i18n.localize(documentType.metadata.label ?? `DOCUMENT.NIH.${documentType.documentName}`)
    }));
  ...
  static async prompt(documentType, data={}, { folders, types, ...createOptions }={}, { ok={}, ...config }={}) {
    const label = game.i18n.localize(documentType.metadata.label ?? `DOCUMENT.NIH.${documentType.documentName}`);
    ...
    dialog.addEventListener("close", event => {
      ...
      // TODO: Temp patch until advancement data is migrated (https://github.com/foundryvtt/nih/issues/5782)
