...
export default class ItemChoiceAdvancement extends ItemGrantAdvancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/nih/icons/svg/item-choice.svg",
      title: game.i18n.localize("NIH.ADVANCEMENT.ItemChoice.Title"),
      hint: game.i18n.localize("NIH.ADVANCEMENT.ItemChoice.Hint"),
      multiLevel: true,
      apps: {
  ...
  titleForLevel(level, { configMode=false }={}) {
    ...
    let tag;
    if ( data.count ) tag = game.i18n.format("NIH.ADVANCEMENT.ItemChoice.Choose", { count: data.count });
    else if ( data.replacement ) tag = game.i18n.localize("NIH.ADVANCEMENT.ItemChoice.Replacement.Title");
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return Object.values(items).reduce((html, uuid) => html + game.nih.utils.linkForUuid(uuid), "");
  }
  ...
  restore(level, data) {
    ...
    if ( data.replaced ) {
      if ( !original ) {
        throw new ItemChoiceAdvancement.ERROR(game.i18n.localize("NIH.ADVANCEMENT.ItemChoice.Warning.NoOriginal"));
      }
  ...
  _validateItemType(item, { type, restriction, strict=true }={}) {
    ...
    if ( type && (type !== item.type) ) {
      ...
      return handleError("NIH.ADVANCEMENT.ItemChoice.Warning.InvalidType", { type: typeLabel });
    }

    // If additional type restrictions applied, make sure they are valid
    if ( (type === "feat") && restriction.type ) {
      const typeConfig = CONFIG.NIH.featureTypes[restriction.type];
      ...
      if ( errorLabel ) return handleError("NIH.ADVANCEMENT.ItemChoice.Warning.InvalidType", { type: errorLabel });
    }
    ...
    if ( (type === "spell") && !Number.isNaN(l) && (item.system.level !== l) ) {
      ...
      return handleError("NIH.ADVANCEMENT.ItemChoice.Warning.SpellLevelSpecific", { level });
    }
    ...
    if ( (type === "spell") && restriction.list.size ) {
      const lists = Array.from(restriction.list)
        .map(l => nih.registry.spellLists.forType(l))
        .filter(_ => _);
      if ( !lists.some(l => l.has(item)) ) return handleError("NIH.ADVANCEMENT.ItemChoice.Warning.SpellList", {
