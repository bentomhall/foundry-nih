...
export default class ItemChoiceAdvancement extends ItemGrantAdvancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      order: 50,
      icon: "systems/nih/icons/svg/item-choice.svg",
      title: game.i18n.localize("NIH.AdvancementItemChoiceTitle"),
      hint: game.i18n.localize("NIH.AdvancementItemChoiceHint"),
      multiLevel: true,
      apps: {
  ...
  titleForLevel(level, { configMode=false }={}) {
    return `${this.title} <em>(${game.i18n.localize("NIH.AdvancementChoices")})</em>`;
  }
  ...
  _validateItemType(item, { type, restriction, strict=true }={}) {
    ...
    if ( type && (type !== item.type) ) {
      ...
      if ( strict ) throw new Error(game.i18n.format("NIH.AdvancementItemChoiceTypeWarning", {type: typeLabel}));
      return false;
    }
    ...
    if ( (type === "feat") && restriction.type ) {
      ...
      if ( errorLabel ) {
        if ( strict ) throw new Error(game.i18n.format("NIH.AdvancementItemChoiceTypeWarning", {type: errorLabel}));
        return false;
      }
    }
