...
export default class TraitAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/nih/icons/svg/trait.svg",
      title: game.i18n.localize("NIH.ADVANCEMENT.Trait.Title"),
      hint: game.i18n.localize("NIH.ADVANCEMENT.Trait.Hint"),
      apps: {
        config: TraitConfig,
        flow: TraitFlow
      }
  ...
  static localize() {
    ...
    localizeSchema(
      ...
      ["NIH.ADVANCEMENT.Trait.FIELDS.choices"]
    );
  }
  ...
  sortingValueForLevel(levels) {
    const traitOrder = Object.keys(CONFIG.NIH.traits).findIndex(k => k === this.representedTraits().first());
    const modeOrder = Object.keys(CONFIG.NIH.traitModes).findIndex(k => k === this.configuration.mode);
  ...
  async apply(level, data) {
    ...
    for ( const key of data.chosen ) {
      ...
      if ( key.startsWith("tool") ) {
        ...
        const ability = CONFIG.NIH.tools[toolId]?.ability;
  ...
  async actorSelected() {
    ...
    const traitTypes = this.configuration.mode === "default" ? Object.keys(CONFIG.NIH.traits).filter(k => k !== "dm")
      : filteredKeys(CONFIG.NIH.traits, t => t[this.configuration.mode === "mastery" ? "mastery" : "expertise"]);
  ...
  async availableChoices(chosen) {
    ...
    if ( this.configuration.allowReplacements && (unfilteredLength > available.length) ) {
      ...
      if ( rep.size === 1 ) return {
        ...
        label: game.i18n.format("NIH.ADVANCEMENT.Trait.ChoicesRemaining", {
