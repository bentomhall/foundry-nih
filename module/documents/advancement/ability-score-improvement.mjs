...
export default class AbilityScoreImprovementAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/nih/icons/svg/ability-score-improvement.svg",
      title: game.i18n.localize("NIH.ADVANCEMENT.AbilityScoreImprovement.Title"),
      hint: game.i18n.localize("NIH.ADVANCEMENT.AbilityScoreImprovement.Hint"),
      apps: {
  ...
  /** @inheritDoc */
  get _defaultTitle() {
    if ( this.isEpicBoon ) return game.i18n.localize("NIH.ADVANCEMENT.AbilityScoreImprovement.TitleEpic");
   ...
   */
  get allowFeat() {
    return (this.item.type === "class") && (game.settings.get("nih", "allowFeats")
   ...
   */
  canImprove(ability) {
    return CONFIG.NIH.abilities[ability]?.improvement !== false;
  }
  ...
  titleForLevel(level, { configMode=false }={}) {
    ...
    return game.i18n.localize("NIH.Feature.Feat");
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    if ( configMode && this.isEpicBoon ) {
      return nih.utils.linkForUuid(this.configuration.recommendation);
    }

    else if ( configMode ) {
      const entries = Object.entries(this.configuration.fixed).map(([key, value]) => {
        if ( !value ) return null;
        const name = CONFIG.NIH.abilities[key]?.label ?? key;
      ...
      if ( this.configuration.points ) entries.push(`<span class="tag">${
        game.i18n.localize("NIH.ADVANCEMENT.AbilityScoreImprovement.FIELDS.points.label")}: <strong>${
  ...
  async apply(level, data) {
    if ( data.type === "asi" ) {
      const assignments = Object.keys(CONFIG.NIH.abilities).reduce((obj, key) => {
