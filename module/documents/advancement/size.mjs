...
export default class SizeAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/nih/icons/svg/size.svg",
      title: game.i18n.localize("NIH.ADVANCEMENT.Size.Title"),
      hint: game.i18n.localize("NIH.ADVANCEMENT.Size.Hint"),
      apps: {
        config: SizeConfig,
        flow: SizeFlow
      }
  ...
  get automaticHint() {
    ...
    if ( this.configuration.sizes.size === 1 ) return game.i18n.format("NIH.ADVANCEMENT.Size.DefaultHint.Single", {
      size: CONFIG.NIH.actorSizes[this.configuration.sizes.first()].label
    });

    const listFormatter = new Intl.ListFormat(game.i18n.lang, { type: "disjunction" });
    return game.i18n.format("NIH.ADVANCEMENT.Size.DefaultHint.Multiple", {
      sizes: listFormatter.format(this.configuration.sizes.map(s => CONFIG.NIH.actorSizes[s].label))
    });
  }
  ...
  summaryForLevel(level, { configMode=false }={}) {
    ...
    return sizes.map(s => `<span class="tag">${CONFIG.NIH.actorSizes[s].label}</span>`).join("");
  }
