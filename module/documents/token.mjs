...
export default class TokenDocument5e extends SystemFlagsMixin(TokenDocument) {
  ...
  static getConsumedAttributes(data) {
    return CONFIG.NIH.consumableResources;
  }
  ...
  static getTrackedAttributeChoices(attributes) {
    ...
    const i18n = {
      abilities: game.i18n.localize("NIH.AbilityScorePl"),
      movement: game.i18n.localize("NIH.MOVEMENT.FIELDS.speeds.label"),
      senses: game.i18n.localize("NIH.Senses"),
      skills: game.i18n.localize("NIH.SkillPassives"),
      slots: game.i18n.localize("JOURNALENTRYPAGE.NIH.Class.SpellSlots")
    };
  ...
  prepareData() {
    ...
    if ( !size ) return;
    const dts = CONFIG.NIH.actorSizes[size].dynamicTokenScale ?? 1;
  ...
  static registerMovementActions() {
    ...
    CONFIG.Token.movement.actions.crawl.getCostFunction = token => {
      const noAutomation = game.settings.get("nih", "movementAutomation") === "none";
      const { actor } = token;
  ...
  static getMovementActionCostFunction(type, token, options) {
    ...
    const { actor } = token;
    const actorMovement = actor?.system.attributes?.movement;
    const walkFallback = CONFIG.NIH.movementTypes[type]?.walkFallback;
    const hasMovement = actorMovement !== undefined;
  ...
  getRingColors() {
    ...
    if ( this.hasStatusEffect(CONFIG.specialStatusEffects.DEFEATED) ) {
      colors.ring = CONFIG.NIH.tokenRingColors.defeated;
    }
    return colors;
  }
  ...
  flashRing(type) {
    ...
    const color = CONFIG.NIH.tokenRingColors[type];
    if ( !color ) return;
    const options = {};
    if ( type === "damage" ) {
  ...
  async _preCreate(data, options, user) {
    ...
    if ( (this.actor?.type === "npc") && !this.actorLink
      ...
      const autoRoll = options.nih?.autoRollNPCHP ?? game.settings.get("nih", "autoRollNPCHP");
  ...
  _onDelete(options, userId) {
    ...
    const origin = this.actor?.getFlag("nih", "summon.origin");
    if ( origin ) {
      const { collection, primaryId } = foundry.utils.parseUuid(origin);
      nih.registry.summons.untrack(collection?.get?.(primaryId)?.uuid, this.actor.uuid);
    }
  }
}
