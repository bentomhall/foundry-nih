...
export default class Item5e extends SystemDocumentMixin(Item) {
  ...
  /** @override */
  static DEFAULT_ICON = "systems/nih/icons/svg/documents/item.svg";
  ...
  static compendiumBrowserTypes({ chosen=new Set() }={}) {
    ...
    choices.physical = {
      label: game.i18n.localize("NIH.ITEM.Category.Physical"),
  ...
  _initializeSource(data, options={}) {
     ...
     * @function nih.initializeItemSource
     * @memberof hookEvents
     ...
     */
    if ( options.pack || options.parent?.pack ) Hooks.callAll("nih.initializeItemSource", this, data, options);
   ...
   */
  get canDelete() {
    return !this.flags.nih?.cachedFor;
  }
   ...
   */
  get dependentOrigin() {
    return fromUuidSync(this.flags.nih?.dependentOn, { relative: this, strict: false }) ?? null;
  }
   ...
   */
  get scalingIncrease() {
    return this.system?.scalingIncrease ?? this.getFlag("nih", "scaling") ?? 0;
  }
  ...
  *allApplicableEffects() {
  ...
  prepareDerivedData() {
    ...
    if ( this.system.properties ) {
      this.labels.properties = this.system.properties.reduce((acc, prop) => {
        ...
        acc.push({
          abbr: prop,
          label: CONFIG.NIH.itemProperties[prop]?.label,
          icon: CONFIG.NIH.itemProperties[prop]?.icon
        });
        return acc;
      }, []);
    }
  ...
  async displayCard(message={}) {
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        content: await foundry.applications.handlebars.renderTemplate(
          "systems/nih/templates/chat/item-card.hbs", context
        ),
        flags: {
          "nih.item": { id: this.id, uuid: this.uuid, type: this.type }
        },
     ...
     */
    if ( Hooks.call("nih.preDisplayCard", this, messageConfig) === false ) return;
    if ( Hooks.call("nih.preDisplayCardV2", this, messageConfig) === false ) return;
  ...
  async createActivity(type, data={}, { renderSheet=true }={}) {
    ...
    const config = CONFIG.NIH.activityTypes[type];
    if ( !config ) throw new Error(`${type} not found in CONFIG.NIH.activityTypes`);
  ...
  createAdvancement(type, data={}, { renderSheet=true, showConfig=renderSheet, source=false }={}) {
    ...
    const config = CONFIG.NIH.advancementTypes[type];
    if ( !config ) throw new Error(`${type} not found in CONFIG.NIH.advancementTypes`);
    const cls = config.documentClass;
  ...
  async deleteDialog(options={}) {
    ...
    if ( this.actor?.system.metadata?.supportsAdvancement && !game.settings.get("nih", "disableAdvancements") ) {
    ...
    if ( count ) {
      return Dialog.confirm({
        title: `${game.i18n.format("DOCUMENT.Delete", {type: game.i18n.localize("NIH.Container")})}: ${this.name}`,
        content: `<h4>${game.i18n.localize("AreYouSure")}</h4>
          <p>${game.i18n.format("NIH.ContainerDeleteMessage", {count})}</p>
          <label>
            <input type="checkbox" name="deleteContents">
            ${game.i18n.localize("NIH.ContainerDeleteContents")}
          </label>`,
        yes: html => {
  ...
  static async createWithContents(items, { container, keepId=false, transformAll, transformFirst }={}) {
    ...
    if ( container ) {
      ...
      if ( depth > PhysicalItemTemplate.MAX_DEPTH ) {
        ui.notifications.warn(game.i18n.format("NIH.ContainerMaxDepth", { depth: PhysicalItemTemplate.MAX_DEPTH }));
        return;
      }
    }
  ...
  static async createScrollFromSpell(spell, options={}, config={}) {
    ...
    const values = {};
    if ( (spell instanceof Item5e) && spell.isOwned && (game.settings.get("nih", "rulesVersion") === "modern") ) {
    ...
    config = foundry.utils.mergeObject({
      explanation: game.user.getFlag("nih", "creation.scrollExplanation") ?? "reference",
    ...
    if ( config.dialog !== false ) {
      ...
      await game.user.setFlag("nih", "creation.scrollExplanation", config.explanation);
    }
    ...
    if ( Number.isNumeric(config.level) ) {
      flags.nih ??= {};
      flags.nih.scaling = Math.max(0, config.level - spell.system.level);
      flags.nih.spellLevel = {
        value: config.level,
     ...
     * @function nih.preCreateScrollFromSpell
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("nih.preCreateScrollFromSpell", itemData, options, config) === false ) return;

    let { activities, level, properties, source } = itemData.system;

    // Get scroll data
    let scrollUuid;
    const id = CONFIG.NIH.spellScrollIds[level];
    if ( foundry.data.validators.isValidId(id) ) {
      scrollUuid = game.packs.get(CONFIG.NIH.sourcePacks.ITEMS).index.get(id).uuid;
    } else {
      scrollUuid = id;
    }
    ...
    for ( const level of Array.fromRange(itemData.system.level + 1).reverse() ) {
      const values = CONFIG.NIH.spellScrollValues[level];
      if ( values ) {
     ...
     * @param {SpellScrollConfiguration} config  Configuration options for scroll creation.
     */
    Hooks.callAll("nih.createScrollFromSpell", spell, spellScrollData, config);
  ...
  static async createScrollFromCompendiumSpell(uuid, config={}) {
     ...
     * @function nih.preCreateScrollFromCompendiumSpell
     * @memberof hookEvents
     * @param {Item5e} spell                     Spell to add to the scroll.
     * @param {SpellScrollConfiguration} config  Configuration options for scroll creation.
     * @returns {boolean}                        Explicitly return `false` to prevent the scroll to be created.
     */
    if ( Hooks.call("nih.preCreateScrollFromCompendiumSpell", spell, config) === false ) return;

    // Get scroll data
    let scrollUuid;
    const id = CONFIG.NIH.spellScrollIds[spell.system.level];
    if ( foundry.data.validators.isValidId(id) ) {
      scrollUuid = game.packs.get(CONFIG.NIH.sourcePacks.ITEMS).index.get(id).uuid;
    } else {
      scrollUuid = id;
    ...
    // Create the spell scroll data
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("NIH.SpellScroll")}: ${spell.name}`,
      system: {
    ...
    switch ( config.explanation ) {
      case "full":
        ...
        return [
          ...
          `<h3>${spell.name} (${game.i18n.format("NIH.LevelNumber", { level })})</h3>`,
          isConc ? `<p><em>${game.i18n.localize("NIH.Scroll.RequiresConcentration")}</em></p>` : null,
          spellDescription,
          `<h3>${game.i18n.localize("NIH.Scroll.Details")}</h3>`,
          scrollDetails || scrollIntro
        ].filterJoin("");
      case "reference":
        return [
          "<p><em>",
          CONFIG.NIH.spellLevels[level] ?? level,
          " &Reference[Spell Scroll]",
          isConc ? `, ${game.i18n.localize("NIH.Scroll.RequiresConcentration")}` : null,
          "</em></p>",
          spellDescription
        ].filterJoin("");
    }
  ...
  static getDefaultArtwork(itemData={}) {
    ...
    return { img: CONFIG.NIH.defaultArtwork.Item[type] ?? img };
  }
}
