 ...
 * @param {string} trait  Trait as defined in `CONFIG.NIH.traits`.
 * @returns {object}      Trait categories defined within `CONFIG.NIH`.
 */
export function categories(trait) {
  const traitConfig = CONFIG.NIH.traits[trait];
  return CONFIG.NIH[traitConfig.configKey ?? trait];
}
...
export async function choices(trait, chosen=new Set()) {
  ...
  if ( traitConfig.children ) {
    for ( const [categoryKey, childrenKey] of Object.entries(traitConfig.children) ) {
      const children = CONFIG.NIH[childrenKey];
  ...
  if ( traitConfig.subtypes ) {
    ...
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      if ( CONFIG.NIH[key] ) Object.assign(obj, CONFIG.NIH[key]);
      return obj;
    }, {});
  ...
  // Sort Categories
  if ( traitConfig.sortCategories ) data = nih.utils.sortObjectEntries(data, "label");

  // Sort Children
  for ( const category of Object.values(data) ) {
    if ( !category.children ) continue;
    category.children = nih.utils.sortObjectEntries(category.children, "label");
  }
...
export function traitLabel(trait, count) {
  ...
  return game.i18n.localize(`NIH.Trait${typeCap}Plural.${pluralRule}`);
}
...
export function keyLabel(trait, key) {
  ...
  for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
    if ( CONFIG.NIH[childrenKey]?.[key] ) return CONFIG.NIH[childrenKey]?.[key];
  }

  for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
    if ( !CONFIG.NIH[idsKey]?.[key] ) continue;
    const index = getBaseItem(CONFIG.NIH[idsKey][key], { indexOnly: true });
...
export function choiceLabel(trait, choice) {
  ...
  if ( !choice.pool ) {
    return game.i18n.format("NIH.TraitConfigChooseAny", {
      count: choice.count,
