 ...
 * @param {string} trait   Trait as defined in `CONFIG.NIH.traits`.
 * @returns {object|void}
 */
...
export function actorKeyPath(trait) {
  const traitConfig = CONFIG.NIH.traits[trait];
 ...
 * @param {string} [trait]  Trait as defined in `CONFIG.NIH.traits`, only needed if key isn't prefixed.
 * @returns {string|void}
 */
...
export async function categories(trait) {
  const traitConfig = CONFIG.NIH.traits[trait];
  const config = foundry.utils.deepClone(CONFIG.NIH[traitConfig.configKey ?? trait]);
  ...
  for ( const key of Object.keys(config) ) {
    ...
    if ( traitConfig.children?.[key] ) {
      ...
      for ( const [childKey, value] of Object.entries(CONFIG.NIH[traitConfig.children[key]]) ) {
  ...
  if ( traitConfig.subtypes ) {
    const map = CONFIG.NIH[`${trait}ProficienciesMap`];

    // Merge all ID lists together
    const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
      foundry.utils.mergeObject(obj, CONFIG.NIH[key] ?? {});
      return obj;
    }, {});
...
export function getBaseItemUUID(identifier) {
  ...
  let pack = CONFIG.NIH.sourcePacks.ITEMS;
...
export function traitIndexFields() {
  ...
  for ( const traitConfig of Object.values(CONFIG.NIH.traits) ) {
...
export function traitLabel(trait, count) {
  ...
  if ( !traitConfig ) return game.i18n.localize(`NIH.TraitGenericPlural.${pluralRule}`);
 ...
 * @param {string} [config.trait]   Trait as defined in `CONFIG.NIH.traits` if not using a prefixed key.
...
export function keyLabel(key, config={}) {
  ...
  const traitData = CONFIG.NIH[traitConfig.configKey ?? trait] ?? {};
  ...
  // Wildcards (e.g. "Artisan's Tools", "any Artisan's Tools", "any 2 Artisan's Tools", or "2 other Artisan's Tools")
    ...
    const localization = `NIH.TraitConfigChoose${final ? "Other" : `Any${count ? "Counted" : "Uncounted"}`}`;
    ...
    for ( const childrenKey of Object.values(traitConfig.children ?? {}) ) {
      const childLabel = CONFIG.NIH[childrenKey]?.[lastKey];
      if ( childLabel ) return childLabel;
    }

    // Base item (e.g. "Shortsword")
    for ( const idsKey of traitConfig.subtypes?.ids ?? [] ) {
      let baseItemId = CONFIG.NIH[idsKey]?.[lastKey];
...
export function choiceLabel(choice, { only=false, final=false }={}) {
  ...
  return game.i18n.format("NIH.TraitConfigChooseList", {
    count: choice.count,
...
export function localizedList({ grants=new Set(), choices=[] }) {
  ...
  return game.i18n.format("NIH.TraitConfigChooseWrapper", {
