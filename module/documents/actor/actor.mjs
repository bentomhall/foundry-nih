...
export default class Actor5e extends SystemDocumentMixin(Actor) {
  ...
  /** @override */
  static DEFAULT_ICON = "systems/nih/icons/svg/documents/actor.svg";
   ...
   */
  get coverBonus() {
    const { coverHalf, coverThreeQuarters } = CONFIG.NIH.statusEffects;
   ...
   */
  get isPolymorphed() {
    return this.getFlag("nih", "isPolymorphed") || false;
  }
  ...
  get concentration() {
    ...
    for ( const effect of this.effects ) {
      ...
      const data = effect.getFlag("nih", "item");
  ...
  get summonedCreatures() {
    return nih.registry.summons.creatures(this);
  }
  ...
  _initializeSource(source, options={}) {
    ...
    if ( (source.type === "group") && (source.system?.type?.value === "encounter") ) {
      ...
      foundry.utils.setProperty(source, "flags.nih.persistSourceMigration", true);
    }
    ...
    const art = game.nih.moduleArt.map.get(uuid);
  ...
  /** @inheritDoc */
  prepareData() {
    if ( this.system.modelProvider !== nih ) return super.prepareData();
  ...
  *allApplicableEffects() {
    for ( const effect of super.allApplicableEffects() ) {
      ...
      if ( effect.parent?.getFlag("nih", "riders.effect")?.includes(effect.id) ) continue;
      yield effect;
    }
  }
  ...
  static async fetchExisting(uuid, options={}) {
    ...
    if ( !actor ) throw new Error(game.i18n.format("NIH.ACTOR.Warning.NoActor", { uuid }));
    ...
    const localActor = game.actors.find(a => {
      ...
      return (a.getFlag("nih", "isAutoImported") || a.getFlag("nih", "summonedCopy")) // Back-compat
    ...
    if ( !game.user.can("ACTOR_CREATE") ) throw new Error("NIH.ACTOR.Warning.CreateActor");
  ...
  async getPreferredArtwork() {
    if ( !this._preferredArtwork ) {
      const showTokenPortrait = this.getFlag("nih", "showTokenPortrait") === true;
  ...
  /** @inheritDoc */
  prepareDerivedData() {
    const origin = this.getFlag("nih", "summon.origin");
    if ( origin && this.token?.id ) {
      const { collection, primaryId } = foundry.utils.parseUuid(origin);
      nih.registry.summons.track(collection?.get?.(primaryId)?.uuid, this.uuid);
    }
  }
  ...
  getConcentrationDC(damage) {
    return Math.clamp(
      Math.floor(damage / 2), 10, game.settings.get("nih", "rulesVersion") === "modern" ? 30 : Infinity
    );
  }
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.NIH.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.NIH.CR_EXP_LEVELS[cr] ?? Object.values(CONFIG.NIH.CR_EXP_LEVELS).pop();
  }
   ...
   * @param {string} key      A key in `NIH.conditionEffects`.
   * @returns {boolean}       Whether the actor is affected.
   */
  hasConditionEffect(key) {
    const props = CONFIG.NIH.conditionEffects[key] ?? new Set();
    ...
    const applyExhaustion = (level !== null) && !imms.has("exhaustion")
      && (game.settings.get("nih", "rulesVersion") === "legacy");
  ...
  _prepareSpellcasting() {
    ...
    const progression = Object.values(CONFIG.NIH.spellcasting).reduce((acc, model) => {
    ...
    for ( const [type, model] of Object.entries(CONFIG.NIH.spellcasting) ) {
  ...
  static computeClassProgression(progression, cls, {actor, spellcasting, count=1}={}) {
     ...
     * The actual hook names include the spellcasting type (e.g. `nih.computeLeveledProgression`).
     * @param {object} progression                    Spellcasting progression data. *Will be mutated.*
     ...
     * @function nih.computeSpellcastingProgression
     * @memberof hookEvents
     */
    const allowed = Hooks.call(
      `nih.compute${type.capitalize()}Progression`, progression, actor, cls, spellcasting, count
    );
    const model = CONFIG.NIH.spellcasting[type];
    ...
    if ( model.isSingleLevel ) {
      if ( foundry.utils.getDefiningClass(this, "computePactProgression") !== Actor5e ) {
        foundry.utils.logCompatibilityWarning("Actor5e.computePactProgression is deprecated. Please use "
          + "SpellcastingModel#computeProgression instead.", { since: "Nih 5.1", until: "Nih 5.4" });
    ...
    if ( model.isSingleLevel ) {
      if ( foundry.utils.getDefiningClass(this, "preparePactSlots") !== Actor5e ) {
        foundry.utils.logCompatibilityWarning("Actor5e.preparePactSlots is deprecated. Please use "
          + "SpellcastingModel#prepareSlots instead.", { since: "Nih 5.1", until: "Nih 5.4" });
  ...
  async _preCreate(data, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      const size = CONFIG.NIH.actorSizes[this.system.traits.size || "med"].token ?? 1;
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( "size" in (this.system.traits || {}) ) {
      ...
      if ( newSize && (newSize !== this.system.traits?.size) ) {
        let size = CONFIG.NIH.actorSizes[newSize].token ?? 1;
    ...
    if ( "hp" in (this.system.attributes || {}) ) {
      ...
      foundry.utils.setProperty(options, "nih.hp", { ...this.system.attributes.hp });
    }

    // Record previous exhaustion level.
    if ( Number.isFinite(foundry.utils.getProperty(changed, "system.attributes.exhaustion")) ) {
      foundry.utils.setProperty(options, "nih.originalExhaustion", this.system.attributes.exhaustion);
    }
  }
  ...
  async applyDamage(damages, options={}) {
     ...
     * @function nih.preApplyDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("nih.preApplyDamage", this, amount, updates, options) === false ) return this;
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @function nih.applyDamage
     * @memberof hookEvents
     */
    Hooks.callAll("nih.applyDamage", this, amount, options);
  ...
  calculateDamage(damages, options={}) {
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @returns {boolean}                         Explicitly return `false` to prevent damage application.
     * @function nih.preCalculateDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("nih.preCalculateDamage", this, damages, options) === false ) return false;
    ...
    const hasEffect = (category, type, properties) => {
      ...
      if ( !CONFIG.NIH.damageTypes[type]?.isPhysical || !properties?.size ) return true;
      return !config.bypasses?.intersection(properties)?.size;
    };

    const skipped = type => {
      if ( options.only === "damage" ) return type in CONFIG.NIH.healingTypes;
      if ( options.only === "healing" ) return type in CONFIG.NIH.damageTypes;
      return false;
    };
     ...
     * @param {DamageApplicationOptions} options  Additional damage application options.
     * @returns {boolean}                         Explicitly return `false` to prevent damage application.
     * @function nih.calculateDamage
     * @memberof hookEvents
     */
    if ( Hooks.call("nih.calculateDamage", this, damages, options) === false ) return false;
  ...
  async beginConcentrating(activity, effectData={}) {
     ...
     * @function nih.preBeginConcentrating
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("nih.preBeginConcentrating", this, activity.item, effectData, activity) === false ) return;
     ...
     * @function nih.createConcentrating
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor initiating concentration.
     ...
     * @param {Activity} activity         The activity that triggered the concentration.
     */
    Hooks.callAll("nih.beginConcentrating", this, activity.item, effect, activity);
  ...
  async endConcentration(target) {
     ...
     * @function nih.preEndConcentration
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("nih.preEndConcentration", this, effect) === false) return [];

    await effect.delete();

    /**
     * A hook that is called after a concentration effect is deleted.
     * @function nih.endConcentration
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor ending concentration.
     * @param {ActiveEffect5e} effect     The ActiveEffect that was deleted.
     */
    Hooks.callAll("nih.endConcentration", this, effect);
  ...
  async challengeConcentration({ dc=10, ability=null }={}) {
    ...
    if ( ability in CONFIG.NIH.abilities ) dataset.ability = ability;
    ...
    return ChatMessage.implementation.create({
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/nih/templates/chat/roll-request-card.hbs",
        {
          buttons: [{
  ...
  _isRemarkableAthlete(ability) {
    return (game.settings.get("nih", "rulesVersion") === "legacy") && this.getFlag("nih", "remarkableAthlete")
      && CONFIG.NIH.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  addRollExhaustion(parts, data) {
    if ( (game.settings.get("nih", "rulesVersion") !== "modern") || !this.system.attributes?.exhaustion ) return;
    const amount = this.system.attributes.exhaustion * (CONFIG.NIH.conditionTypes.exhaustion?.reduction?.rolls ?? 0);
    if ( amount ) {
  ...
  static async handleSkillCheckRequest(actor, request, config, { event }={}) {
    const data = {};
    foundry.utils.setProperty(data, "flags.nih.requestResult", { actorUuid: actor.uuid, requestId: request.id });
  ...
  async rollSkill(config={}, dialog={}, message={}) {
    ...
    const skillLabel = CONFIG.NIH.skills[config.skill]?.label ?? "";
    const ability = config.ability ?? this.system.skills[config.skill]?.ability ?? CONFIG.NIH.skills[config.skill]?.ability ?? "";
    const abilityLabel = CONFIG.NIH.abilities[ability]?.label ?? "";
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("NIH.SkillPromptTitle", { skill: skillLabel, ability: abilityLabel }),
          subtitle: this.name
        }
      }
    }, dialog);
  ...
  async rollToolCheck(config={}, dialog={}, message={}) {
    ...
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("NIH.ToolPromptTitle", { tool: toolLabel }),
          subtitle: this.name
        }
      }
    }, dialog);
  ...
  async #rollSkillTool(type, config={}, dialog={}, message={}) {
    ...
    const skillConfig = CONFIG.NIH.skills[config.skill];
    const toolConfig = CONFIG.NIH.tools[config.tool] ?? CONFIG.NIH.vehicleTypes[config.tool];
    ...
    const hostActor = this.isPolymorphed && this.flags?.nih?.transformOptions?.mergeSkills && (type === "skill")
      ? game.actors.get(this.flags.nih?.originalActor) : null;
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      halflingLucky: this.getFlag("nih", "halflingLucky"),
      reliableTalent: (relevant?.value >= 1) && this.getFlag("nih", "reliableTalent")
    }, config);
    ...
    const abilityLabel = CONFIG.NIH.abilities[abilityId]?.label ?? "";
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        ...
        flavor: type === "skill"
          ? game.i18n.format("NIH.SkillPromptTitle", { skill: skillConfig.label, ability: abilityLabel })
          : game.i18n.format("NIH.ToolPromptTitle", { tool: Trait.keyLabel(config.tool, { trait: "tool" }) ?? "" }),
     ...
     * @function nih.rollSkill
     * @function nih.rollToolCheck
     * @memberof hookEvents
     * @param {D20Roll[]} rolls       The resulting rolls.
     * @param {object} data
     * @param {string} data.ability   Ability used as defined in `CONFIG.NIH.abilities`.
     * @param {string} [data.skill]   ID of the skill that was rolled as defined in `CONFIG.NIH.skills`.
     * @param {string} [data.tool]    ID of the tool that was rolled as defined in `CONFIG.NIH.tools`.
    ...
    Hooks.callAll(`nih.roll${name}`, rolls, data);
    Hooks.callAll(`nih.roll${name}V2`, rolls, data);
  ...
  _buildSkillToolConfig(type, hostActor, process, config, formData, index) {
    ...
    const ability = this.system.abilities?.[abilityId];
    const { calculateSkillToolProficiency } = nih.dataModels.actor.CommonTemplate;
  ...
  rollAbility(config={}, dialog={}, message={}) {
    ...
    const label = CONFIG.NIH.abilities[abilityId]?.label ?? "";
    new foundry.applications.api.Dialog({
      window: { title: `${game.i18n.format("NIH.AbilityPromptTitle", { ability: label })}: ${this.name}` },
      position: { width: 400 },
      content: `<p>${game.i18n.format("NIH.AbilityPromptText", { ability: label })}</p>`,
      buttons: [
        {
          action: "test",
          label: game.i18n.localize("NIH.ActionAbil"),
          callback: () => this.rollAbilityCheck(config, dialog, message)
        },
        {
          action: "save",
          label: game.i18n.localize("NIH.ActionSave"),
  ...
  async rollAbilityCheck(config={}, dialog={}, message={}) {
    const abilityLabel = CONFIG.NIH.abilities[config.ability]?.label ?? "";
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("NIH.AbilityPromptTitle", { ability: abilityLabel }),
          subtitle: this.name
        }
      }
    }, dialog);
  ...
  async rollSavingThrow(config={}, dialog={}, message={}) {
    ...
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("NIH.SavePromptTitle", { ability: abilityLabel }),
          subtitle: this.name
        }
      }
    }, dialog);
  ...
  async #rollD20Test(type, config={}, dialog={}, message={}) {
    ...
    const abilityConfig = CONFIG.NIH.abilities[config.ability];
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        ...
        flavor: game.i18n.format(
          `NIH.${type === "check" ? "Ability" : "Save"}PromptTitle`, { ability: abilityConfig?.label ?? "" }
        ),
        speaker: ChatMessage.getSpeaker({ actor: this })
      }
    }, message);
     ...
     * @function nih.rollAbilityCheck
     * @function nih.rollSavingThrow
     * @memberof hookEvents
     * @param {D20Roll[]} rolls       The resulting rolls.
     * @param {object} data
     * @param {string} data.ability   ID of the ability that was rolled as defined in `CONFIG.NIH.abilities`.
     * @param {Actor5e} data.subject  Actor for which the roll has been performed.
     */
    Hooks.callAll(`nih.roll${name}`, rolls, { ability: config.ability, subject: this });
  ...
  async rollDeathSave(config={}, dialog={}, message={}) {
    ...
    if ( (this.system.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3) ) {
      ui.notifications.warn("NIH.DeathSaveUnnecessary", { localize: true });
      return null;
    }
    ...
    if ( this.getFlag("nih", "diamondSoul") ) {
      parts.push("@prof");
    ...
    const messageConfig = foundry.utils.mergeObject({
      data: {
        ...
        flavor: game.i18n.localize("NIH.DeathSavingThrow")
      }
    }, message);
    ...
    if ( roll.total >= (roll.options.target ?? 10) ) {
      ...
      if ( roll.isCritical ) {
        ...
        details.chatString = "NIH.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        details.chatString = "NIH.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        details.chatString = "NIH.DeathSaveFailure";
      }
    }
     ...
     */
    if ( Hooks.call("nih.rollDeathSave", rolls, details) === false ) return returnValue;
    if ( Hooks.call("nih.rollDeathSaveV2", rolls, details) === false ) return returnValue;
     ...
     * @function nih.postRollDeathSave
     * @memberof hookEvents
     * @param {D20Roll[]} rolls                  The resulting rolls.
     * @param {object} data
     ...
     * @param {Actor5e} data.subject             Actor for which the death saving throw has been rolled.
     */
    Hooks.callAll("nih.postRollDeathSave", rolls, { message: resultsMessage, subject: this });
  ...
  async rollConcentration(config={}, dialog={}, message={}) {
    ...
    const rollConfig = foundry.utils.mergeObject({
      ability: (conc.ability in CONFIG.NIH.abilities) ? conc.ability : CONFIG.NIH.defaultAbilities.concentration,
    ...
    const dialogConfig = foundry.utils.mergeObject({
      options: {
        window: {
          title: game.i18n.format("NIH.SavePromptTitle", { ability: game.i18n.localize("NIH.Concentration") })
        }
      }
    }, dialog);
     ...
     * @function nih.rollConcentration
     * @memberof hookEvents
     * @param {D20Roll[]} rolls     The resulting rolls.
     * @param {object} data
     * @param {Actor5e} data.actor  Actor for which the saving throw has been rolled.
     */
    Hooks.callAll("nih.rollConcentration", rolls, { subject: this });
    Hooks.callAll("nih.rollConcentrationV2", rolls, { subject: this });
  ...
  getInitiativeRollConfig(options={}) {
    ...
    const flags = this.flags.nih ?? {};
    const abilityId = init?.ability || CONFIG.NIH.defaultAbilities.initiative;
    const ability = this.system.abilities?.[abilityId];
    ...
    let { parts, data } = CONFIG.Dice.D20Roll.constructParts({
      ...
      abilityCheckBonus: this.system.bonuses?.abilities?.check,
      alert: flags.initiativeAlert && (game.settings.get("nih", "rulesVersion") === "legacy") ? 5 : null
    }, rollData);
     ...
     * @function nih.preConfigureInitiative
     * @memberof hookEvents
     * @param {Actor5e} subject              The Actor that is rolling initiative.
     * @param {D20RollConfiguration} config  Configuration data for the pending roll.
     */
    Hooks.callAll("nih.preConfigureInitiative", this, rollConfig);
  ...
  async rollInitiativeDialog(rollOptions={}) {
    ...
    if ( config.rolls[0].options?.fixed === undefined ) {
      const dialog = { options: { title: game.i18n.localize("NIH.InitiativeRoll") } };
  ...
  async rollInitiative(options={}, rollOptions={}) {
     ...
     * @function nih.preRollInitiative
     * @memberof hookEvents
     * @param {Actor5e} actor  The Actor that is rolling initiative.
     * @param {D20Roll} roll   The initiative roll.
     */
    if ( Hooks.call("nih.preRollInitiative", this, this._cachedInitiativeRoll) === false ) {
  ...
  async rollHitDie(config={}, dialog={}, message={}) {
    ...
    if ( this.type === "npc" ) {
      ...
      if ( !this.system.attributes.hd.value ) {
        ui.notifications.error(game.i18n.format("NIH.HitDiceNPCWarn", {name: this.name}));
        return null;
      }
    }
    ...
    // Otherwise check classes
      ...
      if ( !cls ) {
        ui.notifications.error(game.i18n.format("NIH.HitDiceWarn", {name: this.name, formula: config.denomination}));
        return null;
      }
    }
    const rulesVersion = game.settings.get("nih", "rulesVersion");
    ...
    const flavor = game.i18n.localize("NIH.HitDiceRoll");
    const messageConfig = foundry.utils.mergeObject({
     ...
     * @returns {boolean}                  Explicitly return `false` to prevent updates from being performed.
     */
    if ( Hooks.call("nih.rollHitDie", rolls, { subject: this, updates }) === false ) return returnValue;
    if ( Hooks.call("nih.rollHitDieV2", rolls, { subject: this, updates }) === false ) return returnValue;
     ...
     * @function nih.postRollHitDie
     * @memberof hookEvents
     * @param {BasicRoll[]} rolls     The resulting rolls.
     * @param {object} data
     * @param {Actor5e} data.subject  Actor for which the roll was performed.
     */
    Hooks.callAll("nih.postRollHitDie", rolls, { subject: this });
   ...
   * @see {@link nih.preRollClassHitPoints}
   */
  async rollClassHitPoints(item, { chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("NIH.ADVANCEMENT.HitPoints.Roll", { class: item.name });
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.implementation.getSpeaker({ actor: this }),
      "flags.nih.roll": { type: "hitPoints" }
    };

    /**
     * A hook event that fires before hit points are rolled for a character's class.
     * @function nih.preRollClassHitPoints
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("nih.preRollClassHitPoints", this, item, rollData, messageData);
     ...
     * @function nih.rollClassHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("nih.rollClassHitPoints", this, roll);
   ...
   * @param {boolean} [options.chatMessage=true]  Display the chat message for this roll.
   * @returns {Promise<Roll>}                     The completed roll.
   * @see {@link nih.preRollNPCHitPoints}
   */
  async rollNPCHitPoints({ chatMessage=true }={}) {
    ...
    const flavor = game.i18n.format("NIH.HPFormulaRollMessage");
    const messageData = {
      title: `${flavor}: ${this.name}`,
      flavor,
      speaker: ChatMessage.getSpeaker({ actor: this }),
     ...
     * @function nih.preRollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor            Actor for which the hit points are being rolled.
     * @param {object} rollData
     * @param {string} rollData.formula  The string formula to parse.
     ...
     * @param {object} messageData       The data object to use when creating the message.
     */
    Hooks.callAll("nih.preRollNPCHitPoints", this, rollData, messageData);
     ...
     * @function nih.rollNPCHitPoints
     * @memberof hookEvents
     * @param {Actor5e} actor  Actor for which the hit points have been rolled.
     * @param {Roll} roll      The resulting roll.
     */
    Hooks.callAll("nih.rollNPCHitPoints", this, roll);
  ...
  async shortRest(config={}) {
    ...
    if ( !game.user.isGM && !game.settings.get("nih", "allowRests") && !config.request ) {
      ui.notifications.warn("NIH.REST.Warning.OnlyByRequest", { localize: true, log: false });
      return;
    }

    const clone = this.clone();
    const restConfig = CONFIG.NIH.restTypes.short;
    config = foundry.utils.mergeObject({
      type: "short", dialog: true, chat: true, newDay: false, advanceTime: false, autoHD: false, autoHDThreshold: 3,
      duration: CONFIG.NIH.restTypes.short.duration[game.settings.get("nih", "restVariant")],
     ...
     * @function nih.preShortRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("nih.preShortRest", this, config) === false ) return;
  ...
  async longRest(config={}) {
    ...
    const clone = this.clone();
    const restConfig = CONFIG.NIH.restTypes.long;
    config = foundry.utils.mergeObject({
      type: "long", dialog: true, chat: true, newDay: true, advanceTime: false,
      duration: restConfig.duration[game.settings.get("nih", "restVariant")],
      recoverTemp: restConfig.recoverTemp, recoverTempMax: restConfig.recoverTempMax,
      exhaustionDelta: restConfig.exhaustionDelta
    }, config);
     ...
     * @function nih.preLongRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being started.
     */
    if ( Hooks.call("nih.preLongRest", this, config) === false ) return;
     ...
     * @function nih.longRest
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that is being rested.
     * @param {RestConfiguration} config  Configuration options for the rest.
     * @returns {boolean}                 Explicitly return `false` to prevent the rest from being continued.
     */
    if ( Hooks.call("nih.longRest", this, config) === false ) return;
  ...
  async _rest(config, result={}) {
     ...
     */
    if ( Hooks.call("nih.preRestCompleted", this, result, config) === false ) return result;
     ...
     * @function nih.restCompleted
     * @memberof hookEvents
     * @param {Actor5e} actor             The actor that just completed resting.
     * @param {RestResult} result         Details on the rest completed.
     * @param {RestConfiguration} config  Configuration data for that occurred.
     */
    Hooks.callAll("nih.restCompleted", this, result, config);

    if ( config.advanceBastionTurn && game.user.isGM && game.settings.get("nih", "bastionConfiguration").enabled
      && this.itemTypes.facility.length ) await nih.bastion.advanceAllFacilities(this);
  ...
  async _displayRestResultMessage(config, result) {
    ...
    const typeConfig = CONFIG.NIH.restTypes[config.type] ?? {};

    // Determine the chat message to display
    let message;
    if ( diceRestored && healthRestored ) message = `NIH.REST.${length}.Result.Full`;
    else if ( longRest && !diceRestored && healthRestored ) message = "NIH.REST.Long.Result.HitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "NIH.REST.Long.Result.HitDice";
    else message = `NIH.REST.${length}.Result.Short`;
    ...
    let chatData = {
      content: game.i18n.format(message, {
        name: this.name,
        dice: game.i18n.format(`NIH.HITDICE.Counted.${pr.select(dhd)}`, { number: formatNumber(dhd) }),
        health: game.i18n.format(`NIH.HITPOINTS.Counted.${pr.select(dhp)}`, { number: formatNumber(dhp) })
      }),
    ...
    if ( config.request ) foundry.utils.setProperty(chatData, "flags.nih.requestResult", {
  ...
  createRestFlavor(config, result) {
    ...
    if ( result?.newDay ?? config.newDay ) parts.push(game.i18n.localize("NIH.REST.NewDay.Label").toLowerCase());
  ...
  _getRestHitDiceRecovery({ maxHitDice, fraction, ...config }={}, result={}) {
    const restConfig = CONFIG.NIH.restTypes[config.type];
    if ( !this.system.attributes.hd || !restConfig?.recoverHitDice ) return;
    fraction ??= game.settings.get("nih", "rulesVersion") === "modern" ? 1 : 0.5;
  ...
  _getRestHitPointRecovery({ recoverTemp, recoverTempMax, ...config }={}, result={}) {
    const restConfig = CONFIG.NIH.restTypes[config.type ?? "long"];
    const hp = this.system.attributes?.hp;
  ...
  _prepareMovementAttribution() {
    ...
    const unit = CONFIG.NIH.movementUnits[units]?.formattingUnit;
    const formatValue = value => `<span class="value">${
      unit ? formatLength(value ?? 0, unit, { parts: true })
        : `${value ?? 0} <span class="units">${units}</span>`
    }</span>`;
    return Object.entries(CONFIG.NIH.movementTypes).reduce((html, [k, { label }]) => {
  ...
  async _prepareArmorClassAttribution({ title }={}) {
    ...
    const cfg = CONFIG.NIH.armorClasses[ac.calc];
    const attribution = [];

    if ( ac.calc === "flat" ) {
      attribution.push({
        label: game.i18n.localize("NIH.ArmorClassFlat"),
    ...
    switch ( ac.calc ) {
      ...
      case "natural":
        attribution.push({
          label: game.i18n.localize("NIH.ArmorClassNatural"),
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: ac.flat
        });
        break;
      ...
      default:
        ...
        let label = game.i18n.localize("NIH.PropertyBase");
        if ( armorInFormula ) label = this.armor?.name ?? game.i18n.localize("NIH.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base
    ...
    if ( ac.shield !== 0 ) attribution.push({
      label: this.shield?.name ?? game.i18n.localize("NIH.EquipmentShield"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.shield
    });
    ...
    if ( ac.cover !== 0 ) attribution.push({
      label: game.i18n.localize("NIH.Cover"),
      mode: CONST.ACTIVE_EFFECT_MODES.ADD,
      value: ac.cover
    });
  ...
  getOriginalStats() {
    ...
    if ( this.isPolymorphed ) {
      const transformOptions = this.flags.nih?.transformOptions;
      const original = game.actors?.get(this.flags.nih?.originalActor);
      if ( original ) {
  ...
  async transformInto(source, settings=new TransformationSetting(), options={}) {
    ...
    const allowed = game.settings.get("nih", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      ui.notifications.warn("NIH.TRANSFORM.Warning.NoPermission", { localize: true });
      return null;
    }

    // Get the original Actor data and the new source data
    const o = this.toObject();
    o.flags.nih = o.flags.nih || {};
    o.flags.nih.transformOptions = {
      ...settings.toObject(),
    ...
    if ( settings.keep.has("self") ) {
      ...
      o.name = o.prototypeToken.name = `${o.name} (${game.i18n.localize("NIH.TRANSFORM.Preset.Appearance.Label")})`;
    }
    ...
    if ( !settings.keep.has("self") ) {
      ...
      for ( let k of Object.keys(abilities) ) {
        ...
        const type = CONFIG.NIH.abilities[k]?.type;
      ...
      const spellIdentifiers = settings.spellLists.size ? new Set(
        Array.from(settings.spellLists)
          .map(id => nih.registry.spellLists.forType(id))
      ...
      if ( !settings.keep.has("class") && ("cr" in d.system.details) ) {
        if ( settings.keep.has("hp") ) {
          let profOverride = d.effects.findSplice(e => e._id === staticID("nihTransformProf"));
          if ( !profOverride ) profOverride = new ActiveEffect.implementation({
            _id: staticID("nihTransformProf"),
            name: game.i18n.localize("NIH.Proficiency"),
          ...
          const cls = new nih.dataModels.item.ClassData({ levels: d.system.details.cr });
          d.items.push({
            type: "class",
            name: game.i18n.localize("NIH.TRANSFORM.TemporaryClass"),
            system: cls.toObject()
          });
        }
      }
      ...
      d.effects = d.effects.filter(e => {
        ...
        if ( settings.keep.has("hp") && !settings.keep.has("class") && (e._id === staticID("nihTransformProf")) ) {
          return true;
        }
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.nih.originalActor ) d.flags.nih.originalActor = this.id;
    d.flags.nih.isPolymorphed = true;

    // Gather previous actor data
    const previousActorIds = this.getFlag("nih", "previousActorIds") || [];
    previousActorIds.push(this._id);
    foundry.utils.setProperty(d.flags, "nih.previousActorIds", previousActorIds);
    ...
    if ( this.isToken ) {
      ...
      if ( !this.token.flags.nih?.previousActorData ) {
        ...
        foundry.utils.setProperty(tokenData, "flags.nih.previousActorData", previousActorData);
        foundry.utils.setProperty(tokenData, "flags.nih.previousTokenData", previousTokenData);
      }
     ...
     */
    Hooks.callAll("nih.transformActor", this, source, d, settings, options);
    Hooks.callAll("nih.transformActorV2", this, source, d, settings, options);
    ...
    const updates = tokens.map(t => {
      ...
      const dOriginalActor = foundry.utils.getProperty(d, "flags.nih.originalActor");
      foundry.utils.setProperty(newTokenData, "flags.nih.originalActor", dOriginalActor);
      foundry.utils.setProperty(newTokenData, "flags.nih.isPolymorphed", true);
      if ( !t.document.flags.nih?.previousTokenData ) {
        const previousTokenData = { texture: {} };
        for ( const k of [...tokenPropsFromSource, ...tokenPropsFromSelf, "name"] ) {
        ...
        foundry.utils.setProperty(newTokenData, "flags.nih.previousTokenData", previousTokenData);
      }
      return newTokenData;
    });
  ...
  async revertOriginalForm(options={}) {
    ...
    if ( !this.isOwner ) {
      ui.notifications.warn("NIH.TRANSFORM.Warning.NoOwnership", { localize: true });
      return null;
    }
     ...
     * @function nih.revertOriginalForm
     * @memberof hookEvents
     ...
     */
    Hooks.callAll("nih.revertOriginalForm", this, options);

    const transformOptions = this.getFlag("nih", "transformOptions");
    const previousActorIds = this.getFlag("nih", "previousActorIds") ?? [];
    const isRendered = this.sheet.rendered;

    // Obtain a reference to the original actor
    const original = game.actors.get(this.getFlag("nih", "originalActor"));
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("NIH.TRANSFORM.Warning.OriginalActor", {
          reference: this.getFlag("nih", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = (await baseActor.getTokenDocument()).toObject();
      const actorData = this.token.getFlag("nih", "previousActorData");
      ...
      foundry.utils.mergeObject(tokenUpdate, this.token.getFlag("nih", "previousTokenData"));
      tokenUpdate.sight = prototypeTokenData.sight;
      tokenUpdate.detectionModes = prototypeTokenData.detectionModes;
      delete tokenUpdate.flags.nih.previousActorData;
      delete tokenUpdate.flags.nih.previousTokenData;
    ...
    if ( canvas.ready ) {
      ...
      const tokenUpdates = tokens.map(t => {
        ...
        foundry.utils.mergeObject(update, t.document.getFlag("nih", "previousTokenData"));
        delete update.x;
        delete update.y;
        return update;
      });
    ...
    if ( game.user.isGM ) {
      ...
      const actorUpdates = polymorphedActorIds.filter(id => game.actors.get(id).isOwner).map(p => {
        return { _id: p, "flags.nih": { "-=isPolymorphed": null, "-=previousActorIds": null } };
      });
      await Actor.implementation.updateDocuments(actorUpdates);

      if ( isRendered ) this.sheet?.close();
    }
    if ( !foundry.utils.isEmpty(update) ) await original.update(update, { nih: { concentrationCheck: false } });
  ...
  static addDirectoryContextOptions(app, entryOptions) {
    ...
    entryOptions.push({
      name: "NIH.TRANSFORM.Action.Restore",
      ...
      name: "NIH.Group.Primary.Set",
      icon: '<i class="fa-solid fa-star"></i>',
      callback: li => {
        game.settings.set("nih", "primaryParty", { actor: game.actors.get(li.dataset.documentId ?? li.dataset.entryId) });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
      ...
      name: "NIH.Group.Primary.Remove",
      icon: '<i class="fa-regular fa-star"></i>',
      callback: li => {
        game.settings.set("nih", "primaryParty", { actor: null });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
  ...
  static formatCreatureType(typeData) {
    ...
    } else if ( typeData.value in CONFIG.NIH.creatureTypes ) {
      const code = CONFIG.NIH.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? code.plural : code.label);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("NIH.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.NIH.actorSizes[typeData.swarm].label),
        type: localizedType
      });
    }
  ...
  async _onUpdate(data, options, userId) {
    ...
    if ( isHpUpdate && hp && !options.isRest && !options.isAdvancement ) {
      ...
      if ( Number.isInteger(changes.total) && (changes.total !== 0) ) {
        ...
        if ( !game.settings.get("nih", "disableConcentration") && (userId === game.userId)
          && (options.nih?.concentrationCheck !== false)
         ...
         */
        Hooks.callAll(`nih.${changes.total > 0 ? "heal" : "damage"}Actor`, this, changes, data, userId);
      }
    }
  }
  ...
  _onDelete(options, userId) {
    ...
    for ( const id in this.apps ) {
      ...
      if ( app instanceof nih.applications.actor.GroupActorSheet ) delete this.apps[id];
    }
    ...
    if ( origin ) {
      const { collection, primaryId } = foundry.utils.parseUuid(origin);
      nih.registry.summons.untrack(collection?.get?.(primaryId)?.uuid, this.uuid);
    }
  }
  ...
  _displayTokenEffect(changes) {
    ...
    const fill = CONFIG.NIH.tokenHPColors[key];
  ...
  async _onUpdateExhaustion(data, options) {
    ...
    if ( level < 1 ) return effect?.delete();
      ...
      const originalExhaustion = foundry.utils.getProperty(options, "nih.originalExhaustion");
      return effect.update({ "flags.nih.exhaustionLevel": level }, { nih: { originalExhaustion } });
    } else {
      effect = await ActiveEffect.implementation.fromStatusEffect("exhaustion", { parent: this });
      effect.updateSource({ "flags.nih.exhaustionLevel": level });
  ...
  updateBloodied(options) {
    const hp = this.system.attributes?.hp;
    if ( !hp?.effectiveMax || (game.settings.get("nih", "bloodied") === "none") ) return;

    const effect = this.effects.get(ActiveEffect5e.ID.BLOODIED);
    if ( hp.value > hp.effectiveMax * CONFIG.NIH.bloodied.threshold ) return effect?.delete();
    if ( effect ) return;

    return ActiveEffect.implementation.create({
      _id: ActiveEffect5e.ID.BLOODIED,
      name: game.i18n.localize(CONFIG.NIH.bloodied.name),
      img: CONFIG.NIH.bloodied.img,
      statuses: ["bloodied"]
  ...
  updateEncumbrance(options) {
    ...
    if ( !encumbrance || (game.settings.get("nih", "encumbrance") === "none") ) return;
    const statuses = [];
    const variant = game.settings.get("nih", "encumbrance") === "variant";
    ...
    const effectData = { ...CONFIG.NIH.encumbrance.effects[statuses[0]], statuses };
    if ( effect ) {
      const originalEncumbrance = effect.statuses.first();
      return effect.update(effectData, { nih: { originalEncumbrance } });
    }
  ...
  static getDefaultArtwork(actorData={}) {
    const img = CONFIG.NIH.defaultArtwork.Actor[actorData.type];
  ...
  static computeLeveledProgression(progression, actor, cls, spellcasting, count) {
    foundry.utils.logCompatibilityWarning("Actor5e.computeLeveledProgression is deprecated. Please use "
      + "SpellcastingModel#computeProgression instead.", { since: "Nih 5.1", until: "Nih 5.4" });
    CONFIG.NIH.spellcasting.spell.computeProgression(progression, actor, cls, spellcasting, count);
  }
  ...
  static computePactProgression(progression, actor, cls, spellcasting, count) {
    foundry.utils.logCompatibilityWarning("Actor5e.computePactProgression is deprecated. Please use "
      + "SpellcastingModel#computeProgression instead.", { since: "Nih 5.1", until: "Nih 5.4" });
    CONFIG.NIH.spellcasting.pact.computeProgression(progression, actor, cls, spellcasting, count);
  }
  ...
  static prepareAltSlots(spells, actor, progression, key, table) {
    foundry.utils.logCompatibilityWarning("Actor.prepareAltSlots is deprecated. Please use "
      + "SpellcastingModel#prepareSlots instead.", { since: "Nih 5.1", until: "Nih 5.4" });
    const model = CONFIG.NIH.spellcasting[key];
    if ( !model ) return;
  ...
  static prepareLeveledSlots(spells, actor, progression) {
    foundry.utils.logCompatibilityWarning("Actor.prepareLeveledSlots is deprecated. Please use "
      + "SpellcastingModel#prepareSlots instead.", { since: "Nih 5.1", until: "Nih 5.4" });
    CONFIG.NIH.spellcasting.spell.prepareSlots(spells, actor, progression);
  }
  ...
  static preparePactSlots(spells, actor, progression) {
    foundry.utils.logCompatibilityWarning("Actor.preparePactSlots is deprecated. Please use "
      + "SpellcastingModel#prepareSlots instead.", { since: "Nih 5.1", until: "Nih 5.4" });
    CONFIG.NIH.spellcasting.pact.prepareSlots(spells, actor, progression);
  }
}
