...
export default class Bastion {
   ...
   * @type {string}
   */
  static ATTACK_TEMPLATE = "systems/nih/templates/chat/bastion-attack-summary.hbs";

  /**
   * The template for the chat card summary of a bastion turn.
   * @type {string}
   */
  static TURN_TEMPLATE = "systems/nih/templates/chat/bastion-turn-summary.hbs";
  ...
  async advanceAllBastions() {
    ...
    const { duration } = game.settings.get("nih", "bastionConfiguration");
  ...
  #evaluateEnlargeOrder(facility, updates) {
    ...
    const sizes = Object.entries(CONFIG.NIH.facilities.sizes).sort(([, a], [, b]) => a.value - b.value);
  ...
  async #onClaimGold(message) {
    const results = message.getFlag("nih", "bastion");
    const { gold } = results;
    const actor = message.getAssociatedActor();
    ...
    gold.claimed = true;
    const content = await this.#renderTurnSummary(actor, results);
    return message.update({ content, flags: { nih: { bastion: results } } });
  }
  ...
  async #renderAttackSummary(actor, roll, { damaged, deaths, undefended, resolved }={}) {
    ...
    context.description = game.i18n.format(`NIH.Bastion.Attack.Result.${key}`, { deaths });
    context.roll = await roll.render();
    context.buttons = [];
    if ( !resolved && (deaths || undefended) ) {
      context.buttons.push({
        label: game.i18n.localize("NIH.Bastion.Attack.Automatic"),
    ...
    if ( damaged ) {
      ...
      if ( facility ) context.damaged = game.i18n.format("NIH.Bastion.Attack.Result.Damaged", {
  ...
  async #renderTurnSummary(actor, results) {
    ...
    context.orders = results.orders.map(({ id, order }) => {
      ...
      return {
        ...
        order: CONFIG.NIH.facilities.orders[order]?.label
      };
    });
    context.supplements = [];
    if ( results.gold.value ) {
      context.supplements.push(`
        <strong>${game.i18n.localize("NIH.CurrencyGP")}</strong>
        ${formatNumber(results.gold.value)}
        (${game.i18n.localize(`NIH.Bastion.Gold.${results.gold.claimed ? "Claimed" : "Unclaimed"}`)})
      `);
    }
    context.buttons = [];
    if ( results.gold.value && !results.gold.claimed ) {
      context.buttons.push({
        label: game.i18n.localize("NIH.Bastion.Gold.Claim"),
  ...
  async confirmAdvance() {
    ...
    const proceed = await foundry.applications.api.DialogV2.confirm({
      content: game.i18n.localize("NIH.Bastion.Confirm"),
      rejectClose: false
    });
  ...
  initializeUI() {
    ...
    const { button, enabled } = game.settings.get("nih", "bastionConfiguration");
    ...
    if ( !turnButton ) {
      document.querySelector("#controls, #scene-controls")?.insertAdjacentHTML("afterend", `
        <button type="button" id="bastion-turn" data-action="bastionTurn" class="nih2 faded-ui">
          <i class="fas fa-chess-rook"></i>
          <span>${game.i18n.localize("NIH.Bastion.Action.BastionTurn")}</span>
        </button>
      `);
  ...
  async promptAttack(actor) {
    ...
    if ( !actor ) {
      ui.notifications.warn("NIH.Bastion.Attack.NoActorWarning", { localize: true });
      return;
    }
...
class BastionAttackDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      title: "NIH.Bastion.Attack.Title",
    ...
    buttons: [{
      action: "resolve",
      label: "NIH.Bastion.Attack.Resolve",
      icon: "fas fa-dice",
      default: true
    }]
  };
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/nih/templates/apps/bastion-attack-dialog.hbs"
    }
  };
  ...
  async _prepareContentContext(context, options) {
    ...
    context.formula = {
      field: new StringField({ initial: "", label: "NIH.Formula" }),
      name: "formula"
    };
    return context;
  }
  ...
  static #handleFormSubmission(event, form, formData) {
    ...
    return this.close({ nih: { submitted: true } });
  }
  ...
  /** @override */
  _onClose(options={}) {
    if ( !options.nih?.submitted ) this.#formula = null;
  }
