...
export default function ActivityMixin(Base) {
  class Activity extends DependentDocumentMixin(PseudoDocumentMixin(Base)) {
    ...
    static metadata = Object.freeze({
      name: "Activity",
      label: "DOCUMENT.NIH.Activity",
      sheetClass: ActivitySheet,
      usage: {
        actions: {},
        chatCard: "systems/nih/templates/chat/activity-card.hbs",
    ...
    static localize() {
      ...
      if ( fields.damage?.fields.parts ) {
        localizeSchema(fields.damage.fields.parts.element, ["NIH.DAMAGE.FIELDS.damage.parts"]);
      }
      if ( fields.consumption ) {
        localizeSchema(fields.consumption.fields.targets.element, ["NIH.CONSUMPTION.FIELDS.consumption.targets"]);
      }
      if ( fields.uses ) localizeSchema(fields.uses.fields.recovery.element, ["NIH.USES.FIELDS.uses.recovery"]);
    }
     ...
     */
    get canConfigure() {
      if ( CONFIG.NIH.activityTypes[this.type]?.configurable === false ) return false;
     ...
     */
    get damageFlavor() {
      return game.i18n.localize("NIH.DamageRoll");
    }
     ...
     */
    get dependentOrigin() {
      return this.item.effects.get(this.flags?.nih?.dependentOn) ?? null;
    }
    ...
    get validConsumptionTypes() {
      const types = new Set(Object.keys(CONFIG.NIH.activityConsumptionTypes));
    ...
    async use(usage={}, dialog={}, message={}) {
      ...
      if ( !this.item.isOwner ) {
        ui.notifications.error("NIH.DocumentUseWarn", { localize: true });
        return;
      }
      if ( !this.canUse ) {
        ui.notifications.error("NIH.ACTIVITY.Warning.UsageNotAllowed", { localize: true });
        return;
      }
       ...
       */
      if ( Hooks.call("nih.preUseActivity", activity, usageConfig, dialogConfig, messageConfig) === false ) return;
      ...
      if ( usageConfig.concentration?.begin ) {
        const effect = await item.actor.beginConcentrating(activity, { "flags.nih.scaling": usageConfig.scaling });
        if ( effect ) {
          results.effects ??= [];
          results.effects.push(effect);
          foundry.utils.setProperty(messageConfig.data, "flags.nih.use.concentrationId", effect.id);
        }
       ...
       */
      if ( Hooks.call("nih.postUseActivity", activity, usageConfig, results) === false ) return results;

      // Trigger any primary action provided by this activity
      if ( usageConfig.subsequentActions !== false ) {
        const deltas = results.message?.flags?.nih?.use?.consumed
          ?? results.message?.data?.flags?.nih?.use?.consumed;
        const consumed = this.createConsumedFlag(this.actor, deltas);
        if ( consumed ) item.updateSource({ "flags.nih.consumed": consumed });
    ...
    async consume(usageConfig, messageConfig) {
       ...
       * @function nih.preActivityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       */
      if ( Hooks.call("nih.preActivityConsumption", this, usageConfig, messageConfig) === false ) return false;
       ...
       * performed.
       * @function nih.activityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       * @returns {boolean}  Explicitly return `false` to prevent activity from being activated.
       */
      if ( Hooks.call("nih.activityConsumption", this, usageConfig, messageConfig, updates) === false ) return false;

      const consumed = await this.#applyUsageUpdates(updates);
      if ( !foundry.utils.isEmpty(consumed) ) {
        foundry.utils.setProperty(messageConfig, "data.flags.nih.use.consumed", consumed);
      }
      if ( usageConfig.cause?.activity ) {
        foundry.utils.setProperty(messageConfig, "data.flags.nih.use.cause", usageConfig.cause.activity);
      }

      /**
       * A hook event that fires after an item's resource consumption is calculated and applied.
       * @function nih.postActivityConsumption
       * @memberof hookEvents
       * @param {Activity} activity                           Activity being activated.
       * @param {ActivityUseConfiguration} usageConfig        Configuration data for the activation.
       * @param {ActivityMessageConfiguration} messageConfig  Configuration info for the created chat message.
       ...
       * @returns {boolean}  Explicitly return `false` to prevent activity from being activated.
       */
      if ( Hooks.call("nih.postActivityConsumption", this, usageConfig, messageConfig, updates) === false ) return false;
    ...
    _prepareUsageConfig(config) {
      ...
      if ( config.consume !== false ) {
        const activationConfig = CONFIG.NIH.activityActivationTypes[this.activation.type] ?? {};
      ...
      const levelingFlag = this.item.getFlag("nih", "spellLevel");
      if ( levelingFlag ) {
      ...
      if ( this.requiresConcentration && !game.settings.get("nih", "disableConcentration") ) {
        ...
        if ( limit && (limit <= effects.size) ) config.concentration.end ??= effects.find(e => {
          const data = e.flags.nih?.item?.data ?? {};
    ...
    async _prepareUsageScaling(usageConfig, messageConfig, item) {
      ...
      if ( levelingFlag ) {
        ...
        if ( level ) {
          ...
          foundry.utils.setProperty(messageConfig, "data.flags.nih.use.spellLevel", level);
        }
      }

      if ( usageConfig.scaling ) {
        foundry.utils.setProperty(messageConfig, "data.flags.nih.scaling", usageConfig.scaling);
        if ( usageConfig.scaling !== item.flags.nih?.scaling ) {
          item.actor._embeddedPreparation = true;
          item.updateSource({ "flags.nih.scaling": usageConfig.scaling });
    ...
    async _prepareUsageUpdates(config, { returnErrors=false }={}) {
      ...
      if ( ((config.consume === true) || config.consume.action) && activationConfig?.consume ) {
        ...
        if ( current && !containsConsumption ) {
          let message;
          if ( current.value < 1 ) message = "NIH.ACTIVATION.Warning.NoActions";
          else if ( count > current.value ) message = "NIH.ACTIVATION.Warning.NotEnoughActions";
          if ( message ) {
        ...
        if ( linkedActivity ) {
          ...
          if ( foundry.utils.getType(results) === "Object" ) {
            ...
            if ( updates.delete.includes(linkedActivity.item.id)
              && (this.item.getFlag("nih", "cachedFor") === otherLinkedActivity?.relativeUUID) ) {
      ...
      // Handle spell slot consumption
        ...
        const spellcasting = CONFIG.NIH.spellcasting[this.item.system.method];
        ...
        if ( slotData ) {
          if ( slotData.value ) {
            ...
            const err = new ConsumptionError(game.i18n.format("NIH.SpellCastNoSlots", {
      ...
      if ( config.concentration?.begin ) {
        ...
        if ( config.concentration.end ) {
          ...
          if ( !replacedEffect ) errors.push(
            new ConsumptionError(game.i18n.localize("NIH.ConcentratingMissingItem"))
          );
        }
        ...
        // Cannot begin more concentrations than the limit
          ...
          new ConsumptionError(game.i18n.localize("NIH.ConcentratingLimited"))
        );
      }
    ...
    async _usageChatContext(message) {
      ...
      if ( this.activation.condition ) {
        supplements.push(`<strong>${game.i18n.localize("NIH.Trigger")}</strong> ${this.activation.condition}`);
      }
      if ( data.materials?.value ) {
        supplements.push(`<strong>${game.i18n.localize("NIH.Materials")}</strong> ${data.materials.value}`);
      }
      const buttons = this._usageChatButtons(message);

      // Include spell level in the subtitle.
      if ( this.item.type === "spell" ) {
        const spellLevel = foundry.utils.getProperty(message, "data.flags.nih.use.spellLevel");
        const { spellLevels, spellSchools } = CONFIG.NIH;
    ...
    _finalizeMessageConfig(usageConfig, messageConfig, results) {
      ...
      if ( effects ) foundry.utils.setProperty(messageConfig.data, "flags.nih.use.effects", effects);
    }
    ...
    _usageChatButtons(message) {
      ...
      if ( this.target?.template?.type ) buttons.push({
        label: game.i18n.localize("NIH.TARGET.Action.PlaceTemplate"),
      ...
      if ( message.hasConsumption ) buttons.push({
        label: game.i18n.localize("NIH.CONSUMPTION.Action.ConsumeResource"),
        icon: '<i class="fa-solid fa-cubes-stacked" inert></i>',
        dataset: {
          action: "consumeResource"
        }
      }, {
        label: game.i18n.localize("NIH.CONSUMPTION.Action.RefundResource"),
    ...
    shouldHideChatButton(button, message) {
      const flag = message.getFlag("nih", "use.consumed");
    ...
    async _createUsageMessage(message) {
       ...
       * @function nih.preCreateUsageMessage
       * @memberof hookEvents
       * @param {Activity} activity                     Activity for which the card will be created.
       * @param {ActivityMessageConfiguration} message  Configuration info for the created message.
       */
      Hooks.callAll("nih.preCreateUsageMessage", this, messageConfig);
       ...
       * @function nih.postCreateUsageMessage
       * @memberof hookEvents
       ...
       */
      Hooks.callAll("nih.postCreateUsageMessage", this, card);
    ...
    async rollDamage(config={}, dialog={}, message={}) {
      ...
      if ( canUpdate && !foundry.utils.isEmpty(lastDamageTypes)
        ...
        await this.item.setFlag("nih", `last.${this.id}.damageType`, lastDamageTypes);
      }
       ...
       */
      Hooks.callAll("nih.rollDamage", rolls, { subject: this });
      Hooks.callAll("nih.rollDamageV2", rolls, { subject: this });
    ...
    getContextMenuOptions() {
      ...
      if ( this.item.isOwner && !compendiumLocked ) {
        ...
        entries.push({
          name: "NIH.ContextMenuActionView",
      ...
      if ( "favorites" in (this.actor?.system ?? {}) ) {
        ...
        entries.push({
          name: isFavorited ? "NIH.FavoriteRemove" : "NIH.Favorite",
    ...
    async #onChatAction(event, target, message) {
      const consumed = this.createConsumedFlag(message.getAssociatedActor(), message.getFlag("nih", "use.consumed"));
      const scaling = message.getFlag("nih", "scaling") ?? 0;
      const item = (consumed || scaling) ? this.item.clone({
        "flags.nih": { consumed, scaling }
    ...
    static onContextMenu(item, target) {
       ...
       * @function nih.getItemActivityContext
       * @memberof hookEvents
       ...
       */
      Hooks.callAll("nih.getItemActivityContext", activity, target, menuItems);
    ...
    async #consumeResource(event, target, message) {
      const messageConfig = {};
      const scaling = message.getFlag("nih", "scaling");
      const usageConfig = { consume: true, event, scaling };
      const linkedActivity = this.getLinkedActivity(message.getFlag("nih", "use.cause"));
    ...
    async #refundResource(event, target, message) {
      const consumed = message.getFlag("nih", "use.consumed");
      if ( !foundry.utils.isEmpty(consumed) ) {
        await this.refund(consumed);
        await message.unsetFlag("nih", "use.consumed");
      }
    }
    ...
    async #placeTemplate() {
      ...
      try {
        ...
        Hooks.onError("Activity#placeTemplate", err, {
          msg: game.i18n.localize("NIH.TARGET.Warning.PlaceTemplate"),
          log: "error",
          notify: "error"
        });
      }
    ...
    getLinkedActivity(relativeUUID) {
      ...
      relativeUUID ??= this.item.getFlag("nih", "cachedFor");
    ...
    getRollData(options) {
      ...
      rollData.consumed = this.item.flags.nih?.consumed;
    ...
    static _createDialogTypes(parent) {
      return Object.entries(CONFIG.NIH.activityTypes)
