...
export default class EnchantActivity extends ActivityMixin(BaseEnchantActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "NIH.ENCHANT"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "enchant",
      img: "systems/nih/icons/svg/activity/enchant.svg",
      title: "NIH.ENCHANT.Title",
      hint: "NIH.ENCHANT.Hint",
  ...
  _prepareUsageConfig(config) {
    ...
    const existingProfile = this.existingEnchantment?.flags.nih?.enchantmentProfile;
  ...
  _finalizeMessageConfig(usageConfig, messageConfig, results) {
    ...
    if ( usageConfig.enchantmentProfile ) foundry.utils.setProperty(
      messageConfig, "data.flags.nih.use.enchantmentProfile", usageConfig.enchantmentProfile
    );

    // Don't display message if just auto-disabling existing enchantment
    if ( this.existingEnchantment?.flags.nih?.enchantmentProfile === usageConfig.enchantmentProfile ) {
  ...
  async _triggerSubsequentActions(config, results) {
    ...
    if ( !existingEnchantment || (existingEnchantment.flags.nih?.enchantmentProfile !== config.enchantmentProfile) ) {
  ...
  async applyEnchantment(profile, item, { chatMessage, concentration, strict=true }={}) {
    ...
    if ( !game.user.isGM && concentration && !concentration.isOwner ) {
      if ( strict ) {
        ui.notifications.error("NIH.EffectApplyWarningConcentration", { console: false, localize: true });
        return null;
      } else {
        concentration = null;
      }
    ...
    const enchantmentData = effect.clone({ origin: this.uuid, "flags.nih": flags }).toObject();

    /**
     * Hook that fires before an enchantment is applied to an item.
     * @function nih.preApplyEnchantment
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("nih.preApplyEnchantment", item, enchantmentData, { activity: this }) === false ) return null;
    ...
    if ( item.inCompendium ) {
      ...
      if ( !actor ) {
        ui.notifications.warn("NIH.ENCHANT.Warning.NoTargetActor", { localize: true });
        return null;
      }
      enchantmentData._id = foundry.utils.randomID();
      const toCreate = await Item5e.createWithContents([item], {
        transformAll: item => item.clone({ "flags.nih.dependentOn": `.ActiveEffect.${enchantmentData._id}` })
      });
     ...
     * @param {Activity} options.activity   Enchant activity applied the enchantment.
     */
    Hooks.callAll("nih.applyEnchantment", item, enchantment, { activity: this });
  ...
  canEnchant(item) {
    ...
    if ( !this.restrictions.allowMagical && item.system.properties?.has("mgc")
      ...
      errors.push(new EnchantmentError(game.i18n.localize("NIH.ENCHANT.Warning.NoMagicalItems")));
    }

    if ( this.restrictions.type && (item.type !== this.restrictions.type) ) {
      errors.push(new EnchantmentError(game.i18n.format("NIH.ENCHANT.Warning.WrongType", {
    ...
    if ( this.restrictions.categories.size && !this.restrictions.categories.has(item.system.type?.value) ) {
      ...
      errors.push(new EnchantmentError(game.i18n.format(
        `NIH.ENCHANT.Warning.${item.system.type?.value ? "WrongType" : "NoSubtype"}`,
        {
    ...
    if ( this.restrictions.properties.size
      ...
      errors.push(new EnchantmentError(game.i18n.format("NIH.Enchantment.Warning.MissingProperty", {
        validProperties: game.i18n.getListFormatter({ type: "disjunction" }).format(
          Array.from(this.restrictions.properties).map(p => CONFIG.NIH.itemProperties[p]?.label ?? p)
        )
      })));
    }
     ...
     */
    Hooks.callAll("nih.canEnchant", this, item, errors);
