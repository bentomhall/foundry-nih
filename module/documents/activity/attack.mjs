...
export default class AttackActivity extends ActivityMixin(BaseAttackActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "NIH.ATTACK"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "attack",
      img: "systems/nih/icons/svg/activity/attack.svg",
      title: "NIH.ATTACK.Title.one",
      hint: "NIH.ATTACK.Hint",
  ...
  _usageChatButtons(message) {
    const buttons = [{
      label: game.i18n.localize("NIH.Attack"),
      icon: '<i class="nih-icon" data-src="systems/nih/icons/svg/trait-weapon-proficiencies.svg" inert></i>',
      dataset: {
        action: "rollAttack"
      }
    }];
  ...
  async _triggerSubsequentActions(config, results) {
    this.rollAttack({ event: config.event }, {}, { data: { "flags.nih.originatingMessage": results.message?.id } });
  }
  ...
  async rollAttack(config={}, dialog={}, message={}) {
    ...
    if ( (this.item.type === "weapon") && (this.item.system.quantity === 0) ) {
      ui.notifications.warn("NIH.ATTACK.Warning.NoQuantity", { localize: true });
    }

    const buildConfig = this._buildAttackConfig.bind(this);

    const rollConfig = foundry.utils.mergeObject({
      ammunition: this.item.getFlag("nih", `last.${this.id}.ammunition`),
      attackMode: this.item.getFlag("nih", `last.${this.id}.attackMode`),
      elvenAccuracy: this.actor?.getFlag("nih", "elvenAccuracy")
        && CONFIG.NIH.characterFlags.elvenAccuracy.abilities.includes(this.ability),
      halflingLucky: this.actor?.getFlag("nih", "halflingLucky"),
      mastery: this.item.getFlag("nih", `last.${this.id}.mastery`),
    ...
    const dialogConfig = foundry.utils.mergeObject({
      ...
      options: {
        ...
        window: {
          title: game.i18n.localize("NIH.AttackRoll"),
    ...
    const messageConfig = foundry.utils.mergeObject({
      create: true,
      data: {
        flavor: `${this.item.name} - ${game.i18n.localize("NIH.AttackRoll")}`,
        flags: {
          nih: {
            ...this.messageFlags,
            messageType: "roll",
    ...
    for ( const key of ["ammunition", "attackMode", "mastery"] ) {
      ...
      foundry.utils.setProperty(messageConfig.data, `flags.nih.roll.${key}`, rolls[0].options[key]);
    }
    ...
    if ( canUpdate && !foundry.utils.isEmpty(flags) && (this.actor && this.actor.items.has(this.item.id)) ) {
      await this.item.setFlag("nih", `last.${this.id}`, flags);
    }
     ...
     */
    Hooks.callAll("nih.rollAttack", rolls, { subject: this, ammoUpdate });
    Hooks.callAll("nih.rollAttackV2", rolls, { subject: this, ammoUpdate });
    ...
    if ( canUpdate && ammoUpdate?.destroy ) {
      ...
      const messageId = messageConfig.data?.flags?.nih?.originatingMessage
        ?? rollConfig.event?.target.closest("[data-message-id]")?.dataset.messageId;
      const attackMessage = nih.registry.messages.get(messageId, "attack")?.pop();
      await attackMessage?.setFlag("nih", "roll.ammunitionData", data);
     ...
     * @function nih.postRollAttack
     * @memberof hookEvents
     * @param {D20Roll[]} rolls                   The resulting rolls.
     * @param {object} data
     * @param {AttackActivity|null} data.subject  The activity that performed the attack.
     */
    Hooks.callAll("nih.postRollAttack", rolls, { subject: this });
  ...
  static #rollDamage(event, target, message) {
    ...
    const attackMode = lastAttack?.getFlag("nih", "roll.attackMode");

    // Fetch the ammunition used with the last attack roll
    let ammunition;
    const actor = lastAttack?.getAssociatedActor();
    if ( actor ) {
      const storedData = lastAttack.getFlag("nih", "roll.ammunitionData");
      ammunition = storedData
        ? new Item.implementation(storedData, { parent: actor })
        : actor.items.get(lastAttack.getFlag("nih", "roll.ammunition"));
    }
