...
export default class CheckActivity extends ActivityMixin(BaseCheckActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "NIH.CHECK"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "check",
      img: "systems/nih/icons/svg/activity/check.svg",
      title: "NIH.CHECK.Title",
      hint: "NIH.CHECK.Hint",
  ...
  _usageChatButtons(message) {
    ...
    const createButton = (abilityKey, associated) => {
      const ability = CONFIG.NIH.abilities[abilityKey]?.label;
      const checkType = (associated in CONFIG.NIH.skills) ? "skill"
        : (associated in CONFIG.NIH.tools) ? "tool": "ability";
      ...
      let label = ability;
      let type;
      if ( checkType === "skill" ) type = CONFIG.NIH.skills[associated]?.label;
      else if ( checkType === "tool" ) type = Trait.keyLabel(associated, { trait: "tool" });
      if ( type ) label = game.i18n.format("EDITOR.NIH.Inline.SpecificCheck", { ability, type });
      else label = ability;

      buttons.push({
        label: dc ? `
          <span class="visible-dc">${game.i18n.format("EDITOR.NIH.Inline.DC", { dc, check: wrap(label) })}</span>
          <span class="hidden-dc">${wrap(label)}</span>
        ` : wrap(label),
        icon: checkType === "tool" ? '<i class="fa-solid fa-hammer" inert></i>'
          : '<i class="nih-icon" data-src="systems/nih/icons/svg/ability-score-improvement.svg" inert></i>',
        dataset
      });
    };
    const wrap = check => game.i18n.format("EDITOR.NIH.Inline.CheckShort", { check });
  ...
  static async #rollCheck(event, target, message) {
    ...
    if ( ability in CONFIG.NIH.abilities ) rollData.ability = ability;
