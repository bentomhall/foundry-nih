...
export default class SummonActivity extends ActivityMixin(BaseSummonActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "NIH.SUMMON"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "summon",
      img: "systems/nih/icons/svg/activity/summon.svg",
      title: "NIH.SUMMON.Title",
      hint: "NIH.SUMMON.Hint",
   ...
   */
  get canSummon() {
    return game.user.can("TOKEN_CREATE") && (game.user.isGM || game.settings.get("nih", "allowSummoning"));
  }
  ...
  _finalizeMessageConfig(usageConfig, messageConfig, results) {
    ...
    delete messageConfig.data.flags?.nih?.use?.effects;
  }
  ...
  _usageChatButtons(message) {
    ...
    return [{
      label: game.i18n.localize("NIH.SUMMON.Action.Summon"),
  ...
  async placeSummons(options) {
    ...
    if ( !profile ) throw new Error(
      game.i18n.format("NIH.SUMMON.Warning.NoProfile", { profileId: options.profile, item: this.item.name })
    );

    /**
     * A hook event that fires before summoning is performed.
     * @function nih.preSummon
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("nih.preSummon", this, profile, options) === false ) return;
    ...
    const actor = await nih.documents.Actor5e.fetchExisting(summonUuid, {
      origin: { key: "flags.nih.summon.origin", value: this.item?.uuid }
    });

    // Verify ownership of actor
    if ( !actor.isOwner ) {
      throw new Error(game.i18n.format("NIH.SUMMON.Warning.NoOwnership", { actor: actor.name }));
    }
    ...
    try {
      ...
      for ( const placement of placements ) {
         ...
         */
        if ( Hooks.call("nih.preSummonToken", this, profile, tokenUpdateData, options) === false ) continue;
         ...
         * @param {SummoningConfiguration} options  Additional summoning options.
         */
        Hooks.callAll("nih.summonToken", this, profile, tokenData, options);
     ...
     * @param {SummoningConfiguration} options  Additional summoning options.
     */
    Hooks.callAll("nih.postSummon", this, profile, createdTokens, options);
  ...
  async getChanges(actor, profile, options) {
    ...
    if ( this.match.proficiency ) {
      const proficiencyEffect = new ActiveEffect({
        _id: staticID("nihMatchProficiency"),
        changes: [{
        ...
        name: game.i18n.localize("NIH.SUMMON.FIELDS.match.proficiency.label")
      });
      ...
      if ( acBonus.total ) {
        if ( actor.system.attributes.ac.calc === "flat" ) {
          ...
          actorUpdates.effects.push((new ActiveEffect({
            ...
            name: game.i18n.localize("NIH.SUMMON.FIELDS.bonuses.ac.label")
          })).toObject());
        }
      }
    }
    ...
    if ( this.bonuses.hd && (actor.type === "npc") ) {
      ...
      if ( hdBonus.total ) {
        actorUpdates.effects.push((new ActiveEffect({
          ...
          name: game.i18n.localize("NIH.SUMMON.FIELDS.bonuses.hd.label")
        })).toObject());
      }
    }
      ...
      if ( hpBonus.total ) {
        ...
        const maxHpEffect = hpField => {
          return (new ActiveEffect({
            ...
            name: game.i18n.localize("NIH.SUMMON.FIELDS.bonuses.hp.label")
          })).toObject();
        };
    ...
    if ( this.creatureSizes.size ) {
      ...
      const config = CONFIG.NIH.actorSizes[size];
      if ( config ) {
    ...
    for ( const item of actor.items ) {
      ...
      if ( changes.length ) {
        const effect = (new ActiveEffect({
          _id: staticID("nihItemChanges"),
          changes,
          disabled: false,
          icon: "icons/skills/melee/strike-slashes-orange.webp",
          name: game.i18n.localize("NIH.SUMMON.ItemChanges.Label"),
          origin: this.uuid,
          type: "enchantment"
        })).toObject();
  ...
  async getTokenData({ actor, placement, tokenUpdates, actorUpdates }) {
    if ( actor.prototypeToken.randomImg && !game.user.can("FILES_BROWSE") ) {
      ...
      ui.notifications.warn("NIH.SUMMON.Warning.Wildcard", { localize: true });
    }
      ...
      try {
        await SummonUsageDialog.create(this, config, {
          button: {
            ...
            label: "NIH.SUMMON.Action.Summon"
          },
          display: {
            all: false,
            create: { summons: true }
  ...
  fetchActor(uuid) {
    foundry.utils.logCompatibilityWarning("SummonActivity#fetchActor is deprecated. "
      + "Please use Actor5e.fetchExisting instead.", { since: "Nih 5.1", until: "Nih 5.3" });
    return nih.documents.Actor5e.fetchExisting(uuid, {
      origin: { key: "flags.nih.summon.origin", value: this.item?.uuid }
    });
  }
}
