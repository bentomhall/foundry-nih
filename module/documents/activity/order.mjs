...
export default class OrderActivity extends ActivityMixin(BaseOrderActivityData) {
  ...
  static metadata = Object.freeze(foundry.utils.mergeObject(super.metadata, {
    type: "order",
    img: "systems/nih/icons/svg/activity/order.svg",
    title: "NIH.FACILITY.Order.Issue",
    usage: {
      actions: {
        pay: OrderActivity.#onPayOrder
      },
      chatCard: "systems/nih/templates/chat/order-activity-card.hbs",
  ...
  _prepareUsageScaling(usageConfig, messageConfig, item) {
    ...
    messageConfig.data.flags.nih.order = { costs, craft, trade };
  }
  ...
  _usageChatButtons(message) {
    const { costs } = message.data.flags.nih.order;
    if ( !costs.gold || costs.paid ) return [];
    return [{
      label: game.i18n.localize("NIH.FACILITY.Costs.Automatic"),
      icon: '<i class="fas fa-coins"></i>',
      dataset: { action: "pay", method: "automatic" }
    }, {
      label: game.i18n.localize("NIH.FACILITY.Costs.Manual"),
  ...
  async _usageChatContext(message) {
    const { costs, craft, trade } = message.data.flags.nih.order;
    const { type } = this.item.system;
    const supplements = [];
    if ( costs.days ) supplements.push(`
      <strong>${game.i18n.localize("NIH.DurationTime")}</strong>
      ${game.i18n.format("NIH.FACILITY.Costs.Days", { days: costs.days })}
    `);
    if ( costs.gold ) supplements.push(`
      <strong>${game.i18n.localize("NIH.CurrencyGP")}</strong>
      ${formatNumber(costs.gold)}
      (${game.i18n.localize(`NIH.FACILITY.Costs.${costs.paid ? "Paid" : "Unpaid"}`)})
    `);
    if ( craft?.item ) {
    ...
    if ( trade?.stock?.value && trade.sell ) supplements.push(`
      <strong>${game.i18n.localize("NIH.FACILITY.Trade.Sell.Supplement")}</strong>
      ${formatNumber(trade.stock.value)}
      ${CONFIG.NIH.currencies.gp?.abbreviation ?? ""}
    `);
    if ( trade?.creatures ) {
      const creatures = [];
      if ( trade.sell ) {
      ...
      supplements.push(`
        <strong>${game.i18n.localize(`NIH.FACILITY.Trade.${trade.sell ? "Sell" : "Buy"}.Supplement`)}</strong>
        ${game.i18n.getListFormatter({ style: "narrow" }).format(creatures.map(a => a.toAnchor().outerHTML))}
      `);
    }
    const facilityType = game.i18n.localize(`NIH.FACILITY.Types.${type.value.titleCase()}.Label.one`);
    const buttons = this._usageChatButtons(message);
    return {
      supplements,
      buttons: buttons.length ? buttons : null,
      description: game.i18n.format("NIH.FACILITY.Use.Description", {
        order: game.i18n.localize(`NIH.FACILITY.Orders.${this.order}.inf`),
  ...
  static async #onPayOrder(event, target, message) {
    ...
    const order = message.getFlag("nih", "order");
    const config = foundry.utils.expandObject({ "data.flags.nih.order": order });
    if ( method === "automatic" ) {
      try {
    ...
    foundry.utils.setProperty(config, "data.flags.nih.order.costs.paid", true);
