...
export default class TransformActivity extends ActivityMixin(BaseTransformActivityData) {
  ...
  /** @inheritDoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "NIH.TRANSFORM"];
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "transform",
      img: "systems/nih/icons/svg/activity/transform.svg",
      title: "NIH.TRANSFORM.Title",
      hint: "NIH.TRANSFORM.Hint",
   ...
   */
  get canTransform() {
    return game.user.can("ACTOR_CREATE") && (game.user.isGM || game.settings.get("nih", "allowPolymorphing"));
  }
  ...
  async _finalizeMessageConfig(usageConfig, messageConfig, results) {
    ...
    if ( usageConfig.transform?.profile ) {
      foundry.utils.setProperty(messageConfig.data, "flags.nih.transform.profile", usageConfig.transform.profile);
    }
  }
  ...
  _usageChatButtons(message) {
    ...
    return [{
      label: game.i18n.localize("NIH.TRANSFORM.Action.Transform"),
  ...
  async _finalizeUsage(config, results) {
    ...
    if ( profile ) {
      ...
      if ( uuid ) {
        if ( results.message instanceof ChatMessage ) results.message.setFlag("nih", "transform.uuid", uuid);
        else foundry.utils.setProperty(results.message, "flags.nih.transform.uuid", uuid);
      }
    }
  ...
  static async #transformActor(event, target, message) {
    ...
    if ( !targets.length ) {
      ui.notifications.warn("NIH.ActionWarningNoToken", { localize: true });
      return;
    }

    const profileId = message.getFlag("nih", "transform.profile");
    const profile = this.profiles.find(p => p._id === profileId) || this.profiles[0];
    const uuid = message.getFlag("nih", "transform.uuid") ?? await this.queryActor(profile);
    const source = await fromUuid(uuid);
    if ( !source ) {
      ui.notifications.warn("NIH.TRANSFORM.Warning.SourceActor", { localize: true });
      return;
    }
