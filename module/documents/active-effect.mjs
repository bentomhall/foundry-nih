...
export default class ActiveEffect5e extends DependentDocumentMixin(ActiveEffect) {
   ...
   */
  static ID = {
    BLOODIED: staticID("nihbloodied"),
    ENCUMBERED: staticID("nihencumbered"),
    EXHAUSTION: staticID("nihexhaustion")
  };
  ...
  /** @inheritdoc */
  static LOCALIZATION_PREFIXES = [...super.LOCALIZATION_PREFIXES, "NIH.ACTIVEEFFECT"];
  ...
  get dependentOrigin() {
    ...
    return this.parent.effects.get(this.flags.nih?.dependentOn) ?? null;
  }
  ...
  get isConcealed() {
    ...
    if ( (this.id === this.constructor.ID.BLOODIED) && (game.settings.get("nih", "bloodied") === "player") ) {
  ...
  get isSuppressed() {
    ...
    if ( this.parent instanceof nih.documents.Item5e ) {
   ...
   */
  async getSource() {
    if ( (this.target instanceof nih.documents.Actor5e) && (this.parent instanceof nih.documents.Item5e) ) {
      return this.parent;
    }
  ...
  _initializeSource(data, options={}) {
    ...
    if ( data.flags?.nih?.type === "enchantment" ) {
      data.type = "enchantment";
      delete data.flags.nih.type;
      foundry.utils.setProperty(data, "flags.nih.persistSourceMigration", true);
    }
  ...
  static migrateData(data) {
    ...
    for ( const change of data.changes ?? [] ) {
      if ( change.key === "flags.nih.initiativeAdv" ) {
  ...
  apply(doc, change) {
    ...
    if ( change.key.startsWith("flags.nih.") ) change = this._prepareFlagChange(doc, change);
  ...
  _prepareFlagChange(actor, change) {
    ...
    const data = CONFIG.NIH.characterFlags[key.replace("flags.nih.", "")];
  ...
  prepareDerivedData() {
    ...
    if ( this.isAppliedEnchantment && this.uuid ) nih.registry.enchantments.track(this.origin, this.uuid);
  }
  ...
  _prepareExhaustionLevel() {
    const config = CONFIG.NIH.conditionTypes.exhaustion;
    let level = this.getFlag("nih", "exhaustionLevel");
    if ( !Number.isFinite(level) ) level = 1;
    this.img = this.constructor._getExhaustionImage(level);
    this.name = `${game.i18n.localize("NIH.Exhaustion")} ${level}`;
    if ( level >= config.levels ) {
      this.statuses.add("dead");
      CONFIG.NIH.statusEffects.dead.statuses?.forEach(s => this.statuses.add(s));
    }
  }
  ...
  async createRiderConditions() {
    ...
    for ( const status of this.getFlag("nih", "riders.statuses") ?? [] ) {
      riders.add(status);
    }
    ...
    const createRider = async id => {
      const existing = this.parent.effects.get(staticID(`nih${id}`));
      if ( existing ) return;
  ...
  async createRiderEnchantments(options={}) {
    ...
    for ( const id of profile.riders.activity ) {
      ...
      foundry.utils.setProperty(activityData, "flags.nih.dependentOn", this.id);
    ...
    riderEffects.push(...profile.riders.effect.map(id => {
      ...
      if ( effectData ) {
        ...
        delete effectData.flags?.nih?.rider;
        effectData.origin = this.origin;
      }
      return effectData;
    }));
    riderEffects = riderEffects.filter(_ => _);
    riderEffects.forEach(e => foundry.utils.setProperty(e, "flags.nih.dependentOn", this.id));
      ...
      const riderItems = await Item5e.createWithContents(
        (await Promise.all(profile.riders.item.map(uuid => fromUuid(uuid)))).filter(_ => _), {
          transformAll: item => {
            ...
            foundry.utils.setProperty(itemData, "flags.nih.dependentOn", this.uuid);
            foundry.utils.setProperty(itemData, "flags.nih.enchantment.origin", this.uuid);
            return itemData;
          }
        }
      );
  ...
  async _preCreate(data, options, user) {
    ...
    if ( (this.type === "enchantment") && (this.parent instanceof Actor) ) {
      ui.notifications.error("NIH.ENCHANTMENT.Warning.NotOnActor", { localize: true });
      return false;
    }
  ...
  _onUpdate(data, options, userId) {
    ...
    const originalLevel = foundry.utils.getProperty(options, "nih.originalExhaustion");
    const newLevel = foundry.utils.getProperty(data, "flags.nih.exhaustionLevel");
    const originalEncumbrance = foundry.utils.getProperty(options, "nih.originalEncumbrance");
    ...
    // Display proper scrolling status effects for encumbrance
      ...
      if ( !increase ) this.name = CONFIG.NIH.encumbrance.effects[originalEncumbrance].name;
  ...
  async _preDelete(options, user) {
    ...
    if ( dependents.length && !game.users.activeGM ) {
      ui.notifications.warn("NIH.ConcentrationBreakWarning", { localize: true });
      return false;
    }
  ...
  _onDelete(options, userId) {
    ...
    if ( this.isAppliedEnchantment ) nih.registry.enchantments.untrack(this.origin, this.uuid);
  ...
  static createConcentrationEffectData(activity, data={}) {
    ...
    const effectData = foundry.utils.mergeObject({
      ...statusEffect,
      name: `${game.i18n.localize("EFFECT.NIH.StatusConcentrating")}: ${item.name}`,
      description: `<p>${game.i18n.format("NIH.ConcentratingOn", {
        name: item.name,
    ...
    if ( item.type === "spell" ) effectData["flags.nih.spellLevel"] = item.system.level;
  ...
  static onRenderActiveEffectConfig(app, html, context) {
    const element = new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {}).toFormGroup({
      label: game.i18n.localize("NIH.CONDITIONS.RiderConditions.label"),
      hint: game.i18n.localize("NIH.CONDITIONS.RiderConditions.hint")
    }, {
      name: "flags.nih.riders.statuses",
      value: app.document.getFlag("nih", "riders.statuses") ?? [],
    ...
    const tooltipText = game.i18n.format("NIH.ACTIVEEFFECT.AttributeKeyTooltip", {
      url: app.document.type === "enchantment"
        ? "https://github.com/foundryvtt/nih/wiki/Enchantment"
        : "https://github.com/foundryvtt/nih/wiki/Active-Effect-Guide"
    });
  ...
  static _getExhaustionImage(level) {
    const { img } = CONFIG.NIH.conditionTypes.exhaustion;
  ...
  static _manageConcentration(event, actor) {
    ...
    const choices = effects.reduce((acc, effect) => {
      ...
      acc[effect.id] = data?.name ?? actor.items.get(data?.id)?.name ?? game.i18n.localize("NIH.ConcentratingItemless");
      return acc;
    }, {});
    const options = HandlebarsHelpers.selectOptions(choices, { hash: { sort: true } });
    const content = `
    <p>${game.i18n.localize("NIH.ConcentratingEndChoice")}</p>
    <div class="form-group">
      <label>${game.i18n.localize("NIH.SOURCE.FIELDS.source.label")}</label>
    ...
    foundry.applications.api.Dialog.prompt({
      content,
      window: { title: game.i18n.localize("NIH.Concentration") },
      ok: {
        label: game.i18n.localize("NIH.Confirm"),
  ...
  addDependent(...dependent) {
    ...
    return Promise.all(dependent.map(d => d.setFlag("nih", "dependentOn", this.uuid))).then(() => this);
  }
  ...
  getDependents() {
    ...
    return (this.getFlag("nih", "dependents") || []).reduce((arr, { uuid }) => {
      let doc;
      ...
      return arr;
    }, []).concat(nih.registry.dependents.get(this));
  }
  ...
  async richTooltip(enrichmentOptions={}) {
    const properties = [];
    if ( this.isSuppressed ) properties.push("NIH.EffectType.Unavailable");
    else if ( this.disabled ) properties.push("NIH.EffectType.Inactive");
    else if ( this.isTemporary ) properties.push("NIH.EffectType.Temporary");
    else properties.push("NIH.EffectType.Passive");
    if ( this.type === "enchantment" ) properties.push("NIH.ENCHANTMENT.Label");

    return {
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/nih/templates/effects/parts/effect-tooltip.hbs", {
          effect: this,
      ...
      classes: ["nih2", "nih-tooltip", "effect-tooltip", "themed", "theme-light"]
    };
  }
