...
export default class SpellConfigurationData extends foundry.abstract.DataModel {
  ...
  applySpellChanges(itemData, { ability }={}) {
    ...
    if ( this.method ) {
      ...
      const model = CONFIG.NIH.spellcasting[this.method];
    ...
    if ( this.uses.max && this.uses.per ) {
      ...
      const spellcasting = CONFIG.NIH.spellcasting[itemData.system.method];
        ...
        if ( createForwardActivity ) {
          const newActivity = {
            ...
            name: `${activity.name ?? game.i18n.localize(
              CONFIG.NIH.activityTypes[activity.type]?.documentClass.metadata.title
            )} (${game.i18n.localize("NIH.ADVANCEMENT.SPELLCONFIG.FreeCasting").toLowerCase()})`,
