...
export default class ItemDataModel extends SystemDataModel {
   ...
   * @type {string}
   */
  static ITEM_TOOLTIP_TEMPLATE = "systems/nih/templates/items/parts/item-tooltip.hbs";
  ...
  get advancementRootItem() {
    return this.parent?.actor?.items.get(this.parent.getFlag("nih", "advancementRoot")?.split(".")?.[0]);
  }
  ...
  prepareBaseData() {
    if ( this.parent.isEmbedded && this.parent.actor?.items.has(this.parent.id) ) {
      ...
      const sourceId = this.parent._stats.compendiumSource ?? this.parent.flags.nih?.sourceId;
  ...
  async richTooltip(enrichmentOptions={}) {
    return {
      ...
      classes: ["nih2", "nih-tooltip", "item-tooltip", "themed", "theme-light"]
    };
  }
  ...
  async getCardData({ activity, ...enrichmentOptions }={}) {
    ...
    const context = {
      ...
      config: CONFIG.NIH,
      controlHints: game.settings.get("nih", "controlHints"),
      ...
      description: {
        ...
        concealed: game.user.isGM && game.settings.get("nih", "concealItemDescriptions") && !description.chat
      }
    };
  ...
  async getCraftCost({ baseItem="craft" }={}) {
    ...
    if ( !this.properties.has("mgc") || !rarity ) {
      const { mundane } = CONFIG.NIH.crafting;
    ...
    const { magic } = CONFIG.NIH.crafting;
