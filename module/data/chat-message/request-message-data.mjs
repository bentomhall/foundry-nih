...
export default class RequestMessageData extends ChatMessageDataModel {
  ...
  static metadata = Object.freeze(foundry.utils.mergeObject(super.metadata, {
    ...
    template: "systems/nih/templates/chat/request-card.hbs"
  }, { inplace: false }));
  ...
  async _prepareContext() {
    return {
      button: {
        ...
        label: game.i18n.localize(this.button.label || "NIH.CHATMESSAGE.REQUEST.Action.Handle")
      },
  ...
  static async #handleRequest(event, target) {
    ...
    const result = await CONFIG.NIH.requests[this.handler](actor, this.parent, this.data, { event });
    if ( (result instanceof ChatMessage) && !result.getFlag("nih", "requestResult") ) {
      return result.setFlag("nih", "requestResult", { actorUuid: actor.uuid, requestId: this.parent.id });
    }
  }
  ...
  static onCreateMessage(message) {
    const flag = message.getFlag("nih", "requestResult");
  ...
  static onUpdateResultMessage(message, changes, options, userId) {
    const flag = foundry.utils.getProperty(changes, "flags.nih.requestResult");
    if ( flag && (game.users.activeGM === game.user) ) RequestMessageData.#updateRequestTargets(message, flag);
  }
