...
export default class CreatureTemplate extends CommonTemplate {
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      bonuses: new foundry.data.fields.SchemaField({
        mwak: makeAttackBonuses({label: "NIH.BonusMWAttack"}),
        rwak: makeAttackBonuses({label: "NIH.BonusRWAttack"}),
        msak: makeAttackBonuses({label: "NIH.BonusMSAttack"}),
        rsak: makeAttackBonuses({label: "NIH.BonusRSAttack"}),
        abilities: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "NIH.BonusAbilityCheck"}),
          save: new FormulaField({required: true, label: "NIH.BonusAbilitySave"}),
          skill: new FormulaField({required: true, label: "NIH.BonusAbilitySkill"})
        }, {label: "NIH.BonusAbility"}),
        spell: new foundry.data.fields.SchemaField({
          dc: new FormulaField({required: true, deterministic: true, label: "NIH.BonusSpellDC"})
        }, {label: "NIH.BonusSpell"})
      }, {label: "NIH.Bonuses"}),
      skills: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, min: 0, max: 2, step: 0.5, initial: 0, label: "NIH.ProficiencyLevel"
        }),
        ability: new foundry.data.fields.StringField({required: true, initial: "dex", label: "NIH.Ability"}),
        bonuses: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "NIH.SkillBonusCheck"}),
          passive: new FormulaField({required: true, label: "NIH.SkillBonusPassive"})
        }, {label: "NIH.SkillBonuses"})
      }), {
        initialKeys: CONFIG.NIH.skills, initialValue: this._initialSkillValue,
        initialKeysOnly: true, label: "NIH.Skills"
      }),
      tools: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, min: 0, max: 2, step: 0.5, initial: 1, label: "NIH.ProficiencyLevel"
        }),
        ability: new foundry.data.fields.StringField({required: true, initial: "int", label: "NIH.Ability"}),
        bonuses: new foundry.data.fields.SchemaField({
  ...
  static _initialSkillValue(key, initial) {
    if ( CONFIG.NIH.skills[key]?.ability ) initial.ability = CONFIG.NIH.skills[key].ability;
    return initial;
  }
  ...
  static get _spellLevels() {
    const levels = Object.keys(CONFIG.NIH.spellLevels).filter(a => a !== "0").map(l => `spell${l}`);
    return [...levels, "pact"];
  }
  ...
  static #migrateSensesData(source) {
    ...
    for ( let s of original.split(",") ) {
      ...
      if ( (type in CONFIG.NIH.senses) && !(type in source.attributes.senses) ) {
  ...
  static #migrateToolData(source) {
    ...
    for ( const prof of original.value ) {
      const validProf = (prof in CONFIG.NIH.toolProficiencies) || (prof in CONFIG.NIH.toolIds);
...
function makeAttackBonuses(schemaOptions={}) {
  return new foundry.data.fields.SchemaField({
    attack: new FormulaField({required: true, label: "NIH.BonusAttack"}),
    damage: new FormulaField({required: true, label: "NIH.BonusDamage"})
  }, schemaOptions);
}
