...
export default class CreatureTemplate extends CommonTemplate {
  ...
  static #migrateToolData(source) {
    ...
    for ( const prof of original.value ) {
      const validProf = (prof in CONFIG.NIH.toolProficiencies) || (prof in CONFIG.NIH.tools);
   ...
   *                                             `this.flags.nih.originalActor` are used.
  ...
  prepareSkill(skillId, {
    ...
    const passive = flags.observantFeat && CONFIG.NIH.characterFlags.observantFeat.skills.includes(skillId) ? 5 : 0;
    ...
    skillData.passive = CONFIG.NIH.skillPassive.base + skillData.mod + skillData.bonus + skillData.prof.flat
      + passive + passiveBonus + (advantageMode * CONFIG.NIH.skillPassive.modifier);
