...
export default class AttributesFields {
  ...
  static get armorClass() {
    return {
      calc: new StringField({ initial: "default", label: "NIH.ArmorClassCalculation" }),
      flat: new NumberField({ required: true, integer: true, min: 0, label: "NIH.ArmorClassFlat" }),
  ...
  static get hitPoints() {
    return {
      dt: new NumberField({ integer: true, min: 0, label: "NIH.DamageThreshold" }),
      max: new NumberField({ nullable: true, integer: true, min: 0, initial: null, label: "NIH.HitPointsMax" }),
      temp: new NumberField({ integer: true, initial: 0, min: 0, label: "NIH.HitPointsTemp" }),
      tempmax: new NumberField({
        integer: true, initial: 0, label: "NIH.HitPointsTempMax", hint: "NIH.HitPointsTempMaxHint"
      }),
      value: new NumberField({ nullable: true, integer: true, min: 0, initial: null, label: "NIH.HitPointsCurrent" })
    };
  }
  ...
  static get common() {
    return {
      ac: new SchemaField(this.armorClass, { label: "NIH.ArmorClass" }),
      init: new RollConfigField({
        ability: "",
        bonus: new FormulaField({ required: true, label: "NIH.InitiativeBonus" })
  ...
  static get creature() {
    return {
      ...
      spellcasting: new StringField({ required: true, blank: true, label: "NIH.SpellAbility" }),
      ...
      concentration: new RollConfigField({
        ...
        bonuses: new SchemaField({
          save: new FormulaField({ required: true, label: "NIH.ConcentrationBonus" })
        }),
        limit: new NumberField({ integer: true, min: 0, initial: 1, label: "NIH.ConcentrationLimit" })
      }, { label: "NIH.Concentration" }),
      loyalty: new SchemaField({
        value: new NumberField({ integer: true, min: 0, max: 20, label: "NIH.Loyalty" })
      })
    };
  }
  ...
  static prepareArmorClass(rollData) {
    ...
    const { armors, shields } = this.parent.itemTypes.equipment.reduce((obj, equip) => {
      if ( !equip.system.equipped || !(equip.system.type.value in CONFIG.NIH.armorTypes)) return obj;
    ...
    ac.label = !["custom", "flat"].includes(ac.calc) ? CONFIG.NIH.armorClasses[ac.calc]?.label : null;
      ...
      default:
        ...
        try {
          const replaced = replaceFormulaData(formula, rollData, {
            actor: this, missing: null, property: game.i18n.localize("NIH.ArmorClass")
          });
          ac.base = replaced ? new Roll(replaced).evaluateSync().total : 0;
        } catch(err) {
          this.parent._preparationWarnings.push({
            message: game.i18n.format("NIH.WarnBadACFormula", { formula }), link: "armor", type: "error"
          });
  ...
  static prepareConcentration(rollData) {
    ...
    const abilityId = concentration.ability || CONFIG.NIH.defaultAbilities.concentration;
  ...
  static prepareEncumbrance(rollData, { validateItem }={}) {
    const config = CONFIG.NIH.encumbrance;
    const encumbrance = this.attributes.encumbrance ??= {};
    const baseUnits = CONFIG.NIH.encumbrance.baseUnits[this.parent.type]
      ?? CONFIG.NIH.encumbrance.baseUnits.default;
    ...
    const keys = Object.keys(CONFIG.NIH.actorSizes);
    const index = keys.findIndex(k => k === this.traits.size);
    const sizeConfig = CONFIG.NIH.actorSizes[
      keys[this.parent.flags.nih?.powerfulBuild ? Math.min(index + 1, keys.length - 1) : index]
    ];
  ...
  static prepareExhaustionLevel() {
    ...
    const level = exhaustion?.getFlag("nih", "exhaustionLevel");
  ...
  static prepareInitiative(rollData) {
    ...
    const isLegacy = game.settings.get("nih", "rulesVersion") === "legacy";
    ...
    init.score = CONFIG.NIH.skillPassive.base + init.total + (init.roll.mode * CONFIG.NIH.skillPassive.modifier);
  }
  ...
  static prepareMovement(rollData=this.parent.getRollData()) {
    ...
    for ( const type of Object.keys(CONFIG.NIH.movementTypes) ) {
    ...
    let reduction = game.settings.get("nih", "rulesVersion") === "modern"
      ? (this.attributes.exhaustion ?? 0) * (CONFIG.NIH.conditionTypes.exhaustion?.reduction?.speed ?? 0) : 0;
    reduction = convertLength(reduction, CONFIG.NIH.defaultUnits.length.imperial, units);
    ...
    for ( const type of Object.keys(CONFIG.NIH.movementTypes) ) {
      ...
      if ( speed ) {
        ...
        if ( heavilyEncumbered ) {
          speed = Math.max(0, speed - (CONFIG.NIH.encumbrance.speedReduction.heavilyEncumbered[units] ?? 0));
        } else if ( encumbered ) {
          speed = Math.max(0, speed - (CONFIG.NIH.encumbrance.speedReduction.encumbered[units] ?? 0));
        }
        if ( exceedingCarryingCapacity ) {
          speed = Math.min(speed, CONFIG.NIH.encumbrance.speedReduction.exceedingCarryingCapacity[units] ?? 0);
        }
      }
  ...
  static prepareRace(race, { force=false }={}) {
    for ( const key of Object.keys(CONFIG.NIH.movementTypes) ) {
    ...
    for ( const key of Object.keys(CONFIG.NIH.senses) ) {
  ...
  static prepareSpellcastingAbility() {
    ...
    this.attributes.spell.abilityLabel = CONFIG.NIH.abilities[this.attributes.spellcasting]?.label ?? "";
