...
export default class TraitsField {
  ...
  static get common() {
    return {
      size: new StringField({ required: true, initial: "med", label: "NIH.Size" }),
      di: new DamageTraitField({}, { label: "NIH.DamImm" }),
      dr: new DamageTraitField({}, { label: "NIH.DamRes" }),
      dv: new DamageTraitField({}, { label: "NIH.DamVuln" }),
      dm: new SchemaField({
        amount: new MappingField(new FormulaField({ deterministic: true }), { label: "NIH.DamMod" }),
        bypasses: new SetField(new StringField(), {
          label: "NIH.DamagePhysicalBypass", hint: "NIH.DamagePhysicalBypassHint"
        })
      }),
      ci: new SimpleTraitField({}, { label: "NIH.ConImm" })
    };
  }
  ...
  static prepareLanguages() {
    ...
    if ( languages.value.has("ALL") ) labels.languages.push(game.i18n.localize("NIH.Language.All"));
    else {
      ...
      for ( const [key, data] of Object.entries(CONFIG.NIH.languages) ) {
  ...
  static prepareResistImmune() {
    ...
    if ( this.parent.hasConditionEffect("petrification") ) {
      this.traits.dr.custom = game.i18n.localize("NIH.DamageAll");
      Object.keys(CONFIG.NIH.damageTypes).forEach(type => this.traits.dr.value.add(type));
