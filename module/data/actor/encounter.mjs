...
export default class EncounterData extends GroupTemplate {
    ...
    return this.mergeSchema(super.defineSchema(), {
      members: new ArrayField(new SchemaField({
        ...
        quantity: new SchemaField({
          value: new NumberField({ initial: 1, integer: true, min: 0, label: "NIH.Quantity" }),
          formula: new FormulaField({ label: "NIH.QuantityFormula" })
        })
  ...
  async getDifficulty(party) {
    ...
    const [low, med] = (CONFIG.NIH.ENCOUNTER_DIFFICULTY[level] ?? []).map(t => t * creatures.length);
    if ( !low ) return null;
  ...
  async getPlaceableMembers() {
    return (await Promise.all((await this.getMembers()).map(async member => {
      member.actor = await nih.documents.Actor5e.fetchExisting(member.actor.uuid);
