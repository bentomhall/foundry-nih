...
/**
 * A data model and API layer which handles the schema and functionality of "group" type Actors in the nih system.
...
export default class GroupData extends GroupTemplate {
    ...
    return this.mergeSchema(super.defineSchema(), {
      attributes: new SchemaField({
        travel: new TravelField({}, {
          initialTime: () => CONFIG.NIH.travelTimes.group, initialUnits: () => defaultUnits("travel")
        })
      }, { label: "NIH.Attributes" }),
      details: new SchemaField({
        xp: new SchemaField({
          value: new NumberField({ integer: true, min: 0, label: "NIH.ExperiencePoints.Current" })
  ...
  static #migrateTravel(source) {
    ...
    const config = CONFIG.NIH.movementUnits[units];
  ...
  getTravelPace() {
    ...
    const travelPaces = Object.keys(CONFIG.NIH.travelPace);
    ...
    if ( (slowed && !this.primaryVehicle) || (this.primaryVehicle?.system.details.type === "land") ) {
      ...
      const unitConfig = CONFIG.NIH.travelUnits[units];
    ...
    return {
      pace: {
        ...
        label: CONFIG.NIH.travelPace[pace]?.label,
        value: pace
      },
      paces: { ...paces }
    };
  ...
  async rollSkill(config) {
    ...
    await foundry.documents.ChatMessage.implementation.create({
      flavor: game.i18n.format("NIH.SkillPromptTitle", { skill: skillLabel, ability: abilityLabel }),
      speaker: ChatMessage.getSpeaker({ actor: this.parent, alias: this.parent.name }),
      system: {
        button: {
          icon: "fa-solid fa-dice-d20",
          label: game.i18n.localize("NIH.SkillRoll", { skill: skillLabel, ability: abilityLabel })
        },
        data: { ...config },
        handler: "skill",
  ...
  async rest(config, result) {
     ...
     * @function nih.groupRestCompleted
     * @memberof hookEvents
