...
export default class VehicleData extends CommonTemplate {
    ...
    return this.mergeSchema(super.defineSchema(), {
      attributes: new SchemaField({
        ...
        ac: new SchemaField({
          ...
          calc: new StringField({ initial: "flat", label: "NIH.ArmorClassCalculation" })
        }, { label: "NIH.ArmorClass" }),
        hp: new SchemaField({
          ...AttributesFields.hitPoints,
          mt: new NumberField({
            required: true, integer: true, min: 0, label: "NIH.VEHICLE.Mishap.Threshold.label"
          })
        }, { label: "NIH.HitPoints" }),
        actions: new SchemaField({
          max: new NumberField({
            required: true, nullable: false, integer: true, initial: 3, min: 0, max: 3,
            label: "NIH.VEHICLE.FIELDS.attributes.actions.max.label"
          }),
          spent: new NumberField({
            required: true, nullable: false, integer: true, initial: 0, min: 0, max: 3,
            label: "NIH.VEHICLE.FIELDS.attributes.actions.spent.label"
          }),
          stations: new BooleanField({
            required: true, initial: true, label: "NIH.VEHICLE.FIELDS.attributes.actions.stations.label"
          }),
          thresholds: new SchemaField({
            2: new NumberField({
              required: true, integer: true, min: 0,
              label: "NIH.VEHICLE.FIELDS.attributes.actions.thresholds.full.label"
            }),
            1: new NumberField({
              required: true, integer: true, min: 0,
              label: "NIH.VEHICLE.FIELDS.attributes.actions.thresholds.mid.label"
            }),
            0: new NumberField({
              required: true, integer: true, min: 0,
              label: "NIH.VEHICLE.FIELDS.attributes.actions.thresholds.min.label"
            })
          }, { label: "NIH.VEHICLE.FIELDS.attributes.actions.thresholds.label" })
        }, { label: "NIH.VEHICLE.FIELDS.attributes.actions.label" }),
        capacity: new SchemaField({
          cargo: new SchemaField({
            value: new NumberField({ min: 0, label: "NIH.VEHICLE.FIELDS.attributes.capacity.cargo.value.label" }),
            units: new StringField({
              required: true, blank: false, label: "NIH.UNITS.WEIGHT.Label", initial: () => defaultUnits("weight")
            })
          }, { label: "NIH.VEHICLE.FIELDS.attributes.capacity.cargo.value.label" }),
          creature: new StringField({ required: true, label: "NIH.VehicleCreatureCapacity" }) // FIXME: Leave in the model until we decide how to migrate it.
        }, { label: "NIH.VEHICLE.FIELDS.attributes.capacity.label" }),
        price: new SchemaField({
          value: new NumberField({ initial: null, min: 0, label: "NIH.Price" }),
          denomination: new StringField({ required: true, blank: false, initial: "gp", label: "NIH.Currency" })
        }, { label: "NIH.Price" }),
        ...
        travel: new TravelField({ pace: false, }, {
          initialTime: () => CONFIG.NIH.travelTimes.vehicle, initialUnits: () => defaultUnits("travel")
        })
      ...
      details: new SchemaField({
        ...
        type: new StringField({ required: true, blank: false, initial: "water", label: "NIH.VEHICLE.Type.label" })
      }, { label: "NIH.Details" }),
      ...
      traits: new SchemaField({
        ...TraitsFields.common,
        size: new StringField({ required: true, blank: false, initial: "lg", label: "NIH.Size" }),
        weight: new SchemaField({
          value: new NumberField({ min: 0, label: "NIH.Weight" }),
          units: new StringField({
            required: true, blank: false, label: "NIH.UNITS.WEIGHT.Label", initial: () => defaultUnits("weight")
          })
        }, { label: "NIH.Weight" }),
        keel: new SchemaField({
          value: new NumberField({ min: 0, label: "NIH.VEHICLE.FIELDS.traits.keel.value.label" }),
          units: new StringField({
            required: true, blank: false, label: "NIH.UNITS.DISTANCE.Label", initial: () => defaultUnits("length")
          })
        }),
        beam: new SchemaField({
          value: new NumberField({ min: 0, label: "NIH.VEHICLE.FIELDS.traits.beam.value.label" }),
          units: new StringField({
            required: true, blank: false, label: "NIH.UNITS.DISTANCE.Label", initial: () => defaultUnits("length")
          })
        }),
        dimensions: new StringField({ required: true, label: "NIH.Dimensions" }) // FIXME: Leave in the model until we decide how to migrate it.
      }, { label: "NIH.Traits" }),
  ...
  static #migrateMovement(source) {
    ...
    let max = 0;
    for ( const p in CONFIG.NIH.movementTypes ) {
