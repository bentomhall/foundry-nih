...
export default class BaseEnchantActivityData extends BaseActivityData {
  ...
  get appliedEnchantments() {
    return nih.registry.enchantments.applied(this.uuid);
  }
  ...
  static transformEffectsData(source, options) {
    ...
    for ( const effect of source.effects ) {
      if ( (effect.type !== "enchantment") && (effect.flags?.nih?.type !== "enchantment") ) continue;
      effects.push({ _id: effect._id, ...(effect.flags?.nih?.enchantment ?? {}) });
      delete effect.flags?.nih?.enchantment;
    }
    return effects;
  }
