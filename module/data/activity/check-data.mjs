...
export default class BaseCheckActivityData extends BaseActivityData {
  ...
  /** @override */
  get ability() {
    if ( this.check.dc.calculation in CONFIG.NIH.abilities ) return this.check.dc.calculation;
  ...
  static transformTypeData(source, activityData, options) {
    return foundry.utils.mergeObject(activityData, {
      check: {
        ability: source.system.ability ?? Object.keys(CONFIG.NIH.abilities)[0]
      }
    });
  }
  ...
  getAbility(associated) {
    ...
    if ( associated in CONFIG.NIH.skills ) return CONFIG.NIH.skills[associated]?.ability ?? null;
    else if ( associated in CONFIG.NIH.tools ) {
      if ( (this.item.type === "tool") && this.item.system.ability ) return this.item.system.ability;
      return CONFIG.NIH.tools[associated]?.ability ?? null;
    }
    return null;
  }
}
