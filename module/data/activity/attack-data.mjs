...
export default class BaseAttackActivityData extends BaseActivityData {
  ...
  get ability() {
    ...
    if ( this.attack.ability in CONFIG.NIH.abilities ) return this.attack.ability;
  ...
  getActionLabel(attackMode) {
    ...
    if ( attackMode ) {
      ...
      attackModeLabel = game.i18n.localize(`NIH.ATTACK.Mode.${key}`);
    }
    const actionType = this.getActionType(attackMode);
    let actionTypeLabel = game.i18n.localize(`NIH.Action${actionType.toUpperCase()}`);
    const isLegacy = game.settings.get("nih", "rulesVersion") === "legacy";
    const isUnarmed = this.attack.type.classification === "unarmed";
    if ( isUnarmed ) attackModeLabel = game.i18n.localize("NIH.ATTACK.Classification.Unarmed");
    const isSpell = (actionType === "rsak") || (actionType === "msak");
    if ( isLegacy || isSpell ) return [actionTypeLabel, attackModeLabel].filterJoin(" • ");
    actionTypeLabel = game.i18n.localize(`NIH.ATTACK.Attack.${actionType}`);
    if ( isUnarmed ) return [actionTypeLabel, attackModeLabel].filterJoin(" • ");
    const weaponType = CONFIG.NIH.weaponTypeMap[this.item.system.type?.value];
    const weaponTypeLabel = weaponType
      ? game.i18n.localize(`NIH.ATTACK.Weapon.${weaponType.capitalize()}`)
      : CONFIG.NIH.weaponTypes[this.item.system.type?.value];
  ...
  getDamageConfig(config={}) {
    ...
    if ( ammo ) {
      const properties = Array.from(ammo.properties).filter(p => CONFIG.NIH.itemProperties[p]?.isPhysical);
  ...
  getRangeLabel() {
    ...
    if ( this.validAttackTypes.has("melee") ) {
      ...
      parts.push(game.i18n.format("NIH.RANGE.Formatted.Reach", {
    ...
    if ( this.validAttackTypes.has("ranged") ) {
      ...
      if ( range ) parts.push(game.i18n.format("NIH.RANGE.Formatted.Range", { range }));
    }
  ...
  _processDamagePart(damage, rollConfig, rollData, index=0) {
    ...
    const criticalBonusDice = this.actor?.getFlag("nih", "meleeCriticalDamageDice") ?? 0;
