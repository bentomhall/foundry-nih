...
export class ConsumptionTargetData extends foundry.abstract.DataModel {
  ...
  get combatOnly() {
    ...
    return recovery.every(r => CONFIG.NIH.limitedUsePeriods[r.period]?.type === "combat");
  }
   ...
   */
  get validTargets() {
    const config = CONFIG.NIH.activityConsumptionTypes[this.type];
  ...
  async consume(config, updates) {
    const typeConfig = CONFIG.NIH.activityConsumptionTypes[this.type];
  ...
  static async consumeActivityUses(config, updates) {
    const result = await this._usesConsumption(config, {
      ...
      type: game.i18n.format("NIH.CONSUMPTION.Type.ActivityUses.Warning", {
  ...
  static async consumeAttribute(config, updates) {
    ...
    if ( !foundry.utils.hasProperty(this.actor, keyPath) ) throw new ConsumptionError(
      game.i18n.format("NIH.CONSUMPTION.Warning.MissingAttribute", {
    ...
    let warningMessage;
    if ( (cost > 0) && !current ) warningMessage = "NIH.CONSUMPTION.Warning.None";
    else if ( current < cost ) warningMessage = "NIH.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      ...
      type: game.i18n.format("NIH.CONSUMPTION.Type.Attribute.Warning", { attribute })
    }));
  ...
  static async consumeHitDice(config, updates) {
    ...
    let warningMessage;
    if ( !validClasses.length ) warningMessage = "NIH.CONSUMPTION.Warning.MissingHitDice";
    else if ( (cost > 0) && !total ) warningMessage = "NIH.CONSUMPTION.Warning.None";
    else if ( total < cost ) warningMessage = "NIH.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      ...
      throw new ConsumptionError(game.i18n.format(warningMessage, {
        ...
        type: game.i18n.format("NIH.CONSUMPTION.Type.HitDice.Warning", { denomination })
      }));
    }
  ...
  static async consumeItemUses(config, updates) {
    ...
    if ( !item ) throw new ConsumptionError(game.i18n.format("NIH.CONSUMPTION.Warning.MissingItem", {
      activity: this.activity.name, item: this.item.name
    }));

    const result = await this._usesConsumption(config, {
      uses: item.system.uses,
      type: game.i18n.format("NIH.CONSUMPTION.Type.ItemUses.Warning", { name: this.item.name }),
      rolls: updates.rolls,
  ...
  static async consumeMaterial(config, updates) {
    ...
    let warningMessage;
    if ( cost > 0 && !item.system.quantity ) warningMessage = "NIH.CONSUMPTION.Warning.None";
    else if ( cost > item.system.quantity ) warningMessage = "NIH.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) throw new ConsumptionError(game.i18n.format(warningMessage, {
      available: formatNumber(item.system.quantity), cost: formatNumber(cost),
      type: game.i18n.format("NIH.CONSUMPTION.Type.Material.Warning", { name: item.name })
    }));
  ...
  static async consumeSpellSlots(config, updates) {
    const levelNumber = Math.clamp(
      this.resolveLevel({ config, rolls: updates.rolls }), 1, Object.keys(CONFIG.NIH.spellLevels).length - 1
    );
    ...
    let warningMessage;
    if ( !levelData?.max ) warningMessage = "NIH.CONSUMPTION.Warning.MissingSpellSlot";
    else if ( (cost > 0) && !levelData.value ) warningMessage = "NIH.CONSUMPTION.Warning.None";
    else if ( newValue < 0 ) warningMessage = "NIH.CONSUMPTION.Warning.NotEnough";
    if ( warningMessage ) {
      const level = CONFIG.NIH.spellLevels[levelNumber];
      const type = game.i18n.format("NIH.CONSUMPTION.Type.SpellSlots.Warning", { level });
      throw new ConsumptionError(game.i18n.format(warningMessage, {
  ...
  async _usesConsumption(config, { uses, type, rolls, delta }) {
    ...
    let warningMessage;
    if ( cost > 0 && !uses.value ) warningMessage = "NIH.CONSUMPTION.Warning.None";
    else if ( cost > uses.value ) warningMessage = "NIH.CONSUMPTION.Warning.NotEnough";
  ...
  static consumptionLabelsActivityUses(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`NIH.CONSUMPTION.Type.ActivityUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NIH.CONSUMPTION.Type.ActivityUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`NIH.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
          availableUse: game.i18n.localize(`NIH.CONSUMPTION.Type.Use.${usesPluralRule}`)
        }
      ),
  ...
  static consumptionLabelsAttribute(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`NIH.CONSUMPTION.Type.Attribute.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NIH.CONSUMPTION.Type.Attribute.PromptHint${increaseKey}`,
        {
          cost,
  ...
  static consumptionLabelsHitDice(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    let denomination;
    if ( this.target === "smallest" ) denomination = game.i18n.localize("NIH.ConsumeHitDiceSmallest");
    else if ( this.target === "largest" ) denomination = game.i18n.localize("NIH.ConsumeHitDiceLargest");
    ...
    return {
      label: game.i18n.localize(`NIH.CONSUMPTION.Type.HitDice.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NIH.CONSUMPTION.Type.HitDice.PromptHint${increaseKey}`,
        {
          cost, denomination: denomination.toLowerCase(),
          die: game.i18n.localize(`NIH.CONSUMPTION.Type.HitDie.${pluralRule}`),
  ...
  static consumptionLabelsItemUses(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    const item = this.actor.items.get(this.target);
    const itemName = item ? item.name : game.i18n.localize("NIH.CONSUMPTION.Target.ThisItem").toLowerCase();
    ...
    if ( simplifiedCost > uses.value ) warn = true;
      ...
      type: "warn",
      message: game.i18n.format("NIH.CONSUMPTION.Warning.WillDestroy", { item: itemName })
    });

    return {
      label: game.i18n.localize(`NIH.CONSUMPTION.Type.ItemUses.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NIH.CONSUMPTION.Type.ItemUses.PromptHint${increaseKey}`,
        {
          cost,
          use: game.i18n.localize(`NIH.CONSUMPTION.Type.Use.${pluralRule}`),
          available: formatNumber(uses.value),
  ...
  static consumptionLabelsMaterial(config, { consumed }={}) {
    ...
    return {
      label: game.i18n.localize(`NIH.CONSUMPTION.Type.Material.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NIH.CONSUMPTION.Type.Material.PromptHint${increaseKey}`,
        {
          cost,
          item: item ? `<em>${item.name}</em>`
            : game.i18n.localize("NIH.CONSUMPTION.Target.UnknownItem").toLowerCase(),
  ...
  static consumptionLabelsSpellSlots(config, { consumed }={}) {
    const { cost, simplifiedCost, increaseKey, pluralRule } = this._resolveHintCost(config);
    const levelNumber = Math.clamp(this.resolveLevel({ config }), 1, Object.keys(CONFIG.NIH.spellLevels).length - 1);
    const level = CONFIG.NIH.spellLevels[levelNumber].toLowerCase();
    const available = this.actor.system.spells?.[`spell${levelNumber}`]?.value ?? 0;
    return {
      label: game.i18n.localize(`NIH.CONSUMPTION.Type.SpellSlots.Prompt${increaseKey}`),
      hint: game.i18n.format(
        `NIH.CONSUMPTION.Type.SpellSlots.PromptHint${increaseKey}`,
        {
          cost,
          slot: game.i18n.format(`NIH.CONSUMPTION.Type.SpellSlot.${pluralRule}`, { level }),
          available: formatNumber(available)
        }
      ),
      warn: simplifiedCost > available
  ...
  static validAttributeTargets() {
    ...
    return TokenDocument.implementation.getConsumedAttributes(this.actor.type).map(attr => {
      let group;
      if ( attr.startsWith("abilities.") ) group = game.i18n.localize("NIH.AbilityScorePl");
      else if ( attr.startsWith("currency.") ) group = game.i18n.localize("NIH.Currency");
      else if ( attr.startsWith("spells.") ) group = game.i18n.localize("NIH.CONSUMPTION.Type.SpellSlots.Label");
      else if ( attr.startsWith("attributes.movement.") ) group = game.i18n.localize("NIH.Speed");
      else if ( attr.startsWith("attributes.senses.") ) group = game.i18n.localize("NIH.Senses");
      else if ( attr.startsWith("attributes.actions.") ) group = game.i18n.localize("NIH.Vehicle");
      else if ( attr.startsWith("resources.") ) group = game.i18n.localize("NIH.Resources");
  ...
  static validHitDiceTargets() {
    return [
      { value: "smallest", label: game.i18n.localize("NIH.ConsumeHitDiceSmallest") },
      ...CONFIG.NIH.hitDieTypes.map(d => ({ value: d, label: d })),
      { value: "largest", label: game.i18n.localize("NIH.ConsumeHitDiceLargest") }
    ];
  }
  ...
  static validItemUsesTargets() {
    const makeLabel = (name, item) => {
      ...
      if ( uses.max && (uses.recovery?.length === 1) && (uses.recovery[0].type === "recoverAll")
        ...
        const per = CONFIG.NIH.limitedUsePeriods[uses.recovery[0].period]?.abbreviation;
        label = game.i18n.format("NIH.AbilityUseConsumableLabel", { max: uses.max, per });
      }
      else label = game.i18n.format("NIH.AbilityUseChargesLabel", { value: uses.value });
      return `${name} (${label})`;
    };
    return [
      { value: "", label: makeLabel(game.i18n.localize("NIH.CONSUMPTION.Target.ThisItem"), this.item) },
      { rule: true },
      ...(this.actor?.items ?? [])
  ...
  static validSpellSlotsTargets() {
    return Object.entries(CONFIG.NIH.spellLevels).reduce((arr, [value, label]) => {
