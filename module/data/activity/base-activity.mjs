...
export default class BaseActivityData extends foundry.abstract.DataModel {
  ...
  static LOCALIZATION_PREFIXES = [
    "NIH.ACTIVITY", "NIH.ACTIVATION", "NIH.CONSUMPTION",
    "NIH.DURATION", "NIH.RANGE", "NIH.TARGET", "NIH.USES"
  ];
   ...
   */
  get isRider() {
    return !!this.item.getFlag("nih", "riders.activity")?.includes(this.id);
  }
   ...
   */
  get isScaledScroll() {
    return !!this.item.getFlag("nih", "spellLevel");
  }
   ...
   * @type {string}
   */
  static INITIAL_ID = staticID("nihactivity");
  ...
  static transformDamagePartData(source, [formula, type]) {
    ...
    if ( parsed && CONFIG.NIH.dieSteps.includes(Number(parsed[2])) ) {
  ...
  static transformEffectsData(source, options) {
    return source.effects
      .filter(e => !e.transfer && (e.type !== "enchantment") && (e.flags?.nih?.type !== "enchantment"))
      .map(e => ({ _id: e._id }));
  }
  ...
  static transformTargetData(source, options) {
    ...
    if ( source.system.target?.type in CONFIG.NIH.areaTargetTypes ) foundry.utils.mergeObject(data, {
      template: {
  ...
  prepareFinalData(rollData) {
    ...
    for ( const target of this.consumption.targets ) {
      ...
      if ( !actor.items.has(target.target) ) {
        const message = game.i18n.format("NIH.CONSUMPTION.Warning.MissingItem", {
    ...
    this.labels.damage = this.labels.damages = rolls.map(roll => {
      ...
      if ( types.length ) {
        label = `${formula} ${game.i18n.getListFormatter({ type: "conjunction" }).format(
          types.map(p => CONFIG.NIH.damageTypes[p]?.label ?? CONFIG.NIH.healingTypes[p]?.label).filter(_ => _)
        )}`;
      }
  ...
  _processDamagePart(damage, rollConfig, rollData, index=0) {
    ...
    const lastType = this.item.getFlag("nih", `last.${this.id}.damageType.${index}`);
    ...
    return {
      ...
      options: {
        ...
        properties: Array.from(this.item.system.properties ?? [])
          .filter(p => CONFIG.NIH.itemProperties[p]?.isPhysical)
      }
    };
  }
