...
export class SpellcastingModel extends foundry.abstract.DataModel {
  ...
  static defineSchema() {
    return {
      ...
      label: new StringField({ required: true, initial: () => game.i18n.localize("NIH.SPELLCASTING.Unlabeled") }),
   ...
   */
  static fromConfig() {
    const { spellcasting } = CONFIG.NIH;

    // Map progressions to spellcasting for faster lookup.
    CONFIG.NIH.spellProgression = { none: { label: game.i18n.localize("NIH.SpellNone") } };
    ...
    Object.entries(spellcasting).forEach(([key, config]) => {
      ...
      Object.entries(config.progression ?? {}).forEach(([k, v]) => {
        if ( k in CONFIG.NIH.spellProgression ) console.warn(`Duplicate spell progression key '${k}' detected.`);
        CONFIG.NIH.spellProgression[k] = { ...v, type: key };
      });
    });
  }
...
export class SlotSpellcasting extends SpellcastingModel {
  ...
  get recovery() {
    ...
    return this.#recovery = Object.entries(CONFIG.NIH.restTypes).reduce((acc, [k, v]) => {
...
export class SingleLevelSpellcasting extends SlotSpellcasting {
  ...
  getLabel({ level, format }={}) {
    if ( !(level in CONFIG.NIH.spellLevels) ) return this.label;
    const short = format === "short";
    return [this.label, short ? "" : "—", short ? "" : CONFIG.NIH.spellLevels[level]].filterJoin(" ");
  }
  ...
  /** @override */
  prepareSlots(spells, actor, progression) {
    let level = Math.clamp(progression[this.key], 0, CONFIG.NIH.maxLevel);
...
export class MultiLevelSpellcasting extends SlotSpellcasting {
  ...
  /** @override */
  getAvailableLevels(actor) {
    return Array.fromRange(Object.keys(CONFIG.NIH.spellLevels).length - 1, 1).reduce((arr, l) => {
  ...
  /** @override */
  getLabel({ level }={}) {
    return game.i18n.localize(`NIH.SPELLCASTING.SLOTS.${this.getSpellSlotKey(level)}`);
  }
