...
export default class ActivationField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.activation.scalar = CONFIG.NIH.activityActivationTypes[this.activation.type]?.scalar ?? false;
    if ( !this.activation.scalar ) this.activation.value = null;

    if ( labels && this.activation.type ) {
      labels.activation = [
        this.activation.value, CONFIG.NIH.activityActivationTypes[this.activation.type]?.label
      ].filterJoin(" ");
      const formatter = game.i18n.getListFormatter({ type: "disjunction" });
      labels.ritualActivation = this.properties?.has?.("ritual")
        ? formatter.format([labels.activation, game.i18n.localize("NIH.Ritual")]) : labels.activation;
    }
  }
}
