...
export default class DurationField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.duration.scalar = this.duration.units in CONFIG.NIH.scalarTimePeriods;
    if ( this.duration.scalar ) {
      prepareFormulaValue(this, "duration.value", "NIH.DURATION.FIELDS.duration.value.label", rollData);
    } else this.duration.value = null;

    if ( labels && this.duration.units ) {
      if ( this.duration.value && (this.duration.units in CONFIG.NIH.timeUnits) ) {
        labels.duration = formatTime(this.duration.value, this.duration.units);
      } else labels.duration = CONFIG.NIH.timePeriods[this.duration.units] ?? "";
      labels.concentrationDuration = this.duration.concentration || this.properties?.has("concentration")
        ? game.i18n.format("NIH.ConcentrationDuration", { duration: labels.duration }) : labels.duration;
    }
