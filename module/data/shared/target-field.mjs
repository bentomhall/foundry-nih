...
export default class TargetField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    ...
    if ( this.target.affects.scalar ) {
      prepareFormulaValue(this, "target.affects.count", "NIH.TARGET.FIELDS.target.affects.count.label", rollData);
    ...
    if ( this.target.template.type ) {
      ...
      prepareFormulaValue(this, "target.template.count", "NIH.TARGET.FIELDS.target.template.count.label", rollData);
      prepareFormulaValue(this, "target.template.size", "NIH.TARGET.FIELDS.target.template.size.label", rollData);
      prepareFormulaValue(this, "target.template.width", "NIH.TARGET.FIELDS.target.template.width.label", rollData);
      prepareFormulaValue(this, "target.template.height", "NIH.TARGET.FIELDS.target.template.height.label", rollData);
    } else {
    ...
    const templateConfig = CONFIG.NIH.areaTargetTypes[this.target.template.type];
    if ( templateConfig ) {
      const parts = [];
      if ( this.target.template.count > 1 ) parts.push(`${this.target.template.count} ×`);
      if ( this.target.template.units in CONFIG.NIH.movementUnits ) {
    ...
    const affectsConfig = CONFIG.NIH.individualTargetTypes[this.target.affects.type];
      ...
      sheet: affectsConfig?.counted ? game.i18n.format(
        `${affectsConfig.counted}.${this.target.affects.count ? pr.select(this.target.affects.count) : "other"}`, {
          number: this.target.affects.count ? formatNumber(this.target.affects.count)
            : game.i18n.localize(`NIH.TARGET.Count.${this.target.template.type ? "Every" : "Any"}`)
        }
      ).trim().capitalize() : (affectsConfig?.label ?? ""),
      statblock: game.i18n.format(
        `${affectsConfig?.counted ?? "NIH.TARGET.Type.Target.Counted"}.${pr.select(this.target.affects.count || 1)}`,
  ...
  static templateDimensions(type) {
    const sizes = CONFIG.NIH.areaTargetTypes[type]?.sizes;
    const dimensions = { size: "NIH.AreaOfEffect.Size.Label" };
    if ( sizes ) {
      dimensions.width = sizes.includes("width") && (sizes.includes("length") || sizes.includes("radius"));
      dimensions.height = sizes.includes("height");
      if ( sizes.includes("radius") ) dimensions.size = "NIH.AreaOfEffect.Size.Radius";
      else if ( sizes.includes("length") ) dimensions.size = "NIH.AreaOfEffect.Size.Length";
      else if ( sizes.includes("width") ) dimensions.size = "NIH.AreaOfEffect.Size.Width";
      if ( sizes.includes("thickness") ) dimensions.width = "NIH.AreaOfEffect.Size.Thickness";
      else if ( dimensions.width ) dimensions.width = "NIH.AreaOfEffect.Size.Width";
      if ( dimensions.height ) dimensions.height = "NIH.AreaOfEffect.Size.Height";
    }
    return dimensions;
  }
}
