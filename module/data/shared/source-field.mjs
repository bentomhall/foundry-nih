...
export default class SourceField extends SchemaField {
  constructor(fields={}, options={}) {
    fields = {
      ...
      rules: new StringField({
        initial: () => game.settings.get("nih", "rulesVersion") === "modern" ? "2024" : "2014"
      }),
      ...fields
    };
    Object.entries(fields).forEach(([k, v]) => !v ? delete fields[k] : null);
    super(fields, { label: "NIH.SOURCE.FIELDS.source.label", ...options });
  }
  ...
  static prepareData(uuid) {
    ...
    this.bookPlaceholder = collection?.metadata?.flags?.nih?.sourceBook ?? SourceField.getModuleBook(pkg) ?? "";
    if ( !this.book ) this.book = this.bookPlaceholder;

    if ( this.custom ) this.label = this.custom;
    else {
      const page = Number.isNumeric(this.page)
        ? game.i18n.format("NIH.SOURCE.Display.Page", { page: this.page }) : (this.page ?? "");
      this.label = game.i18n.format("NIH.SOURCE.Display.Full", { book: this.book, page }).trim();
    }
  ...
  static getModuleBook(pkg) {
    if ( !pkg ) return null;
    const sourceBooks = pkg.flags?.nih?.sourceBooks;
