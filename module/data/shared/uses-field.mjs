...
export default class UsesField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    prepareFormulaValue(this, "uses.max", "NIH.USES.FIELDS.uses.max.label", rollData);
    ...
    for ( const recovery of this.uses.recovery ) {
      if ( recovery.period === "recharge" ) {
        ...
        if ( labels ) labels.recharge ??= `${game.i18n.localize("NIH.Recharge")} [${
          recovery.formula}${parseInt(recovery.formula) < 6 ? "+" : ""}]`;
      } else if ( recovery.period in CONFIG.NIH.limitedUsePeriods ) {
        const config = CONFIG.NIH.limitedUsePeriods[recovery.period];
  ...
  static get rechargeOptions() {
    return Array.fromRange(5, 2).reverse().map(min => ({
      value: min,
      label: game.i18n.format("NIH.USES.Recovery.Recharge.Range", {
  ...
  static getStatblockLabel() {
    ...
    if ( (this.activation?.type === "legendary") || (this.activation?.type === "mythic") ) {
      ...
      return game.i18n.format(`NIH.NPC.ActionCostCounted.${pr.select(this.activation.value)}`, {
    ...
    const type = CONFIG.NIH.limitedUsePeriods[recovery.period]?.type;
    if ( (type === "combat") || (type === "special") ) return "";

    // Recharge X–Y
    if ( recovery.period === "recharge" ) {
      const value = parseInt(recovery.formula);
      return `${game.i18n.localize("NIH.Recharge")} ${value === 6 ? "6" : `${value}–6`}`;
    }

    // Recharge after a Short or Long Rest
    if ( ["lr", "sr"].includes(recovery.period) && (this.uses.max === 1) ) {
      return game.i18n.localize(`NIH.Recharge${recovery.period === "sr" ? "Short" : "Long"}`);
    }

    // X/Day
    const period = CONFIG.NIH.limitedUsePeriods[recovery.period === "sr" ? "sr" : "day"]?.label ?? "";
    if ( !period ) return "";
    ...
    if ( profile.type === "recoverAll" ) updates.spent = 0;
      ...
      try {
        ...
        if ( ["day", "dawn", "dusk"].includes(profile.period)
          && (game.settings.get("nih", "restVariant") === "gritty") ) {
          roll.alter(7, 0, { multiplyNumeric: true });
        }
        total = (await roll.evaluate()).total;
      } catch(err) {
        Hooks.onError("UsesField#recoverUses", err, {
          msg: game.i18n.format("NIH.ItemRecoveryFormulaWarning", {
  ...
  static async rollRecharge(config={}, dialog={}, message={}) {
    ...
    messageConfig.data.flavor = game.i18n.format("NIH.ItemRechargeCheck", {
      name: this.name,
      result: game.i18n.localize(`NIH.ItemRecharge${rolls[0].isSuccess ? "Success" : "Failure"}`)
    });
     ...
     */
    if ( Hooks.call("nih.rollRecharge", rolls, { subject: this, updates }) === false ) return rolls;
    if ( Hooks.call("nih.rollRechargeV2", rolls, { subject: this, updates }) === false ) return rolls;
     ...
     * @function nih.postRollRecharge
     * @memberof hookEvents
     * @param {BasicRoll[]} rolls     The resulting rolls.
     * @param {object} data
     * @param {Actor5e} data.subject  Item or Activity for which the roll was performed.
     */
    Hooks.callAll("nih.postRollRecharge", rolls, { subject: this });
