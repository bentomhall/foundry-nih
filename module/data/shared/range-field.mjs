...
export default class RangeField extends SchemaField {
  ...
  static prepareData(rollData, labels) {
    this.range.scalar = this.range.units in CONFIG.NIH.movementUnits;
    if ( this.range.scalar ) {
      prepareFormulaValue(this, "range.value", "NIH.RANGE.FIELDS.range.value.label", rollData);
    ...
    if ( labels && this.range.units ) {
      if ( this.range.scalar && this.range.value ) {
        ...
        labels.range = CONFIG.NIH.distanceUnits[this.range.units];
      }
    } else if ( labels ) labels.range = game.i18n.localize("NIH.DistSelf");
  }
}
