 ...
 * @property {string} activation.type       Activation type as defined in `NIH.abilityActivationTypes`.
 ...
 * @property {string} duration.units        Time duration period as defined in `NIH.timePeriods`.
 ...
 * @property {string} target.units          Units used for value and width as defined in `NIH.distanceUnits`.
 * @property {string} target.type           Targeting mode as defined in `NIH.targetTypes`.
 ...
 * @property {string} range.units           Units used for value and long as defined in `NIH.distanceUnits`.
 ...
 * @property {string} uses.per              Recharge time for limited uses as defined in `NIH.limitedUsePeriods`.
 * @property {object} consume               Effect's resource consumption.
 * @property {string} consume.type          Type of resource to consume as defined in `NIH.abilityConsumptionTypes`.
...
export default class ActivatedEffectTemplate extends foundry.abstract.DataModel {
  ...
  static defineSchema() {
    return {
      activation: new foundry.data.fields.SchemaField({
        type: new foundry.data.fields.StringField({required: true, blank: true, label: "NIH.ItemActivationType"}),
        cost: new foundry.data.fields.NumberField({required: true, label: "NIH.ItemActivationCost"}),
        condition: new foundry.data.fields.StringField({required: true, label: "NIH.ItemActivationCondition"})
      }, {label: "NIH.ItemActivation"}),
      duration: new foundry.data.fields.SchemaField({
        value: new FormulaField({required: true, deterministic: true, label: "NIH.Duration"}),
        units: new foundry.data.fields.StringField({required: true, blank: true, label: "NIH.DurationType"})
      }, {label: "NIH.Duration"}),
      cover: new foundry.data.fields.NumberField({
        required: true, nullable: true, min: 0, max: 1, label: "NIH.Cover"
      }),
      crewed: new foundry.data.fields.BooleanField({label: "NIH.Crewed"}),
      target: new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({required: true, min: 0, label: "NIH.TargetValue"}),
        width: new foundry.data.fields.NumberField({required: true, min: 0, label: "NIH.TargetWidth"}),
        units: new foundry.data.fields.StringField({required: true, blank: true, label: "NIH.TargetUnits"}),
        type: new foundry.data.fields.StringField({required: true, blank: true, label: "NIH.TargetType"})
      }, {label: "NIH.Target"}),
      range: new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({required: true, min: 0, label: "NIH.RangeNormal"}),
        long: new foundry.data.fields.NumberField({required: true, min: 0, label: "NIH.RangeLong"}),
        units: new foundry.data.fields.StringField({required: true, blank: true, label: "NIH.RangeUnits"})
      }, {label: "NIH.Range"}),
      uses: new this.ItemUsesField({}, {label: "NIH.LimitedUses"}),
      consume: new foundry.data.fields.SchemaField({
        type: new foundry.data.fields.StringField({required: true, blank: true, label: "NIH.ConsumeType"}),
        target: new foundry.data.fields.StringField({
          required: true, nullable: true, initial: null, label: "NIH.ConsumeTarget"
        }),
        amount: new foundry.data.fields.NumberField({required: true, integer: true, label: "NIH.ConsumeAmount"})
      }, {label: "NIH.ConsumeTitle"})
    };
  }
    ...
    constructor(extraSchema, options) {
      super(SystemDataModel.mergeSchema({
        value: new foundry.data.fields.NumberField({
          required: true, min: 0, integer: true, label: "NIH.LimitedUsesAvailable"
        }),
        max: new FormulaField({required: true, deterministic: true, label: "NIH.LimitedUsesMax"}),
        per: new foundry.data.fields.StringField({
          required: true, nullable: true, blank: false, initial: null, label: "NIH.LimitedUsesPer"
        }),
        recovery: new FormulaField({required: true, label: "NIH.RecoveryFormula"})
      }, extraSchema), options);
    }
  };
   ...
   */
  get hasAreaTarget() {
    return this.target.type in CONFIG.NIH.areaTargetTypes;
  }
  ...
  get hasIndividualTarget() {
    return this.target.type in CONFIG.NIH.individualTargetTypes;
  }
  ...
  get hasScalarDuration() {
    return this.duration.units in CONFIG.NIH.scalarTimePeriods;
  }
   ...
   */
  get hasScalarRange() {
    return this.range.units in CONFIG.NIH.movementUnits;
  }
