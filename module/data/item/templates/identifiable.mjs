...
export default class IdentifiableTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      identified: new BooleanField({ required: true, initial: true, label: "NIH.Identified" }),
      unidentified: new SchemaField({
        name: new StringField({ label: "NIH.NameUnidentified" }),
        description: new HTMLField({ label: "NIH.DescriptionUnidentified" })
      })
    };
  }
  ...
  async preUpdateIdentifiable(changed, options, user) {
    ...
    if ( baseItem ) {
      if ( fetchName ) {
        foundry.utils.setProperty(changed, "system.unidentified.name", game.i18n.format(
          "NIH.Unidentified.DefaultName", { name: baseItem.name }
        ));
      }
      if ( fetchDesc ) {
    ...
    if ( fetchName ) foundry.utils.setProperty(changed, "system.unidentified.name", game.i18n.format(
      "NIH.Unidentified.DefaultName", { name: game.i18n.localize(CONFIG.Item.typeLabels[this.parent.type]) }
    ));
  }
}
