...
export default class ItemDescriptionTemplate extends SystemDataModel {
  ...
  static defineSchema() {
    return {
      description: new SchemaField({
        value: new HTMLField({ required: true, nullable: true, label: "NIH.Description" }),
        chat: new HTMLField({ required: true, nullable: true, label: "NIH.DescriptionChat" })
      }),
   ...
   */
  get validProperties() {
    return new Set(CONFIG.NIH.validProperties[this.parent.type] ?? []);
  }
  ...
  prepareDescriptionData() {
    const uuid = this.parent.flags.nih?.sourceId ?? this.parent._stats?.compendiumSource ?? this.parent.uuid;
  ...
  static compendiumBrowserPropertiesFilter(type) {
    return {
      label: "NIH.Properties",
      type: "set",
      config: {
        choices: Object.entries(CONFIG.NIH.itemProperties).reduce((obj, [k, v]) => {
          if ( CONFIG.NIH.validProperties[type]?.has(k) ) obj[k] = v;
          return obj;
        }, {}),
