...
export default class FacilityData extends ItemDataModel.mixin(ActivitiesTemplate, ItemDescriptionTemplate) {
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NIH.FACILITY", "NIH.SOURCE"];
  ...
  static get compendiumBrowserFilters() {
    ...
    return new Map([
      ["level", {
        label: "NIH.FACILITY.FIELDS.level.label",
        type: "range",
        config: {
          keyPath: "system.level",
          min,
          max: CONFIG.NIH.maxLevel
        }
      }],
      ["type", {
        label: "NIH.FACILITY.FIELDS.type.value.label",
        type: "set",
        config: {
          choices: Object.fromEntries(Object.entries(CONFIG.NIH.facilities.types).map(([k, v]) => [k, v.label])),
  ...
  _createOrderActivity(id, order) {
    const activity = new OrderActivity({
      order,
      _id: staticID(id),
      name: game.i18n.format("NIH.FACILITY.Order.Issue", {
        order: game.i18n.localize(`NIH.FACILITY.Orders.${order}.inf`)
      })
    }, { parent: this.parent });
  ...
  prepareBaseData() {
    ...
    // Activities
    if ( (this.type.value === "special") && this.order ) this._createOrderActivity("nihFacOrder", this.order);
    if ( this.enlargeable ) this._createOrderActivity("nihFacEnlarge", "enlarge");
    if ( !this.building.built ) this._createOrderActivity("nihFacBuild", "build");

    // TODO: Allow order activities to be user-creatable and configurable to avoid having to hard-code this.
    if ( this.type.subtype === "garden" ) this._createOrderActivity("nihFacChange", "change");
  }
  ...
  prepareDerivedData() {
    ...
    // Type
    const config = CONFIG.NIH.facilities.types[this.type.value];
    this.type.label = config?.subtypes?.[this.type.subtype] ?? config?.label;

    // Price
    if ( this.type.value === "basic" ) {
      const { value, days } = CONFIG.NIH.facilities.sizes[this.size];
      this.price = { value, days, denomination: "gp" };
    }

    // Squares
    this.squares = CONFIG.NIH.facilities.sizes[this.size].squares;
    ...
    if ( this.order ) labels.order = game.i18n.localize(`NIH.FACILITY.Orders.${this.order}.present`);
  }
  ...
  async getSheetData(context) {
    ...
    context.subtitles = [
      ...
      { label: CONFIG.NIH.facilities.sizes[this.size].label }
    ];

    context.parts = ["nih.details-facility"];
    context.facilitySubtypes = CONFIG.NIH.facilities.types[this.type.value]?.subtypes ?? {};
    context.orders = Object.entries(CONFIG.NIH.facilities.orders).reduce((obj, [value, config]) => {
  ...
  async _preCreate(data, options, user) {
    ...
    if ( !actor ) return;
    const { basic } = CONFIG.NIH.facilities.advancement;
