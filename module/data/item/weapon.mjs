...
export default class WeaponData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NIH.WEAPON", "NIH.VEHICLE.MOUNTABLE", "NIH.RANGE", "NIH.SOURCE"];
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ...
      magicalBonus: new NumberField({ min: 0, integer: true, label: "NIH.MagicalBonus" }),
      mastery: new StringField(),
      properties: new SetField(new StringField(), { label: "NIH.ItemWeaponProperties" }),
      ...
      type: new ItemTypeField({ value: "simpleM", subtype: false }, {label: "NIH.ItemWeaponType"})
    });
  }
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["type", {
        label: "NIH.ItemWeaponType",
        type: "set",
        config: {
          choices: CONFIG.NIH.weaponTypes,
      ...
      ["mastery", {
        ...
        type: "set",
        config: {
          choices: CONFIG.NIH.weaponMasteries,
  ...
  get attackClassification() {
    return CONFIG.NIH.weaponClassificationMap[this.type.value] ?? "weapon";
  }
  ...
  get attackModes() {
    ...
    if ( !(this.properties.has("thr") && (this.attackType === "ranged")) ) {
      ...
      if ( this.isVersatile || !this.properties.has("two") ) modes.push({
        value: "oneHanded", label: CONFIG.NIH.attackModes.oneHanded.label
      });

      // Weapons with the "Two-Handed" property or with the "Versatile" property will have Two-Handed attack
      if ( this.isVersatile || this.properties.has("two") ) modes.push({
        value: "twoHanded", label: CONFIG.NIH.attackModes.twoHanded.label
      });
    }

    const isLight = this.properties.has("lgt") || (this.parent.actor?.getFlag("nih", "enhancedDualWielding")
    ...
    if ( isLight ) modes.push({
      value: "offhand", label: CONFIG.NIH.attackModes.offhand.label
    });

    // Weapons with the "Thrown" property will have Thrown attack
    if ( this.properties.has("thr") ) {
      if ( modes.length ) modes.push({ rule: true });
      modes.push({ value: "thrown", label: CONFIG.NIH.attackModes.thrown.label });

      // Weapons with the "Thrown" & "Light" properties will have an Offhand Throw attack
      if ( isLight ) modes.push({
        value: "thrown-offhand", label: CONFIG.NIH.attackModes["thrown-offhand"].label
      });
    }

    else if ( !this.attackType && ((this.range.value ?? 0) > (this.range.reach ?? 0)) ) {
      if ( modes.length ) modes.push({ rule: true });
      modes.push({ value: "ranged", label: CONFIG.NIH.attackModes.ranged.label });
    }
   ...
   */
  get attackType() {
    return CONFIG.NIH.weaponTypeMap[this.type.value] ?? null;
  }
  ...
  get availableAbilities() {
    const melee = CONFIG.NIH.defaultAbilities.meleeAttack;
    const ranged = CONFIG.NIH.defaultAbilities.rangedAttack;
  ...
  get chatProperties() {
    return [
      this.type.label,
      CONFIG.NIH.weaponMasteries[this.mastery]?.label,
  ...
  get criticalThreshold() {
    return this.parent?.actor?.flags.nih?.weaponCriticalThreshold ?? Infinity;
  }
  ...
  get masteryOptions() {
    ...
    for ( const mastery of this.parent.actor.system.traits.weaponProf.mastery.bonus ?? [] ) {
      ...
      extras.push({ value: mastery, label: CONFIG.NIH.weaponMasteries[mastery]?.label ?? mastery });
    }
    return [{
      value: this.mastery,
      label: CONFIG.NIH.weaponMasteries[this.mastery]?.label ?? this.mastery,
      rule: !!extras.length
    }, ...extras];
  }
  ...
  get proficiencyMultiplier() {
    ...
    const config = CONFIG.NIH.weaponProficienciesMap;
    ...
    const improvised = (this.type.value === "improv") && !!actor.getFlag("nih", "tavernBrawlerFeat");
  ...
  /** @inheritDoc */
  get tooltipSubtitle() {
    return [...super.tooltipSubtitle, CONFIG.NIH.weaponMasteries[this.mastery]?.label];
  }
  ...
  static #migrateReach(source) {
    ...
    if ( (CONFIG.NIH.weaponTypeMap[source.type.value] !== "melee") || source.properties.includes("thr") ) return;
  ...
  prepareDerivedData() {
    ...
    this.type.label = CONFIG.NIH.weaponTypes[this.type.value] ?? game.i18n.localize(CONFIG.Item.typeLabels.weapon);
    this.type.identifier = CONFIG.NIH.weaponIds[this.type.baseItem];

    const labels = this.parent.labels ??= {};
    labels.armor = this.armor.value ? `${this.armor.value} ${game.i18n.localize("NIH.AC")}` : "";
    labels.damage = this.damage.base.formula;
    labels.damageTypes = game.i18n.getListFormatter({ style: "narrow" }).format(
      Array.from(this.damage.base.types).map(t => CONFIG.NIH.damageTypes[t]?.label).filter(t => t)
    );
  ...
  prepareFinalData() {
    ...
    if ( reach ) labels.reach = game.i18n.format("NIH.RANGE.Formatted.Reach", { reach: formatLength(reach, units) });
  }
  ...
  async getFavoriteData() {
    return foundry.utils.mergeObject(await super.getFavoriteData(), {
      subtitle: CONFIG.NIH.itemActionTypes[this.activities.contents[0]?.actionType],
  ...
  async getSheetData(context) {
    ...
    context.info = [{
      label: "NIH.ToHit",
      classes: "info-lg",
      value: nih.utils.formatModifier(parseInt(this.parent.labels.modifier))
    }];
    if ( this.parent.labels.damages?.length ) {
      const config = { ...CONFIG.NIH.damageTypes, ...CONFIG.NIH.healingTypes };
      context.info.push({ value: this.parent.labels.damages.reduce((str, { formula, damageType, firstDamage }) => {
        ...
        return `${str}
          ...
          ${type ? `<span class="damage-type" data-tooltip aria-label="${type.label}">
            <nih-icon src="${type.icon}"></nih-icon>
          </span>` : ""}
        `;
      }, ""), classes: "info-grid damage" });
    }

    context.parts = ["nih.details-weapon", "nih.field-uses"];
    context.damageTypes = Object.entries(CONFIG.NIH.damageTypes).map(([value, { label }]) => {
      return {
        value, label,
    ...
    const makeDenominationOptions = placeholder => [
      ...
      { rule: true },
      ...CONFIG.NIH.dieSteps.map(value => ({ value, label: `d${value}` }))
    ];
  ...
  async _preCreate(data, options, user) {
    ...
    const activityData = new CONFIG.NIH.activityTypes.attack.documentClass({}, { parent: this.parent }).toObject();
