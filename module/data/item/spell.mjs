 ...
 * @property {string} preparation.mode           Spell preparation mode as defined in `NIH.spellPreparationModes`.
 ...
 * @property {string} scaling.mode               Spell scaling mode as defined in `NIH.spellScalingModes`.
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      level: new foundry.data.fields.NumberField({
        required: true, integer: true, initial: 1, min: 0, label: "NIH.SpellLevel"
      }),
      school: new foundry.data.fields.StringField({required: true, label: "NIH.SpellSchool"}),
      components: new MappingField(new foundry.data.fields.BooleanField(), {
        required: true, label: "NIH.SpellComponents",
        initialKeys: [...Object.keys(CONFIG.NIH.spellComponents), ...Object.keys(CONFIG.NIH.spellTags)]
      }),
      materials: new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.StringField({required: true, label: "NIH.SpellMaterialsDescription"}),
        consumed: new foundry.data.fields.BooleanField({required: true, label: "NIH.SpellMaterialsConsumed"}),
        cost: new foundry.data.fields.NumberField({
          required: true, initial: 0, min: 0, label: "NIH.SpellMaterialsCost"
        }),
        supply: new foundry.data.fields.NumberField({
          required: true, initial: 0, min: 0, label: "NIH.SpellMaterialsSupply"
        })
      }, {label: "NIH.SpellMaterials"}),
      preparation: new foundry.data.fields.SchemaField({
        mode: new foundry.data.fields.StringField({
          required: true, initial: "prepared", label: "NIH.SpellPreparationMode"
        }),
        prepared: new foundry.data.fields.BooleanField({required: true, label: "NIH.SpellPrepared"})
      }, {label: "NIH.SpellPreparation"}),
      scaling: new foundry.data.fields.SchemaField({
        mode: new foundry.data.fields.StringField({required: true, initial: "none", label: "NIH.ScalingMode"}),
        formula: new FormulaField({required: true, nullable: true, initial: null, label: "NIH.ScalingFormula"})
      }, {label: "NIH.LevelScaling"})
    });
  }
