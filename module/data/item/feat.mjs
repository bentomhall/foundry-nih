...
export default class FeatData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NIH.FEATURE", "NIH.ENCHANTMENT", "NIH.Prerequisites", "NIH.SOURCE"];
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["category", {
        label: "NIH.ITEM.Category.Label",
        type: "set",
        config: {
          choices: CONFIG.NIH.featureTypes,
          keyPath: "system.type.value"
        }
      }],
      ["subtype", {
        label: "NIH.ItemFeatureType",
        type: "set",
        config: {
          choices: filters => {
            ...
            return Object.entries(CONFIG.NIH.featureTypes).reduce((obj, [type, config]) => {
      ...
      ["abilityScoreImprovement", {
        label: "NIH.ADVANCEMENT.AbilityScoreImprovement.Title",
        type: "set",
        config: {
          choices: CONFIG.NIH.abilities
        },
  ...
  get isEnchantmentSource() {
    return CONFIG.NIH.featureTypes[this.type?.value]?.subtypes?.[this.type?.subtype]
      && (this.type?.subtype in CONFIG.NIH.featureTypes.enchantment.subtypes);
  }
  ...
  prepareDerivedData() {
    ...
    if ( this.type.value ) {
      const config = CONFIG.NIH.featureTypes[this.type.value];
    ...
    if ( activation === "legendary" ) label = game.i18n.localize("NIH.LegendaryAction.Label");
    if ( activation === "mythic" ) label = game.i18n.localize("NIH.MythicActionLabel");
    else if ( activation === "lair" ) label = game.i18n.localize("NIH.LAIR.Action.Label");
    else if ( activation === "action" && this.hasAttack ) label = game.i18n.localize("NIH.Attack");
    else if ( activation ) label = game.i18n.localize("NIH.Action");
    else label = game.i18n.localize("NIH.Passive");
    this.parent.labels ??= {};
  ...
  async getSheetData(context) {
    context.subtitles = [
      ...
      { label: this.requirements, value: this._source.requirements, field: this.schema.getField("requirements"),
        placeholder: "NIH.Requirements" }
    ];

    context.parts = ["nih.details-feat", "nih.field-uses"];
    const itemTypes = CONFIG.NIH.featureTypes[this._source.type.value];
    if ( itemTypes ) {
  ...
  validatePrerequisites(actor, {
    ...
    if ( !this.prerequisites.repeatable && actor.sourcedItems?.get(this.parent.uuid)?.size ) {
      messages.push(game.i18n.localize("NIH.Prerequisites.Warning.NotRepeatable"));
    }
    ...
    if ( !someExist ) {
      messages.push(game.i18n.format("NIH.Prerequisites.Warning.MissingItem", {
    ...
    if ( (this.prerequisites?.level ?? -Infinity) > (level ?? Infinity) ) {
      messages.push(game.i18n.format("NIH.Prerequisites.Warning.InvalidLevel", {
    ...
    if ( showMessage || throwError ) {
      const message = game.i18n.format("NIH.Prerequisites.Warning.Message", {
        actor: actor.name,
