...
export default class ClassData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NIH.CLASS", "NIH.SOURCE"];
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["hasSpellcasting", {
        label: "NIH.CompendiumBrowser.Filters.HasSpellcasting",
        type: "boolean",
  ...
  static _migrateTraitAdvancement(source) {
    ...
    if ( needsMigration ) foundry.utils.setProperty(source, "flags.nih.persistSourceMigration", true);
  }
  ...
  async getSheetData(context) {
    ...
    context.parts = ["nih.details-class", "nih.details-spellcasting", "nih.details-starting-equipment"];
    context.hitDieOptions = CONFIG.NIH.hitDieTypes.map(d => ({ value: d, label: d }));
    context.primaryAbilities = Object.entries(CONFIG.NIH.abilities).map(([value, data]) => ({
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( changed.system.levels <= 0 ) {
      ui.notifications.warn("NIH.MaxClassLevelMinimumWarn", { localize: true });
