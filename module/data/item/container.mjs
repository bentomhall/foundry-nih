 ...
 * @property {string} capacity.type         Method for tracking max capacity as defined in `NIH.itemCapacityTypes`.
    ...
    return this.mergeSchema(super.defineSchema(), {
      capacity: new foundry.data.fields.SchemaField({
        type: new foundry.data.fields.StringField({
          required: true, initial: "weight", blank: false, label: "NIH.ItemContainerCapacityType"
        }),
        value: new foundry.data.fields.NumberField({
          required: true, min: 0, label: "NIH.ItemContainerCapacityMax"
        }),
        weightless: new foundry.data.fields.BooleanField({required: true, label: "NIH.ItemContainerWeightless"})
      }, {label: "NIH.ItemContainerCapacity"})
    });
  }
}
