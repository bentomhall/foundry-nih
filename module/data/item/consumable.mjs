...
export default class ConsumableData extends ItemDataModel.mixin(
  ...
  /** @override */
  static LOCALIZATION_PREFIXES = ["NIH.CONSUMABLE", "NIH.SOURCE"];
  ...
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      ...
      type: new ItemTypeField({ baseItem: false }, { label: "NIH.ItemConsumableType" }),
      uses: new UsesField({
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["type", {
        label: "NIH.ItemConsumableType",
        type: "set",
        config: {
          choices: CONFIG.NIH.consumableTypes,
  ...
  get chatProperties() {
    return [
      this.type.label,
      this.hasLimitedUses ? `${this.uses.value}/${this.uses.max} ${game.i18n.localize("NIH.Charges")}` : null,
      this.priceLabel
    ];
  }
  ...
  static get itemCategories() {
    return CONFIG.NIH.consumableTypes;
  }
  ...
  get proficiencyMultiplier() {
    const isProficient = this.parent?.actor?.getFlag("nih", "tavernBrawlerFeat");
  ...
  get validProperties() {
    ...
    if ( this.type.value === "ammo" ) Object.entries(CONFIG.NIH.itemProperties).forEach(([k, v]) => {
      if ( v.isPhysical ) valid.add(k);
      valid.add("ret");
    });
    else if ( this.type.value === "scroll" ) CONFIG.NIH.validProperties.spell
  ...
  prepareDerivedData() {
    ...
    const config = CONFIG.NIH.consumableTypes[this.type.value];
    if ( config ) {
  ...
  async getSheetData(context) {
    ...
    context.parts = ["nih.details-consumable", "nih.field-uses"];
    ...
    const itemTypes = CONFIG.NIH.consumableTypes[this._source.type.value];
    if ( itemTypes ) {
  ...
  async getCraftCost(options={}) {
    ...
    const { consumable, magic } = CONFIG.NIH.crafting;
    const { rarity } = this;
  ...
  getRollData(...options) {
    ...
    const spellLevel = this.parent.getFlag("nih", "spellLevel");
