...
export default class RaceData extends ItemDataModel.mixin(AdvancementTemplate, ItemDescriptionTemplate) {
  ...
  static get compendiumBrowserFilters() {
    return new Map([
      ["hasDarkvision", {
        label: "NIH.CompendiumBrowser.Filters.HasDarkvision",
        type: "boolean",
  ...
  get movementLabels() {
    ...
    return Object.entries(CONFIG.NIH.movementTypes).reduce((obj, [k, { label }]) => {
  ...
  get sensesLabels() {
    ...
    return Object.entries(CONFIG.NIH.senses).reduce((arr, [k, label]) => {
  ...
  async getSheetData(context) {
    ...
    context.parts = ["nih.details-species"];
    context.info = [{
      label: "NIH.CreatureType",
      classes: "info-sm",
      value: this.typeLabel,
      config: "type",
      tooltip: "NIH.CreatureTypeTitle"
    },
    {
      label: "NIH.Movement",
      classes: "info-sm info-grid",
      config: "movement",
      tooltip: "NIH.MOVEMENT.Action.Configure",
      value: Object.entries(CONFIG.NIH.movementTypes).reduce((str, [k, { label }]) => {
        const value = this.movement[k];
        if ( !value ) return str;
        return `${str}
    ...
    },
    {
      ...
      classes: "info-sm info-grid",
      config: "senses",
      tooltip: "NIH.SensesConfig",
      value: Object.entries(CONFIG.NIH.senses).reduce((str, [k, label]) => {
        const value = this.senses[k];
        if ( !value ) return str;
        return `${str}
          <span class="key">${label}</span>
