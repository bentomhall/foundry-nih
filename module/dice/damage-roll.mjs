...
export default class DamageRoll extends BasicRoll {
  ...
  static async build(config={}, dialog={}, message={}) {
    config.critical ??= {};
    config.critical.multiplyNumeric ??= game.settings.get("nih", "criticalDamageModifiers");
    config.critical.powerfulCritical ??= game.settings.get("nih", "criticalDamageMaxDice");
      ...
      if ( term instanceof DiceTerm ) {
        ...
        if ( this.isCritical ) {
          ...
          if ( critical.powerfulCritical ) {
            ...
            if ( bonus > 0 ) {
              const flavor = term.flavor?.toLowerCase().trim() ?? game.i18n.localize("NIH.PowerfulCritical");
