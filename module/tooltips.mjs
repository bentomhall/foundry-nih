...
export default class Tooltips5e {
  ...
  async _onTooltipActivate() {
    ...
    if ( loading?.dataset.uuid ) {
      ...
      if ( doc instanceof nih.documents.Actor5e ) return this._onHoverActor(doc);
      return this._onHoverContentLink(doc);
    }
  ...
  async _onHoverPassiveLanguage(language) {
    const label = CONFIG.NIH.enrichmentLookup.languages[language];
  ...
  async _onHoverPassiveCheck(skill, ability, dc) {
    const skillConfig = CONFIG.NIH.skills[skill];
    const abilityConfig = CONFIG.NIH.abilities[ability ?? skillConfig.ability];

    let label;
    if ( skillConfig ) {
      label = game.i18n.format("NIH.SkillPassiveSpecificHint", { skill: skillConfig.label, ability: abilityConfig.label });
    } else {
      ...
      label = game.i18n.format("NIH.SkillPassiveHint", { skill: abilityConfig.label });
    }
  ...
  async _onHoverPassive({ label }, memberCallback) {
    ...
    this.tooltip.classList.add("nih-tooltip", "passive-tooltip", "nih2", "themed", "theme-light");
    this.tooltip.classList.remove("theme-dark");
    this.tooltip.innerHTML = await foundry.applications.handlebars.renderTemplate(
      "systems/nih/templates/journal/passive-tooltip.hbs", context
    );
