...
export class ModuleArt {
  ...
  async registerModuleArt() {
    ...
    for ( const module of game.modules ) {
      ...
      try {
        ...
        await this.#parseArtMapping(module.id, mapping, flags["nih-art-credit"]);
      } catch( e ) {
        console.error(e);
      }
    }
    ...
    // Load system mapping.
    try {
      const mapping = await foundry.utils.fetchJsonWithTimeout("systems/nih/json/fa-token-mapping.json");
      const credit = `
        <em>
          Token artwork by
  ...
  async #parseArtMapping(moduleId, mapping, credit) {
    let settings = game.settings.get("nih", "moduleArtConfiguration")?.[moduleId];
...
export class ModuleArtConfig extends FormApplication {
  ...
  constructor(object={}, options={}) {
    object = foundry.utils.mergeObject(game.settings.get("nih", "moduleArtConfiguration"), object, {inplace: false});
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      title: game.i18n.localize("NIH.ModuleArtConfigL"),
      id: "module-art-config",
      template: "systems/nih/templates/apps/module-art-config.html",
      popOut: true,
      width: 600,
      height: "auto"
    });
  ...
  getData(options={}) {
    ...
    context.config.unshift({label: game.system.title, id: game.system.id, ...this.object.nih});
    return context;
  }
  ...
  async _updateObject(event, formData) {
    await game.settings.set("nih", "moduleArtConfiguration", foundry.utils.expandObject(formData));
