...
export default class ModuleArt {
  ...
  async registerModuleArt() {
    ...
    for ( const { id, mapping, credit } of this.constructor.getArtModules().reverse() ) {
      foundry.utils.logCompatibilityWarning(
        "The nih `ModuleArt` system has been deprecated and replaced with core's `CompendiumArt` system.",
        { since: "Nih 5.0", until: "Nih 6.0", once: true }
      );
      try {
  ...
  static getArtModules() {
    const settings = game.settings.get("nih", "moduleArtConfiguration");
    const unsorted = [];
    const configs = [];
    ...
    for ( const module of game.modules ) {
      ...
      if ( !mapping ) continue;
      const config = { id: module.id, label: module.title, credit: flags?.["nih-art-credit"], mapping };
      configs.push(config);
